...
import {
  ...
  MORNING_EASTER_EGGS,
  MORNING_NARRATIONS_SPRING,
  MORNING_NARRATIONS_SUMMER,
  MORNING_NARRATIONS_AUTUMN,
  MORNING_NARRATIONS_WINTER,
  MORNING_NARRATIONS_RAINY,
  MORNING_NARRATIONS_SNOWY,
  MORNING_CHOICE_EVENTS_SPRING,
  MORNING_CHOICE_EVENTS_SUMMER,
  MORNING_CHOICE_EVENTS_AUTUMN,
  MORNING_CHOICE_EVENTS_WINTER,
  MORNING_CHOICE_EVENTS_RAINY
...
const applyEventEffects = (event: { id: string; name: string; description: string; effects: any }) => {
  ...
  if (effects.itemReward) {
    ...
    if (inventoryStore.canAddItems(rewardItems)) {
      ...
      addLog(`【${event.name}】赠礼因背包空间不足未能领取。`)
    }
  }
  ...
  switch (effect.type) {
    ...
    case 'gainFriendship':
      for (const s of npcStore.npcStates) {
        ...
        s.friendship = Math.min(s.friendship + effect.amount, cap)
      }
      break
  }
}
...
const rollMorningEvent = ():
  ...
  const isRainy = weather === 'rainy' || weather === 'stormy' || weather === 'green_rain'
  ...
  if (roll < 0.01) {
    // 合并通用 + 季节 + 天气 选项事件池
    const seasonChoiceMap = {
      spring: MORNING_CHOICE_EVENTS_SPRING,
      summer: MORNING_CHOICE_EVENTS_SUMMER,
      autumn: MORNING_CHOICE_EVENTS_AUTUMN,
      winter: MORNING_CHOICE_EVENTS_WINTER
    }
    const weatherChoices = isRainy ? MORNING_CHOICE_EVENTS_RAINY : []
    const choicePool = [
      ...MORNING_CHOICE_EVENTS,
      ...seasonChoiceMap[season],
      ...weatherChoices
    ]
    const event = choicePool[Math.floor(Math.random() * choicePool.length)]!
  ...
  const seasonNarrationMap = {
    spring: MORNING_NARRATIONS_SPRING,
    summer: MORNING_NARRATIONS_SUMMER,
    autumn: MORNING_NARRATIONS_AUTUMN,
    winter: MORNING_NARRATIONS_WINTER
  }
  const weatherNarrations = isRainy ? MORNING_NARRATIONS_RAINY : isSnowy ? MORNING_NARRATIONS_SNOWY : []
  const basePool = hasCrops ? MORNING_NARRATIONS : NARRATIONS_NO_LOSS
  const narrationPool = [
    ...basePool,
    ...seasonNarrationMap[season],
    ...weatherNarrations
  ]
...
const getEconomyDayTag = (year: number, season: string, day: number) => `${year}-${season}-${day}`

const diffNumericRecord = (before: Record<string, number> | undefined, after: Record<string, number> | undefined) => {
  const keys = new Set([...Object.keys(before ?? {}), ...Object.keys(after ?? {})])
  return Object.fromEntries(
    [...keys]
      .map(key => [key, Math.max(0, Number(after?.[key] ?? 0) - Number(before?.[key] ?? 0))] as const)
      .filter(([, value]) => value > 0)
  )
}

const buildWeeklyEconomyRiskReport = (playerStore: ReturnType<typeof usePlayerStore>) => {
  const overview = playerStore.getEconomyOverview()
  const triggeredMetricIds: string[] = []
  const summaries: string[] = []
  const levelRank = { healthy: 0, watch: 1, warning: 2, critical: 3 } as const
  let level: 'healthy' | 'watch' | 'warning' | 'critical' = 'healthy'

  const raise = (nextLevel: 'watch' | 'warning' | 'critical', metricId: string, summary: string) => {
    if (!triggeredMetricIds.includes(metricId)) triggeredMetricIds.push(metricId)
    summaries.push(summary)
    if (levelRank[nextLevel] > levelRank[level]) level = nextLevel
  }

  const inflationMetric = ECONOMY_AUDIT_CONFIG.coreMetrics.find(metric => metric.id === 'inflation_pressure_index')
  if (inflationMetric?.thresholds.critical != null && overview.inflationPressure >= inflationMetric.thresholds.critical) {
    raise('critical', inflationMetric.id, `通胀压力指数达到 ${overview.inflationPressure.toFixed(2)}`)
  } else if (inflationMetric?.thresholds.warning != null && overview.inflationPressure >= inflationMetric.thresholds.warning) {
    raise('warning', inflationMetric.id, `通胀压力指数达到 ${overview.inflationPressure.toFixed(2)}`)
  } else if (inflationMetric?.thresholds.watch != null && overview.inflationPressure >= inflationMetric.thresholds.watch) {
    raise('watch', inflationMetric.id, `通胀压力指数进入观察区间（${overview.inflationPressure.toFixed(2)}）`)
  }

  const sinkMetric = ECONOMY_AUDIT_CONFIG.coreMetrics.find(metric => metric.id === 'sink_satisfaction_ratio')
  if (sinkMetric?.thresholds.critical != null && overview.sinkSatisfaction <= sinkMetric.thresholds.critical) {
    raise('critical', sinkMetric.id, `消耗满足度偏低（${overview.sinkSatisfaction.toFixed(2)}）`)
  } else if (sinkMetric?.thresholds.warning != null && overview.sinkSatisfaction <= sinkMetric.thresholds.warning) {
    raise('warning', sinkMetric.id, `消耗满足度下降到 ${overview.sinkSatisfaction.toFixed(2)}`)
  } else if (sinkMetric?.thresholds.watch != null && overview.sinkSatisfaction <= sinkMetric.thresholds.watch) {
    raise('watch', sinkMetric.id, `消耗满足度进入观察区间（${overview.sinkSatisfaction.toFixed(2)}）`)
  }

  const diversityMetric = ECONOMY_AUDIT_CONFIG.coreMetrics.find(metric => metric.id === 'loop_diversity_score')
  if (diversityMetric?.thresholds.critical != null && overview.loopDiversity <= diversityMetric.thresholds.critical) {
    raise('critical', diversityMetric.id, `循环多样度过低（${overview.loopDiversity}）`)
  } else if (diversityMetric?.thresholds.warning != null && overview.loopDiversity <= diversityMetric.thresholds.warning) {
    raise('warning', diversityMetric.id, `循环多样度下降到 ${overview.loopDiversity}`)
  } else if (diversityMetric?.thresholds.watch != null && overview.loopDiversity <= diversityMetric.thresholds.watch) {
    raise('watch', diversityMetric.id, `循环多样度进入观察区间（${overview.loopDiversity}）`)
  }

  const dominanceMetric = ECONOMY_AUDIT_CONFIG.coreMetrics.find(metric => metric.id === 'dominant_income_share')
  if (dominanceMetric?.thresholds.critical != null && overview.dominantIncomeShare >= dominanceMetric.thresholds.critical) {
    raise('critical', dominanceMetric.id, `单系统收入占比过高（${(overview.dominantIncomeShare * 100).toFixed(0)}%）`)
  } else if (dominanceMetric?.thresholds.warning != null && overview.dominantIncomeShare >= dominanceMetric.thresholds.warning) {
    raise('warning', dominanceMetric.id, `单系统收入占比达到 ${(overview.dominantIncomeShare * 100).toFixed(0)}%`)
  } else if (dominanceMetric?.thresholds.watch != null && overview.dominantIncomeShare >= dominanceMetric.thresholds.watch) {
    raise('watch', dominanceMetric.id, `单系统收入占比进入观察区间（${(overview.dominantIncomeShare * 100).toFixed(0)}%）`)
  }

  const netInflowMetric = ECONOMY_AUDIT_CONFIG.guardrailMetrics.find(metric => metric.id === 'late_game_net_inflow_ratio')
  if (netInflowMetric?.thresholds.targetMin != null && overview.guardrailNetInflowRatio < netInflowMetric.thresholds.targetMin) {
    raise('warning', netInflowMetric.id, `后期 7 日净流入 / 总资产比偏低（${overview.guardrailNetInflowRatio.toFixed(2)}）`)
  } else if (netInflowMetric?.thresholds.targetMax != null && overview.guardrailNetInflowRatio > netInflowMetric.thresholds.targetMax) {
    raise('warning', netInflowMetric.id, `后期 7 日净流入 / 总资产比偏高（${overview.guardrailNetInflowRatio.toFixed(2)}）`)
  }

  return {
    level,
    triggeredMetricIds,
    summary: summaries.length > 0 ? summaries.join('；') : '后期经济运行平稳。'
  }
}
...
export const handleEndDay = () => {
  ...
  const economyIncomeBefore = {
    total: playerStore.economyTelemetry.lifetimeIncome.total,
    bySystem: { ...playerStore.economyTelemetry.lifetimeIncome.bySystem }
  }
  const economyExpenseBefore = {
    total: playerStore.economyTelemetry.lifetimeExpense.total,
    bySystem: { ...playerStore.economyTelemetry.lifetimeExpense.bySystem }
  }
  const economySinkBefore = {
  ...
  if (zhiji) {
    ...
    const zhijiQuotes: Record<string, string[]> = {
      a_shi: ['「……顺手带的，你拿去用吧。」', '「矿里挖的，放着也是放着。」', '「……嗯，给你。」'],
      dan_qing: ['「闲来画了几幅，顺道替你说了几句好话。」', '「邻里和气才是福，帮你美言几句。」'],
      a_tie: ['「打铁剩的边料，别浪费了。」', '「炉子烧着呢，顺手多打了点。」'],
      yun_fei: ['「山里采的，新鲜着呢，快用。」', '「今儿运气好，给你带了些山货。」'],
      da_niu: ['「牲口我帮你看着了，放心吧。」', '「我顺手喂了，你忙你的。」'],
      mo_bai: ['「弹了首曲子，人总得松快松快。」', '「心烦时听听琴，比什么都管用。」'],
      liu_niang: ['「在村里替你说了几句好话，大家都惦记着你呢。」', '「邻居间哪能生分，我帮你打招呼了。」'],
      qiu_yue: ['「钓了几条，你拿去吃。」', '「清溪边碰巧钓到的，送你尝尝。」'],
      chun_lan: ['「新沏的茶，趁热喝。」', '「茶叶是自家炒的，别嫌弃。」'],
      xue_qin: ['「竹子用得上就拿去。」', '「林子里砍的，搁着也是搁着。」'],
      su_su: ['「这匹布料颜色好，拿去做件衣裳。」', '「织剩的料，放你这比放仓库强。」'],
      hong_dou: ['「自家酿的，今年的新酒。」', '「酒香不怕巷子深，先让你尝个鲜。」'],
      dan_qing2: ['「替你说了几句，大伙儿都夸你勤快。」']
    }
    const getZhijiQuote = (id: string) => {
      const quotes = zhijiQuotes[id] ?? [`「这是给你的。」`]
    ...
    switch (zhiji.npcId) {
      case 'a_shi':
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('a_shi')} 送来了${qty}个${getItemById(ore)?.name ?? '矿石'}。`)
        }
        break
      case 'dan_qing':
        if (Math.random() < 0.2 + bonusChance2) {
          for (const s of npcStore.npcStates) {
            if (s.npcId !== zhiji.npcId) {
              const cap = s.married ? 4000 : 2500
              s.friendship = Math.min(s.friendship + 5, cap)
            }
          }
          addLog(`${zhijiName}${getZhijiQuote('dan_qing')} (全村+5好感)`)
        }
        break
      case 'a_tie':
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('a_tie')} 送来了一些打铁材料。`)
        }
        break
      case 'yun_fei':
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('yun_fei')} 带回了${getItemById(item2)?.name ?? '山货'}。`)
        }
        break
      case 'da_niu':
        if (Math.random() < 0.3 + bonusChance2) {
      ...
      case 'liu_niang':
        if (Math.random() < 0.2 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('liu_niang')} (全村+5好感)`)
        }
        break
      case 'qiu_yue':
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('qiu_yue')} 送来了一条${getItemById(f)?.name ?? '鱼'}。`)
        }
        break
      case 'chun_lan':
        if (Math.random() < 0.25 + bonusChance2) {
          inventoryStore.addItem('green_tea_drink')
          addLog(`${zhijiName}${getZhijiQuote('chun_lan')} 送来了一壶好茶。`)
        }
        break
      case 'xue_qin':
        if (Math.random() < 0.15 + bonusChance2) {
          inventoryStore.addItem('bamboo')
          addLog(`${zhijiName}${getZhijiQuote('xue_qin')} 送来了一捆竹子。`)
        }
        break
      case 'su_su':
        if (Math.random() < 0.25 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('su_su')} 送来了一匹${getItemById(c)?.name ?? '布料'}。`)
        }
        break
      case 'hong_dou':
        if (Math.random() < 0.3 + bonusChance2) {
          ...
          addLog(`${zhijiName}${getZhijiQuote('hong_dou')} 送来了一壶${getItemById(w)?.name ?? '酒'}。`)
        }
        break
    }
  }
  ...
  for (const eq of expiredQuests) {
    if (eq.type === 'special_order' || eq.themeTag) highValueOrderTypes.add(eq.themeTag ?? eq.type)
  ...
  if (spouse) {
    ...
    const highBond = spouse.friendship >= 2500 ? 0.15 : 0
  ...
  if (fishPondStore.pond.built) {
    ...
    if (pondResult.products.length > 0) {
      addLog(`鱼塘产出了${pondResult.products.length}件水产品，可前往鱼塘收取。`)
    }
  ...
  if (gameStore.isRainy || landGodActive) {
    for (const plot of farmStore.plots) {
  ...
  const economyDayTag = getEconomyDayTag(gameStore.year, gameStore.season, gameStore.day)

  villageProjectStore.processOperationalTick(economyDayTag, {
    startedNewWeek: weekBoundaryEvent.startedNewWeek
  })
  museumStore.processOperationalTick(economyDayTag, {
    startedNewWeek: weekBoundaryEvent.startedNewWeek,
    seasonChanged
  })
  guildStore.processSeasonTick({
    currentSeasonId: `${gameStore.year}-${gameStore.season}`,
    previousSeasonId: `${previousWeekInfo.year}-${previousWeekInfo.season}`,
  ...
  const expenseBySystem = diffNumericRecord(economyExpenseBefore.bySystem, playerStore.economyTelemetry.lifetimeExpense.bySystem)
  const sinkDiff = diffNumericRecord(economySinkBefore.byCategory, playerStore.economyTelemetry.lifetimeSinkSpend.byCategory)
  const participatingSystems = new Set<string>([
    ...Object.keys(incomeBySystem),
    ...Object.keys(expenseBySystem),
    shippingIncome > 0 ? 'market' : ''
  ].filter(Boolean))
  const activeSinkCategories = Object.keys(sinkDiff)

  playerStore.appendEconomySnapshot({
    dayTag: economyDayTag,
    disposableMoney: playerStore.money,
    totalIncome: Math.max(0, playerStore.economyTelemetry.lifetimeIncome.total - economyIncomeBefore.total),
    totalExpense: Math.max(0, playerStore.economyTelemetry.lifetimeExpense.total - economyExpenseBefore.total),
    sinkSpend: Math.max(0, playerStore.economyTelemetry.lifetimeSinkSpend.total - economySinkBefore.total),
    dominantIncomeSystem: Object.entries(incomeBySystem).sort((a, b) => Number(b[1]) - Number(a[1]))[0]?.[0] as any,
    participatingSystems: [...participatingSystems] as any,
    highValueOrderTypes: highValueOrderTypes.size,
    incomeBySystem: incomeBySystem as any,
    expenseBySystem: expenseBySystem as any,
    activeSinkCategories: activeSinkCategories as any
  })
  ...
  if (weekBoundaryEvent.startedNewWeek) {
    ...
    if (weeklyRiskReport.level !== 'healthy') {
      addLog(`【经济观测】${weeklyRiskReport.summary}`, {
        category: 'economy',
  ...
  if (goalStore.currentThemeWeek?.preferredQuestThemeTag) {
    highValueOrderTypes.add(goalStore.currentThemeWeek.preferredQuestThemeTag)
  }
  if (questStore.specialOrder?.themeTag) {
    highValueOrderTypes.add(questStore.specialOrder.themeTag)
  }
  for (const quest of questStore.activeQuests) {
    if (quest.type === 'special_order' || quest.themeTag) {
      highValueOrderTypes.add(quest.themeTag ?? quest.type)
    }
  }

  // 今日行情
  const marketInfo = getDailyMarketInfo(gameStore.year, gameStore.seasonIndex, gameStore.day, recentShipping)
  ...
  if (tutorialStore.enabled && gameStore.year === 1) {
    const conditions: Record<string, () => boolean> = {
      ...
      breedingJustUnlocked: () => breedingStore.unlocked && !tutorialStore.isTipShown('tip_breeding_unlock'),
      hasSeedsNeverBred: () => breedingStore.boxCount >= 2 && breedingStore.stationCount > 0 && breedingStore.compendium.length === 0 && !tutorialStore.isTipShown('tip_breeding_has_seeds'),
      canTryHybrid: () => {
        const box = breedingStore.breedingBox
        if (box.length < 2) return false
        const hasHighStat = box.some(s => s.genetics.sweetness >= 35 || s.genetics.yield >= 35)
        return hasHighStat && breedingStore.stationCount > 0 && !tutorialStore.isTipShown('tip_breeding_try_hybrid')
      },
      hasSeedsNoStation: () => breedingStore.boxCount >= 1 && breedingStore.stationCount === 0 && !tutorialStore.isTipShown('tip_breeding_station')
    }
