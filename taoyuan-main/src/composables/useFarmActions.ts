...
export const handleBuySeed = (seedId: string) => {
  ...
  if (!seed) return
  const actualPrice = shopStore.applyDiscount(seed.price)
...
export const handleBatchHarvest = () => {
  ...
  for (const groupId of giantGroups) {
    ...
    const cropId = groupPlot.cropId
    if (!cropId) continue
    const giantQuantity = farmStore.plots.filter(p => p.giantCropGroup === groupId).length * 2
    if (!inventoryStore.canAddItem(cropId, giantQuantity)) {
    ...
    if (result) {
      ...
      inventoryStore.addItemExact(result.cropId, result.quantity)
