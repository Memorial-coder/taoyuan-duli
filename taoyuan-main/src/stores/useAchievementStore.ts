...
export const useAchievementStore = defineStore('achievement', () => {
  ...
  const isConditionMet = (c: AchievementCondition): boolean => {
    switch (c.type) {
      ...
      case 'hybridsShipped': {
        ...
        const shippedHybridCount = shopStore.shippedItems.filter((id: string) => hybridItemIds.has(id)).length
      ...
      case 'fullFishCollection':
        return FISH.length > 0 && FISH.every(f => discoveredItems.value.includes(f.id))
    }
  }
      ...
      if (isConditionMet(achievement.condition)) {
        ...
        if (achievement.reward.items) {
          ...
          if (inventoryStore.canAddItems(rewardItems)) {
            ...
            const fallbackMoney = achievement.reward.items.reduce((sum, item) => sum + (ITEMS.find(entry => entry.id === item.itemId)?.sellPrice ?? 0) * item.quantity, 0)
            if (fallbackMoney > 0) {
    ...
    if (allMet) {
      ...
      if (bundle.reward.items) {
        ...
        if (inventoryStore.canAddItems(rewardItems)) {
          ...
          const fallbackMoney = bundle.reward.items.reduce((sum, item) => sum + (ITEMS.find(entry => entry.id === item.itemId)?.sellPrice ?? 0) * item.quantity, 0)
          if (fallbackMoney > 0) {
            playerStore.earnMoney(fallbackMoney, { countAsEarned: false })
          }
        }
  ...
  const unlockedTitles = computed(() =>
    ACHIEVEMENTS
      .filter(a => a.title && completedAchievements.value.includes(a.id))
