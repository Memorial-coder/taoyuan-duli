...
export const useAnimalStore = defineStore('animal', () => {
  ...
  const dailyIncubatorUpdate = (): { hatched?: { type: AnimalType; name: string } } => {
    ...
    if (incubating.value.daysLeft <= 0) {
      ...
      if (currentCount >= maxCapacity) {
        // 容量满，退回蛋
        const inventoryStore = useInventoryStore()
        incubating.value.daysLeft = 0
        if (inventoryStore.canAddItem(itemId, 1) && inventoryStore.addItemExact(itemId, 1)) {
  ...
  const dailyBarnIncubatorUpdate = (): { hatched?: { type: AnimalType; name: string } } => {
    ...
    if (barnIncubating.value.daysLeft <= 0) {
      ...
      if (currentCount >= maxCapacity) {
        const inventoryStore = useInventoryStore()
        barnIncubating.value.daysLeft = 0
        if (inventoryStore.canAddItem(itemId, 1) && inventoryStore.addItemExact(itemId, 1)) {
  ...
  const dailyUpdate = (): { products: { itemId: string; quality: Quality; quantity?: number }[]; died: string[]; gotSick: string[]; healed: string[] } => {
    ...
    const hasAnimalWhisperer = farmingSkill.perk15 === 'animal_whisperer'
    const hasNatureBond = farmingSkill.perk20 === 'nature_bond'
    const hasBeastSovereign = farmingSkill.perk20 === 'beast_sovereign'
    ...
    for (const animal of animals.value) {
      ...
      // 动物语者：每天自动+1心（100友好度）；自然契约：每天自动+2心
      if (hasNatureBond) {
        animal.friendship = Math.min(1000, animal.friendship + 200)
      } else if (hasAnimalWhisperer) {
        animal.friendship = Math.min(1000, animal.friendship + 100)
      }
