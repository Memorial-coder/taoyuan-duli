...
import {
  exchangeWeeklyOffer,
  fetchWeeklyExchangeStation,
  type WeeklyExchangeActionResponse,
  type WeeklyExchangeOffer,
  type WeeklyExchangeRecord,
  type WeeklyExchangeStationSnapshot
} from '@/utils/weeklyExchangeApi'

export interface WeeklyExchangeSaveSyncState {
  attempted: boolean
  ...
  exchanged_save_slot: number | null
  reason:
    | 'synced'
    | 'no_save_slot'
...
const buildSaveSyncState = (state: WeeklyExchangeSaveSyncState): WeeklyExchangeSaveSyncState => state

export const useWeeklyExchangeStore = defineStore('weeklyExchangeStation', () => {
  const station = ref<WeeklyExchangeStationSnapshot | null>(null)
  const loading = ref(false)
  const actionRunning = ref(false)
  const errorMessage = ref('')
  const lastLoadedAt = ref(0)

  const offers = computed<WeeklyExchangeOffer[]>(() => station.value?.offers ?? [])
  const records = computed<WeeklyExchangeRecord[]>(() => station.value?.my_records ?? [])

  const syncAfterExchange = async (saveSlot: number | null | undefined): Promise<WeeklyExchangeSaveSyncState> => {
    ...
    if (normalizedSaveSlot === null) {
      return buildSaveSyncState({
        ...
        exchanged_save_slot: null,
        reason: 'no_save_slot',
        reason_detail: 'no_save_slot',
        message: '换物已完成，但本次操作没有写入有效的服务端存档槽位。'
      })
    }
    ...
    if (currentStorageMode !== 'server') {
      return buildSaveSyncState({
        ...
        current_session_mode: currentSessionMode,
        current_session_slot: currentSessionSlot,
        exchanged_save_slot: normalizedSaveSlot,
        reason: 'current_session_not_server',
        reason_detail: 'current_storage_mode_not_server',
        message: '换物结果已写入服务端存档，但当前面板不在服务端模式，未自动回读运行态。'
      })
    }
    ...
    if (currentSessionMode !== 'server') {
      return buildSaveSyncState({
        ...
        message: '换物结果已写入服务端存档，但当前运行中的会话并非服务端载入会话，未自动回读。'
      })
    }
    ...
    if (currentSessionSlot === null) {
      return buildSaveSyncState({
        ...
        message: '换物结果已写入服务端存档，但当前没有可安全回读的服务端运行槽位。'
      })
    }
    ...
    if (currentSessionSlot !== normalizedSaveSlot) {
      return buildSaveSyncState({
        ...
        message: '换物结果已写入其他服务端槽位，当前运行态仍停留在不同槽位，未自动切换回读。'
      })
    }
    ...
    if (saveStore.hasPendingServerSave(currentSessionSlot)) {
      return buildSaveSyncState({
        ...
        message: '换物结果已写入当前服务端槽位，但本地仍有待同步副本，已跳过自动回读以避免旧副本覆盖运行态。'
      })
    }
    ...
    return buildSaveSyncState({
      ...
      message: synced
        ? '换物结果已同步到当前服务端运行会话。'
        : '换物结果已写入当前服务端槽位，但自动回读失败，请手动重新载入查看。'
    })
  }
  ...
  const performExchange = async (offerId: string) => {
    actionRunning.value = true
    errorMessage.value = ''
    try {
      const result = await exchangeWeeklyOffer(offerId)
      const saveSyncState = await syncAfterExchange(result.save_slot)
      await refreshStation().catch(() => {})
      return {
        ...result,
        save_sync_state: saveSyncState
      } as WeeklyExchangeActionResponse & {
        save_sync_state: WeeklyExchangeSaveSyncState
      }
    } catch (error) {
      errorMessage.value = error instanceof Error ? error.message : '执行每周换物失败'
      throw error
    } finally {
