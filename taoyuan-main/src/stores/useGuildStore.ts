...
export const useGuildStore = defineStore('guild', () => {
  ...
  const resolveSeasonPhase = (weekOfSeason: number): GuildSeasonPhase => {
    if (weekOfSeason >= 4) return 'p2_world_milestone'
    if (weekOfSeason >= 2) return 'p1_ranked_hunt'
  ...
  const buildAsyncRankScore = () => {
    ...
    return contributionPoints.value + guildExp.value + claimedGoals.value.length * 25 + bossClears * 40 + guildLevel.value * 50
  }
