...
export const useFishingStore = defineStore('fishing', () => {
  ...
  type TreasureResult = { items: { itemId: string; name: string; quantity: number }[]; money: number; blocked: boolean }

  /** 上次钓鱼的宝箱结果 */
  const lastTreasure = ref<TreasureResult | null>(null)
  ...
  const startFishing = (): { success: boolean; message: string; junk?: boolean } => {
    ...
    const seaCaptainReduction = (_fishingSkill.perk15 === 'sea_captain' || _fishingSkill.perk15 === 'bait_master') ? 0.5 : 0
    const staminaCost = zeroStamina ? 0 : Math.max(
      1,
      Math.floor(
        STAMINA_COST *
          rodMultiplier *
    ...
    if (Math.random() < junkChance) {
      ...
      return { success: true, junk: true, message: `钓上了${junkName}，但背包空间不足，没能带走。(-${staminaCost}体力)` }
    }
  ...
  const completeFishing = (rating: MiniGameRating): { message: string; fishName?: string; fishId?: string; difficulty?: string; sellPrice?: number; description?: string; quality?: Quality; quantity?: number; success: boolean } | null => {
    ...
    let quality: Quality = skillStore.rollFishQuality()
    // 水手专精：鱼品质至少为优良；海洋船长perk15：至少为优质；海洋霸主perk20：至少为极品
    const _fishSkill = skillStore.getSkill('fishing')
    if (_fishSkill.perk20 === 'sea_sovereign' || _fishSkill.perk20 === 'lure_deity') {
      if (quality === 'normal' || quality === 'fine') quality = 'excellent'
    } else if (_fishSkill.perk15 === 'sea_captain' || _fishSkill.perk15 === 'bait_master') {
      if (quality === 'normal') quality = 'fine'
      if (quality === 'fine') quality = 'excellent'
    } else if (_fishSkill.perk10 === 'mariner' && quality === 'normal') {
      quality = 'fine'
    }
    // 戒指效果：鱼品质提升
    ...
    const added = inventoryStore.addItemExact(currentFish.value.id, catchQty, quality)
  ...
  const rollTreasureChest = (): TreasureResult | null => {
    ...
    const stagedEntries: { itemId: string; quantity: number; quality?: Quality }[] = []
    const stagedItems: { itemId: string; name: string; quantity: number }[] = []
    let money = 0
      ...
      for (const prize of TREASURE_POOL) {
        ...
        if (roll <= 0) {
          ...
          if (prize.itemId) {
            ...
            stagedEntries.push({ itemId: prize.itemId, quantity: qty, quality: 'normal' })
            stagedItems.push({ itemId: prize.itemId, name: itemDef?.name ?? prize.itemId, quantity: qty })
          } else {
            money += qty
    ...
    if (stagedEntries.length > 0) {
      if (inventoryStore.canAddItems(stagedEntries) && inventoryStore.addItemsExact(stagedEntries)) {
  ...
  const removeCrabPot = (location: FishingLocation): { success: boolean; message: string } => {
    ...
    if (!inventoryStore.canAddItem('crab_pot', 1)) {
      return { success: false, message: '背包空间不足，无法回收蟹笼。' }
    }
    crabPots.value.splice(idx, 1)
    inventoryStore.addItemExact('crab_pot', 1)
