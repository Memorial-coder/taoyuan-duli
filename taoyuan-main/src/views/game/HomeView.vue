...
<script setup lang="ts">
  ...
  const confirmDismantle = () => {
    ...
    if (!info) return
    const refundEntries = [
      ...chest.items.map(item => ({ itemId: item.itemId, quantity: item.quantity, quality: item.quality })),
      ...info.refund.map(mat => ({ itemId: mat.itemId, quantity: mat.quantity, quality: 'normal' as const }))
    ]
    if (refundEntries.length > 0 && !inventoryStore.canAddItems(refundEntries)) {
      addLog('背包空间不足，无法拆卸该箱子。请先腾出足够空间。')
      return
    }

    // 箱内物品返还背包
    if (refundEntries.length > 0) {
      inventoryStore.addItemsExact(refundEntries)
    }
