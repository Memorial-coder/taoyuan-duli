<template>
  <div class="space-y-3" data-testid="online-cohabitation-page">
    ...
    >
              ...
              <div v-if="cohabitationStore.canOpenSelectedContract || latestSeparationPreview" class="border border-accent/10 bg-black/10 p-3">
                ...
                <div v-if="latestSeparationPreview" class="mt-3 space-y-2">
                  ...
                  >
                    ...
                    <p class="leading-4">
                      分居拆分会引用家族建筑真实拆除 / 精确主状态链路的既有回执；没有回执的建筑仍保留校验码和记录明细，等待回滚、拆除或人工收口。
                    </p>
                  </div>
                  <div
                ...
                <div class="mt-3 grid gap-3 text-[0.625rem] text-muted">
                  <div class="grid gap-2 md:grid-cols-2" data-testid="online-cohabitation-safe-version-list">
                    ...
                    >
                      ...
                      <p class="mt-1 break-all">记录校验码 {{ version.snapshot_hash || '待生成' }}</p>
          ...
          <div class="game-panel-muted p-3">
            ...
            <div v-else class="mt-3 space-y-3">
              <div class="border border-accent/10 bg-black/10 p-2 text-[0.625rem] leading-4 text-muted">
                ...
                <p v-if="selectedSharedFarmPlot.plot_state.fertilizer" class="mt-1">替换肥料会消耗新肥料，旧肥料不返还。</p>
              ...
              <div class="grid gap-2">
                ...
                >
                  <Sprout :size="12" />
                  {{ selectedSharedFarmPlot?.plot_state.fertilizer ? `替换为${selectedSharedFarmFertilizer?.label || '肥料'}` : (selectedSharedFarmFertilizer?.label || '施肥') }}
                </button>
                <button
                  class="online-action-btn online-action-btn--compact justify-center"
                  type="button"
      ...
      <CohabitationFamilyFestivalPanel
        v-else-if="activeTab === 'festivalSeats'"
        :panel="familyFestivalSeatsPanel"
        :summary-cards="familyFestivalSeatSummaryCards"
        :scene-objects="familyFestivalSeatSceneObjects"
        :templates="familyFestivalSeatTemplates"
        :members="familyFestivalSeatMembers"
        :guard-cards="familyFestivalSeatGuardCards"
        :deferred-operations="familyFestivalSeatDeferredOperations"
        :action-loading="cohabitationStore.actionLoading"
        :action-message="familyFestivalSeatActionMessage"
        :action-ok="familyFestivalSeatActionOk"
        :visual-type-label="familyFestivalVisualTypeLabel"
        :family-role-label="familyRoleLabel"
        :object-kind-label="familyFestivalObjectKindLabel"
        :seat-state-label="familyFestivalSeatStateLabel"
        :deferred-operation-label="deferredOperationLabel"
        :scene-object-class="familyFestivalSceneObjectClass"
        @reserve="openCohabitationRiskConfirm({ kind: 'family-festival-reserve' })"
        @create-room="openCohabitationRiskConfirm({ kind: 'family-festival-room' })"
        @consume-supplies="openCohabitationRiskConfirm({ kind: 'family-festival-supplies' })"
        @settle-rewards="openCohabitationRiskConfirm({ kind: 'family-festival-settle' })"
      />
            ...
            <div class="mt-3 space-y-3">
              ...
              <div class="border border-accent/10 bg-black/10 p-2 text-[0.625rem] leading-4 text-muted">
                ...
                <p class="mt-1" data-testid="online-cohabitation-offline-queue-selected-state">
                  {{ selectedOfflineQueueActionOption?.enabled ? '系统队列可提交' : selectedOfflineQueueActionOption?.disabledReason || '当前不可提交' }}
                </p>
              </div>
              <label
...
<script setup lang="ts">
  ...
  const warehouseCompensationAuditTimelineRows = computed(() => {
    ...
    return timeline.map(entry => ({
      ...
      value: `${entry.actor_display_name || entry.actor_username || '未知'} / ${formatTime(entry.at)} / ${entry.idempotency_key || '无重复提交记录'}`,
    })).slice(0, 8)
  })
  const warehouseCompensationRollbackAuditRows = computed(() => {
    ...
    const rows = [
      ...
      { label: '回滚重复提交记录', value: auditValueLabel(draft['rollback_idempotency_key'] || audit?.idempotency_key, '无重复提交记录') },
  ...
  const warehouseOperatorReceiptAuditEvidenceRows = computed(() => {
    ...
    const rows = [
      ...
      { label: '执行记录', value: auditValueLabel(draft['compensation_operator_receipt_audit_execution_audit_id'] || detail['execution_audit_id'], '未引用') },
      { label: '执行确认键', value: auditValueLabel(draft['compensation_operator_receipt_audit_execution_idempotency_key'] || detail['execution_idempotency_key'], '未引用') },
      { label: '申诉记录', value: auditValueLabel(draft['compensation_operator_receipt_audit_appeal_resolution_audit_id'] || detail['appeal_resolution_audit_id'], '未绑定') },
      { label: '申诉确认键', value: auditValueLabel(draft['compensation_operator_receipt_audit_appeal_resolution_idempotency_key'] || detail['appeal_resolution_idempotency_key'], '未绑定') },
      ...
      { label: '资产边界', value: '只记录复核结果 / 不改个人背包 / 不还共同仓库' },
    ]
    return rows.filter(row => row.value !== '')
  })
  ...
  const familyOrderSettlementCards = computed(() => {
    ...
    return [
      ...
      { label: '重复提交保护', value: settlement.idempotency_required === true ? '已启用' : '未声明' },
  ...
  const familyReputationGovernanceCards = computed(() => {
    ...
    return [
      { label: '系统确认', value: governance.server_authoritative === true ? '已启用' : '未声明' },
      { label: '重复提交保护', value: governance.idempotency_required_for_future_writes === true ? '已启用' : '未声明' },
  ...
  const familyFestivalSeatGuardCards = computed(() => {
    ...
    return [
      { label: '系统确认', value: governance.server_authoritative === true ? '已启用' : '未声明' },
      { label: '锁席保护', value: governance.seat_reservation_requires_idempotency === true ? '已启用' : '未声明' },
      { label: '断线恢复', value: governance.disconnect_recovery_required === true ? '必须' : '未声明' },
      { label: '节会记录', value: settlement.festival_receipt_required === true ? '已启用' : '未声明' },
  ...
  const offlineSummaryCards = computed(() => {
    ...
    return [
      ...
      { label: '冲突预检', value: summary?.offline_conflict_preflight_enabled ? '系统预检' : '暂未开放' },
      { label: '冲突解决', value: summary?.offline_conflict_resolution_enabled ? '系统记录包' : '暂未开放' },
  ...
  const warehouseItemLabels: Record<string, string> = {
    ...
    food_honey_tea: '蜂蜜茶',
    food_ginger_soup: '姜汤',
    food_osmanthus_cake: '桂花糕',
    food_jujube_cake: '红枣糕',
    food_peach_blossom_cake: '桃花饼',
    food_dried_persimmon: '柿饼',
    food_fish_noodle: '鱼汤面',
    food_miner_iron_pot: '矿工铁锅饭',
    food_stir_fried_potato: '炒土豆丝',
    food_sesame_paste: '芝麻糊',
    ginseng: '人参',
    snow_lotus: '雪莲',
    hanhai_cactus: '瀚海仙人掌',
    hanhai_date: '瀚海椰枣',
    ...
    shadow_ore: '暗影矿',
    void_ore: '虚空矿',
    food_ginseng_soup: '人参汤',
    food_herbal_pill: '百草丹',
    food_longevity_soup: '长生汤',
    food_scholars_porridge: '文曲星粥',
    food_antler_soup: '鹿茸汤',
    food_chive_egg_stir_fry: '韭菜炒蛋',
    food_peanut_candy: '花生糖',
    food_sweet_osmanthus_tea: '桂花甜茶',
    food_aged_radish_stew: '老萝卜炖肉',
    food_embroidered_cake: '绣囊糕',
    food_deep_mine_stew: '深矿炖菜',
    food_wild_berry_jam: '野果酱',
    food_hunters_roast: '猎人烤',
    food_farmers_feast: '农家盛宴',
    food_autumn_moon_feast: '秋月宴',
    food_lovers_pastry: '鸳鸯酥',
    food_forgemasters_meal: '锻造师套餐',
    food_spirit_fruit_wine: '灵果酒',
    food_phoenix_cake: '凤凰糕',
    food_molten_hotpot: '熔岩铁锅',
    food_tea_banquet: '茶宴八珍',
    food_ironforge_stew: '铁匠炖',
    food_ranch_milk_soup: '牧场鲜奶汤',
    food_moonlit_tea_rice: '月下茶泡饭',
    food_spicy_hotpot: '麻辣火锅',
    food_bamboo_shoot_stir_fry: '冬笋炒肉',
    food_lotus_seed_soup: '莲子羹',
    food_corn_pancake: '玉米烙',
    food_osmanthus_lotus_root: '桂花藕粉',
    food_nian_gao: '年糕',
    food_hua_gao: '花糕',
    food_qing_tuan: '青团',
    food_yue_bing: '月饼',
    ...
    food_yam_family_porridge: '山药团圆粥',
    food_garlic_radish_side_dish: '蒜香萝卜',
    food_bitter_gourd_cooling_soup: '苦瓜清暑汤',
    food_rice_flour_roll: '米粉卷',
    food_steamed_bun: '馒头',
    food_dried_vegetable_soup: '干菜米粉汤',
    ...
    food_pigeon_egg_herb_soup: '鸽蛋药草汤',
    food_yak_milk_barley_porridge: '牦牛奶麦粥',
    food_donkey_milk_ginger_tea: '驴奶姜茶',
    food_ostrich_egg_feast: '鸵鸟蛋宴',
    food_antler_velvet_herb_soup: '鹿茸药草汤',
    food_silk_dumpling: '锦囊玉饺',
    food_peacock_feast: '孔雀宴',
    food_spiced_lamb: '香料烤羊',
    food_drunken_chicken: '醉仙鸡',
    food_silk_dumpling_deluxe: '丝路饺子',
    ...
    food_moonlight_sashimi: '月下刺身',
    food_legendary_feast: '传说盛宴',
    food_sturgeon_stew: '鲟鱼羹',
    food_dragon_sashimi: '龙鱼刺身',
    food_collectors_banquet: '收藏家宴',
    ...
    food_jiaozi: '冬至饺',
    food_tangyuan: '汤圆',
    food_bountiful_porridge: '百收粥',
    food_chef_special: '大厨特供',
    food_social_tea: '交际花茶',
    food_pumpkin_pie: '南瓜饼',
    food_golden_fried_rice: '黄金炒饭',
    food_supreme_farm_feast: '田园盛筵',
    food_stone_soup: '矿石汤',
    food_iron_tonic: '铁骨汤',
    food_crystal_jelly: '水晶冻',
    food_gold_dumpling: '金矿饺',
    food_void_essence_soup: '虚空精华汤',
    food_shadow_brew: '暗影酿',
    food_void_elixir: '虚空药剂',
    food_miners_glory: '矿工荣光',
    food_abyss_stew: '深渊炖菜',
    food_snow_plum_soup: '雪梅羹',
    food_desert_cactus_soup: '仙人掌汤',
    food_date_cake: '枣糕',
    food_wild_salad: '野菜沙拉',
    food_mushroom_stew: '蘑菇炖',
    food_forest_tonic: '林间补药',
    food_spirit_herb_elixir: '灵草秘药',
    food_warrior_ration: '战士口粮',
    food_battle_stew: '战斗炖菜',
    food_iron_fist_soup: '铁拳汤',
    ...
    snow_lotus_pearl: '雪莲子',
    dew_bloom: '露花',
    star_lotus: '星莲',
    snow_lotus_calm_elixir: '雪莲清心丹',
  ...
  const sharedWorkshopAlchemyWeightProfiles: Record<string, { profile: string; label: string; weights: SharedAlchemyWeights }> = {
    ...
    shared_snow_lotus_calm_elixir: { profile: 'snow_lotus_pearl_rare_crop', label: '雪莲稀材', weights: { success: 59, partial: 19, failed: 8, rare: 14 } },
    shared_dew_bloom_focus_elixir: { profile: 'dew_bloom_rare_crop', label: '露花稀材', weights: { success: 58, partial: 19, failed: 8, rare: 15 } },
    shared_star_lotus_calm_elixir: { profile: 'star_lotus_rare_crop', label: '星莲稀材', weights: { success: 57, partial: 20, failed: 8, rare: 15 } },
  }
  ...
  const sharedWorkshopRecipeOptions: SharedWorkshopRecipeOption[] = [
    ...
    { id: 'shared_dried_vegetable', label: '共同晒架干菜', station: 'drying_rack', process_kind: 'cooking_material', input_items: [{ item_id: 'cabbage', quantity: 2, quality: 'normal' }], output_item_id: 'dried_vegetable', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_wheat_flour', label: '共同石磨面粉', station: 'stone_mill', process_kind: 'cooking_material', input_items: [{ item_id: 'winter_wheat', quantity: 2, quality: 'normal' }], output_item_id: 'wheat_flour', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_hidden_tofu_potato', label: '共同豆腐坊锦豆腐', station: 'tofu_press', process_kind: 'processing', local_processing_recipe_id: 'hidden_tofu_potato', input_items: [{ item_id: 'potato', quantity: 3, quality: 'normal' }], output_item_id: 'firm_mixed_tofu', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_osmanthus_cake', label: '共同灶台桂花糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'osmanthus', quantity: 3, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }], output_item_id: 'food_osmanthus_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_jujube_cake', label: '共同灶台红枣糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'jujube', quantity: 3, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }], output_item_id: 'food_jujube_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_peach_blossom_cake', label: '共同灶台桃花饼', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'peach', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }], output_item_id: 'food_peach_blossom_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_dried_persimmon', label: '共同晒架柿饼', station: 'drying_rack', process_kind: 'cooking_dish', input_items: [{ item_id: 'persimmon', quantity: 3, quality: 'normal' }], output_item_id: 'food_dried_persimmon', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_fish_noodle', label: '共同灶台鱼汤面', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'crucian', quantity: 1, quality: 'normal' }, { item_id: 'winter_wheat', quantity: 2, quality: 'normal' }], output_item_id: 'food_fish_noodle', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_miner_iron_pot', label: '共同灶台矿工铁锅饭', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'copper_ore', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_miner_iron_pot', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_stir_fried_potato', label: '共同灶台炒土豆丝', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'potato', quantity: 2, quality: 'normal' }], output_item_id: 'food_stir_fried_potato', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_sesame_paste_dessert', label: '共同灶台芝麻糊', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'sesame', quantity: 3, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }], output_item_id: 'food_sesame_paste', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_ginseng_soup', label: '共同灶台人参汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ginseng', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_ginseng_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_chive_egg_stir_fry', label: '共同灶台韭菜炒蛋', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'chives', quantity: 2, quality: 'normal' }, { item_id: 'egg', quantity: 1, quality: 'normal' }], output_item_id: 'food_chive_egg_stir_fry', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_peanut_candy', label: '共同灶台花生糖', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'peanut', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_peanut_candy', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_sweet_osmanthus_tea', label: '共同茶炉桂花甜茶', station: 'tea_maker', process_kind: 'cooking_dish', input_items: [{ item_id: 'osmanthus', quantity: 1, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_sweet_osmanthus_tea', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_aged_radish_stew', label: '共同灶台老萝卜炖肉', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'radish', quantity: 3, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 2, quality: 'normal' }], output_item_id: 'food_aged_radish_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_embroidered_cake', label: '共同灶台绣囊糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'osmanthus', quantity: 2, quality: 'normal' }], output_item_id: 'food_embroidered_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_deep_mine_stew', label: '共同灶台深矿炖菜', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'potato', quantity: 2, quality: 'normal' }, { item_id: 'copper_ore', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_deep_mine_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_wild_berry_jam', label: '共同灶台野果酱', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'wild_berry', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_wild_berry_jam', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_farmers_feast', label: '共同灶台农家盛宴', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'cabbage', quantity: 2, quality: 'normal' }, { item_id: 'egg', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_farmers_feast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_autumn_moon_feast', label: '共同灶台秋月宴', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'mandarin_fish', quantity: 1, quality: 'normal' }, { item_id: 'river_crab', quantity: 1, quality: 'normal' }, { item_id: 'osmanthus', quantity: 2, quality: 'normal' }], output_item_id: 'food_autumn_moon_feast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_lovers_pastry', label: '共同灶台鸳鸯酥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'peach', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_lovers_pastry', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_forgemasters_meal', label: '共同灶台锻造师套餐', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'iron_ore', quantity: 2, quality: 'normal' }, { item_id: 'potato', quantity: 3, quality: 'normal' }, { item_id: 'firewood', quantity: 2, quality: 'normal' }], output_item_id: 'food_forgemasters_meal', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_spirit_fruit_wine', label: '共同酒坊灵果酒', station: 'wine_workshop', process_kind: 'cooking_dish', input_items: [{ item_id: 'wild_berry', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 2, quality: 'normal' }], output_item_id: 'food_spirit_fruit_wine', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_phoenix_cake', label: '共同灶台凤凰糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 3, quality: 'normal' }, { item_id: 'osmanthus', quantity: 2, quality: 'normal' }, { item_id: 'jujube', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_phoenix_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_molten_hotpot', label: '共同灶台熔岩铁锅', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'iron_ore', quantity: 3, quality: 'normal' }, { item_id: 'chili', quantity: 2, quality: 'normal' }, { item_id: 'potato', quantity: 2, quality: 'normal' }], output_item_id: 'food_molten_hotpot', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_tea_banquet', label: '共同茶炉茶宴八珍', station: 'tea_maker', process_kind: 'cooking_dish', input_items: [{ item_id: 'tea', quantity: 3, quality: 'normal' }, { item_id: 'lotus_seed', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_tea_banquet', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_ironforge_stew', label: '共同灶台铁匠炖', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'potato', quantity: 3, quality: 'normal' }, { item_id: 'corn', quantity: 2, quality: 'normal' }, { item_id: 'iron_ore', quantity: 1, quality: 'normal' }], output_item_id: 'food_ironforge_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_hunters_roast', label: '共同灶台猎人烤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'wild_mushroom', quantity: 3, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'pine_cone', quantity: 1, quality: 'normal' }], output_item_id: 'food_hunters_roast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_ranch_milk_soup', label: '共同灶台牧场鲜奶汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'milk', quantity: 2, quality: 'normal' }, { item_id: 'corn', quantity: 2, quality: 'normal' }, { item_id: 'sweet_potato', quantity: 1, quality: 'normal' }], output_item_id: 'food_ranch_milk_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_moonlit_tea_rice', label: '共同茶炉月下茶泡饭', station: 'tea_maker', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'tea', quantity: 2, quality: 'normal' }, { item_id: 'bamboo_shoot', quantity: 1, quality: 'normal' }], output_item_id: 'food_moonlit_tea_rice', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_spicy_hotpot', label: '共同灶台麻辣火锅', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'chili', quantity: 2, quality: 'normal' }, { item_id: 'cabbage', quantity: 1, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_spicy_hotpot', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_bamboo_shoot_stir_fry', label: '共同灶台冬笋炒肉', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'winter_bamboo_shoot', quantity: 2, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_bamboo_shoot_stir_fry', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_lotus_seed_soup', label: '共同灶台莲子羹', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'lotus_seed', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_lotus_seed_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_corn_pancake', label: '共同灶台玉米烙', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'corn', quantity: 2, quality: 'normal' }, { item_id: 'sesame_oil', quantity: 1, quality: 'fine' }], output_item_id: 'food_corn_pancake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_osmanthus_lotus_root', label: '共同灶台桂花藕粉', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'osmanthus', quantity: 1, quality: 'normal' }, { item_id: 'lotus_root', quantity: 1, quality: 'normal' }], output_item_id: 'food_osmanthus_lotus_root', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_nian_gao', label: '共同灶台年糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_nian_gao', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_hua_gao', label: '共同灶台花糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'peach', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_hua_gao', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_qing_tuan', label: '共同灶台青团', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }], output_item_id: 'food_qing_tuan', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_yue_bing', label: '共同灶台月饼', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'lotus_seed', quantity: 2, quality: 'normal' }, { item_id: 'sesame_oil', quantity: 1, quality: 'fine' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_yue_bing', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_pigeon_egg_herb_soup', label: '共同灶台鸽蛋药草汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'pigeon_egg', quantity: 2, quality: 'normal' }, { item_id: 'herb', quantity: 1, quality: 'normal' }], output_item_id: 'food_pigeon_egg_herb_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_yak_milk_barley_porridge', label: '共同灶台牦牛奶麦粥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'yak_milk', quantity: 1, quality: 'normal' }, { item_id: 'wheat', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_yak_milk_barley_porridge', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_donkey_milk_ginger_tea', label: '共同茶炉驴奶姜茶', station: 'tea_maker', process_kind: 'cooking_dish', input_items: [{ item_id: 'donkey_milk', quantity: 1, quality: 'normal' }, { item_id: 'ginger', quantity: 1, quality: 'normal' }, { item_id: 'tea', quantity: 1, quality: 'normal' }], output_item_id: 'food_donkey_milk_ginger_tea', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_ostrich_egg_feast', label: '共同灶台鸵鸟蛋宴', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ostrich_egg', quantity: 1, quality: 'normal' }, { item_id: 'potato', quantity: 1, quality: 'normal' }, { item_id: 'sesame_oil', quantity: 1, quality: 'fine' }], output_item_id: 'food_ostrich_egg_feast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_antler_velvet_herb_soup', label: '共同灶台鹿茸药草汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'antler_velvet', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'goat_milk', quantity: 1, quality: 'normal' }], output_item_id: 'food_antler_velvet_herb_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_antler_soup', label: '共同灶台鹿茸汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'antler_velvet', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'ginseng', quantity: 1, quality: 'normal' }], output_item_id: 'food_antler_soup', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_spiced_lamb', label: '共同灶台香料烤羊', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'hanhai_spice', quantity: 1, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'goat_milk', quantity: 1, quality: 'normal' }], output_item_id: 'food_spiced_lamb', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_silk_dumpling_deluxe', label: '共同灶台丝路饺子', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'hanhai_silk', quantity: 1, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'hanhai_spice', quantity: 1, quality: 'normal' }], output_item_id: 'food_silk_dumpling_deluxe', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_moonlight_sashimi', label: '共同灶台月下刺身', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'sturgeon', quantity: 1, quality: 'normal' }, { item_id: 'ginger', quantity: 1, quality: 'normal' }], output_item_id: 'food_moonlight_sashimi', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_legendary_feast', label: '共同灶台传说盛宴', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'jade_dragon', quantity: 1, quality: 'normal' }, { item_id: 'ginger', quantity: 2, quality: 'normal' }], output_item_id: 'food_legendary_feast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_sturgeon_stew', label: '共同灶台鲟鱼羹', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'sturgeon', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_sturgeon_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_dragon_sashimi', label: '共同灶台龙鱼刺身', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'dragonfish', quantity: 1, quality: 'normal' }, { item_id: 'ginger', quantity: 2, quality: 'normal' }], output_item_id: 'food_dragon_sashimi', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_herbal_pill', label: '共同灶台百草丹', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'herb', quantity: 3, quality: 'normal' }, { item_id: 'ginseng', quantity: 1, quality: 'normal' }], output_item_id: 'food_herbal_pill', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_longevity_soup', label: '共同灶台长生汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ginseng', quantity: 2, quality: 'normal' }, { item_id: 'herb', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_longevity_soup', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_scholars_porridge', label: '共同灶台文曲星粥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 3, quality: 'normal' }, { item_id: 'tea', quantity: 1, quality: 'normal' }, { item_id: 'ginseng', quantity: 1, quality: 'normal' }], output_item_id: 'food_scholars_porridge', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_collectors_banquet', label: '共同灶台收藏家宴', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ginseng', quantity: 1, quality: 'normal' }, { item_id: 'sturgeon', quantity: 1, quality: 'normal' }, { item_id: 'pumpkin', quantity: 1, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }], output_item_id: 'food_collectors_banquet', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_pumpkin_pie', label: '共同灶台南瓜饼', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'pumpkin', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }], output_item_id: 'food_pumpkin_pie', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_golden_fried_rice', label: '共同灶台黄金炒饭', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'egg', quantity: 2, quality: 'normal' }, { item_id: 'corn', quantity: 1, quality: 'normal' }], output_item_id: 'food_golden_fried_rice', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_supreme_farm_feast', label: '共同灶台田园盛筵', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'pumpkin', quantity: 1, quality: 'normal' }, { item_id: 'watermelon', quantity: 1, quality: 'normal' }, { item_id: 'corn', quantity: 1, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }], output_item_id: 'food_supreme_farm_feast', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_stone_soup', label: '共同灶台矿石汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'copper_ore', quantity: 2, quality: 'normal' }, { item_id: 'radish', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_stone_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_iron_tonic', label: '共同灶台铁骨汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'iron_ore', quantity: 2, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_iron_tonic', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_crystal_jelly', label: '共同灶台水晶冻', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'crystal_ore', quantity: 1, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_crystal_jelly', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_gold_dumpling', label: '共同灶台金矿饺', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'gold_ore', quantity: 1, quality: 'normal' }, { item_id: 'winter_wheat', quantity: 2, quality: 'normal' }], output_item_id: 'food_gold_dumpling', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_void_essence_soup', label: '共同灶台虚空精华汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'void_ore', quantity: 1, quality: 'normal' }, { item_id: 'ginseng', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }], output_item_id: 'food_void_essence_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_shadow_brew', label: '共同酒坊暗影酿', station: 'wine_workshop', process_kind: 'cooking_dish', input_items: [{ item_id: 'shadow_ore', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_shadow_brew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_void_elixir', label: '共同灶台虚空药剂', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'void_ore', quantity: 1, quality: 'normal' }, { item_id: 'ginseng', quantity: 1, quality: 'normal' }, { item_id: 'shadow_ore', quantity: 1, quality: 'normal' }], output_item_id: 'food_void_elixir', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_miners_glory', label: '共同灶台矿工荣光', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'gold_ore', quantity: 1, quality: 'normal' }, { item_id: 'egg', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }], output_item_id: 'food_miners_glory', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_abyss_stew', label: '共同灶台深渊炖菜', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'shadow_ore', quantity: 1, quality: 'normal' }, { item_id: 'crystal_shrimp', quantity: 1, quality: 'normal' }, { item_id: 'herb', quantity: 1, quality: 'normal' }], output_item_id: 'food_abyss_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_snow_plum_soup', label: '共同灶台雪梅羹', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'snow_lotus', quantity: 1, quality: 'normal' }, { item_id: 'honey', quantity: 2, quality: 'normal' }], output_item_id: 'food_snow_plum_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_desert_cactus_soup', label: '共同灶台仙人掌汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'hanhai_cactus', quantity: 2, quality: 'normal' }, { item_id: 'hanhai_spice', quantity: 1, quality: 'normal' }], output_item_id: 'food_desert_cactus_soup', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_date_cake', label: '共同灶台枣糕', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'hanhai_date', quantity: 3, quality: 'normal' }, { item_id: 'rice', quantity: 2, quality: 'normal' }], output_item_id: 'food_date_cake', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_wild_salad', label: '共同灶台野菜沙拉', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'herb', quantity: 2, quality: 'normal' }, { item_id: 'wild_berry', quantity: 1, quality: 'normal' }], output_item_id: 'food_wild_salad', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_mushroom_stew', label: '共同灶台蘑菇炖', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'wild_mushroom', quantity: 3, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_mushroom_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_forest_tonic', label: '共同灶台林间补药', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ginseng', quantity: 1, quality: 'normal' }, { item_id: 'wild_mushroom', quantity: 2, quality: 'normal' }, { item_id: 'herb', quantity: 2, quality: 'normal' }], output_item_id: 'food_forest_tonic', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_spirit_herb_elixir', label: '共同灶台灵草秘药', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'ginseng', quantity: 2, quality: 'normal' }, { item_id: 'herb', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_spirit_herb_elixir', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_warrior_ration', label: '共同灶台战士口粮', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'potato', quantity: 2, quality: 'normal' }, { item_id: 'egg', quantity: 1, quality: 'normal' }], output_item_id: 'food_warrior_ration', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_battle_stew', label: '共同灶台战斗炖菜', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'chili', quantity: 1, quality: 'normal' }, { item_id: 'potato', quantity: 1, quality: 'normal' }, { item_id: 'wild_meat', quantity: 1, quality: 'normal' }, { item_id: 'ginger', quantity: 1, quality: 'normal' }], output_item_id: 'food_battle_stew', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_iron_fist_soup', label: '共同灶台铁拳汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'iron_ore', quantity: 1, quality: 'normal' }, { item_id: 'chili', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_iron_fist_soup', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_la_ba_zhou', label: '共同灶台腊八粥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 2, quality: 'normal' }, { item_id: 'peanut', quantity: 1, quality: 'normal' }, { item_id: 'wild_berry', quantity: 1, quality: 'normal' }], output_item_id: 'food_la_ba_zhou', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_jiaozi', label: '共同灶台冬至饺', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'winter_wheat', quantity: 2, quality: 'normal' }, { item_id: 'napa_cabbage', quantity: 2, quality: 'normal' }], output_item_id: 'food_jiaozi', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_tangyuan', label: '共同灶台花生汤圆', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 3, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }, { item_id: 'peanut', quantity: 1, quality: 'normal' }], output_item_id: 'food_tangyuan', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_bountiful_porridge', label: '共同灶台百收粥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'rice', quantity: 3, quality: 'normal' }, { item_id: 'jujube', quantity: 2, quality: 'normal' }], output_item_id: 'food_bountiful_porridge', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_chef_special', label: '共同灶台大厨特供', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'egg', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }, { item_id: 'sesame', quantity: 2, quality: 'normal' }], output_item_id: 'food_chef_special', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_social_tea', label: '共同茶炉交际花茶', station: 'tea_maker', process_kind: 'cooking_dish', input_items: [{ item_id: 'osmanthus', quantity: 2, quality: 'normal' }, { item_id: 'honey', quantity: 1, quality: 'normal' }], output_item_id: 'food_social_tea', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_yam_family_porridge', label: '共同灶台山药团圆粥', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'yam', quantity: 2, quality: 'normal' }, { item_id: 'rice', quantity: 1, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_yam_family_porridge', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_garlic_radish_side_dish', label: '共同灶台蒜香萝卜', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'garlic', quantity: 1, quality: 'normal' }, { item_id: 'radish', quantity: 2, quality: 'normal' }], output_item_id: 'food_garlic_radish_side_dish', output_quantity: 1, output_quality: 'normal' },
    { id: 'shared_bitter_gourd_cooling_soup', label: '共同灶台苦瓜清暑汤', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'bitter_gourd', quantity: 2, quality: 'normal' }, { item_id: 'firewood', quantity: 1, quality: 'normal' }], output_item_id: 'food_bitter_gourd_cooling_soup', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_steamed_bun', label: '共同灶台馒头', station: 'stove', process_kind: 'cooking_dish', input_items: [{ item_id: 'wheat_flour', quantity: 1, quality: 'fine' }], output_item_id: 'food_steamed_bun', output_quantity: 1, output_quality: 'normal' },
    ...
    { id: 'shared_snow_lotus_calm_elixir', label: '共同丹炉雪莲清心丹', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'snow_lotus_pearl', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'lotus_heart_powder', quantity: 1, quality: 'fine' }], output_item_id: 'snow_lotus_calm_elixir', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'success' },
    { id: 'shared_snow_lotus_calm_partial', label: '共同丹炉雪莲偏丹膏', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'snow_lotus_pearl', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'lotus_heart_powder', quantity: 1, quality: 'fine' }], output_item_id: 'partial_elixir_slurry', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'partial' },
    { id: 'shared_snow_lotus_calm_failed', label: '共同丹炉雪莲废丹灰', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'snow_lotus_pearl', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'lotus_heart_powder', quantity: 1, quality: 'fine' }], output_item_id: 'failed_elixir_ash', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'failed' },
    { id: 'shared_snow_lotus_calm_rare', label: '共同丹炉雪莲奇丹晶', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'snow_lotus_pearl', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'lotus_heart_powder', quantity: 1, quality: 'fine' }], output_item_id: 'rare_elixir_crystal', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'rare' },
    { id: 'shared_dew_bloom_focus_elixir', label: '共同丹炉露花凝神丹', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'dew_bloom', quantity: 1, quality: 'normal' }, { item_id: 'osmanthus_honey', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'dew_bloom_focus_elixir', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'success' },
    { id: 'shared_dew_bloom_focus_partial', label: '共同丹炉露花偏丹膏', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'dew_bloom', quantity: 1, quality: 'normal' }, { item_id: 'osmanthus_honey', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'partial_elixir_slurry', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'partial' },
    { id: 'shared_dew_bloom_focus_failed', label: '共同丹炉露花废丹灰', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'dew_bloom', quantity: 1, quality: 'normal' }, { item_id: 'osmanthus_honey', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'failed_elixir_ash', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'failed' },
    { id: 'shared_dew_bloom_focus_rare', label: '共同丹炉露花奇丹晶', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'dew_bloom', quantity: 1, quality: 'normal' }, { item_id: 'osmanthus_honey', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'rare_elixir_crystal', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'rare' },
    { id: 'shared_star_lotus_calm_elixir', label: '共同丹炉星莲安神丹', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'star_lotus', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'star_lotus_calm_elixir', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'success' },
    { id: 'shared_star_lotus_calm_partial', label: '共同丹炉星莲偏丹膏', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'star_lotus', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'partial_elixir_slurry', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'partial' },
    { id: 'shared_star_lotus_calm_failed', label: '共同丹炉星莲废丹灰', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'star_lotus', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'failed_elixir_ash', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'failed' },
    { id: 'shared_star_lotus_calm_rare', label: '共同丹炉星莲奇丹晶', station: 'alchemy_furnace', process_kind: 'alchemy_elixir', input_items: [{ item_id: 'star_lotus', quantity: 1, quality: 'normal' }, { item_id: 'herbal_paste', quantity: 1, quality: 'fine' }, { item_id: 'green_tea_drink', quantity: 1, quality: 'fine' }], output_item_id: 'rare_elixir_crystal', output_quantity: 1, output_quality: 'normal', alchemy_result_kind: 'rare' },
  ]
  ...
  const canFertilizeSelectedSharedFarmPlot = computed(() => {
    ...
    if (plot.plot_state.state === 'wasteland') return false
    return !plot.plot_state.fertilizer || plot.plot_state.fertilizer !== fertilizer.itemId
  })
  ...
  const offlineConflictResolutionLabel = computed(() => {
    ...
    const stale = resolution.client_queue_stale === true ? '本地基线过期，按最新状态处理' : '本地基线一致'
    return `冲突解决记录：提交 ${committed} / 重复 ${idempotent} / 拒绝 ${rejected} · 流水 ${ledgerCount} 笔 · 版本 ${beforeRevision}->${afterRevision} · ${stale}`
  })
  ...
  const offlineConflictPreflightLabel = computed(() => {
    ...
    const stale = preflight.client_queue_stale === true ? '本地基线已过期，请刷新后合并' : '本地基线与云端一致'
    return `冲突预检：本地版本 ${clientRevision} / 云端版本 ${serverRevision} · ${stale} · 不支持动作 ${unsupportedCount} 项`
  })
  const offlineQueueRevisionStateLabel = computed(() => {
    ...
    const stale = merge.client_queue_stale === true ? '本地基线已过期，系统按最新共同资产合并' : '本地基线未过期'
    return `队列版本：本地 ${clientRevision} / 云端 ${beforeRevision} -> ${afterRevision} · ${stale}`
  })
  ...
  const offlineQueueResultStatusLabel = (status: string) => {
    ...
    if (status === 'idempotent') return '重复提交已读回'
    if (status === 'rejected') return '已拒绝'
    return status || '未记录'
  }
  const offlineQueueResultDetail = (entry: CohabitationOfflineQueueMergeEntry) => {
    if (entry.status === 'rejected') return entry.reason || '系统拒绝，未改个人存档或共同资产'
    ...
    const revisionText = typeof entry.server_base_revision === 'number' || typeof entry.server_committed_revision === 'number'
      ? `版本 ${Number(entry.client_base_revision) || 0}/${Number(entry.server_base_revision) || 0}->${Number(entry.server_committed_revision) || 0}`
      : ''
  ...
  const offlineQueueDraftSummaryLabel = computed(() =>
    offlineQueueDraftOperations.value.length > 0
      ? `本地缓存 ${offlineQueueDraftOperations.value.length} 项 · 版本 ${offlineQueueDraftClientRevision.value}`
      : '本地缓存 0 项'
  )
  ...
  const buildSelectedOfflineQueueOperation = (): CohabitationOfflineQueueOperation | null => {
    ...
    if (option.id.includes('shared_farm')) {
      ...
      if (option.id === 'fertilize_shared_farm_basic' || option.id === 'fertilize_shared_farm_premium') {
        ...
        basePayload.replace_existing_fertilizer = Boolean(plot.plot_state.fertilizer)
      }
  ...
  const fertilizeSelectedSharedFarmPlot = async () => {
    ...
    const replacing = Boolean(plot.plot_state.fertilizer)
    sharedFarmActionMessage.value = ''
    sharedFarmActionOk.value = false
    try {
      const result = await cohabitationStore.fertilizeSharedFarmPlot({
        ...
        replace_existing_fertilizer: replacing,
        memo: `前端共同农田${replacing ? '替换肥料' : (fertilizer.premium ? '高级' : '基础') + '施肥'}：${plot.id}`,
        idempotency_key: `ui-shared-farm-${replacing ? 'replace-fertilizer' : 'fertilize'}-${plot.id}-${fertilizer.itemId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      })
      selectedSharedFarmPlotId.value = result?.plot?.id || plot.id
      sharedFarmActionOk.value = true
      sharedFarmActionMessage.value = result?.idempotent || result?.already_fertilized || result?.already_replaced_fertilizer
        ? '已读回共同农田施肥记录'
        : replacing
          ? `共同农田已替换为${fertilizer.label}，旧肥料未返还`
  ...
  const sharedLogDetail = (entry: CohabitationAuditEntry) => {
    ...
    if (entry.action === 'offline_queue_merged') {
      ...
      return `离线队列合并：提交 ${committed}、重复 ${idempotent}、拒绝 ${rejected}，记录 ${ledgerCount} 笔，${stale}`
    }
