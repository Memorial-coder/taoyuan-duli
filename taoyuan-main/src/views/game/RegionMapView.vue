<template>
  <div>
    ...
    <template v-else>
      ...
      <div class="border border-accent/20 rounded-xs p-3 mb-3 bg-bg/70" data-testid="region-social-friend-panel">
...
<script setup lang="ts">
  ...
  const searchMapPlayerBySaveId = async () => {
  ...
  const sendMapFriendRequest = async (saveId: number) => {
  ...
  const acceptMapRequest = async (requestId: string) => {
  ...
  const rejectMapRequest = async (requestId: string) => {
  ...
  const removeMapFriend = async (entry: OnlineRelationCard) => {
  ...
  const blockMapSaveId = async (saveId: number) => {
  ...
  const blockMapRelation = async (entry: OnlineRelationCard) => {
    const saveId = Number(entry.friend_save_id || 0)
    if (!saveId) return
    await blockMapSaveId(saveId)
  }

  const unblockMapRelation = async (entry: OnlineRelationCard) => {
