  ...
  <div class="space-y-3">
    <div class="flex items-center justify-between gap-2">
      <div>
        ...
        <p class="text-[0.625rem] text-muted mt-1">把当前账号的公开名片整理出来，方便后续好友、邻里和来访系统直接复用。</p>
      </div>
      <Button class="text-[0.625rem]" :disabled="socialStore.loading || socialStore.saving" @click="refreshProfile">
    ...
    <template v-else>
      <div class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
        <div class="flex items-start justify-between gap-2">
          <div>
            ...
            <p class="text-[0.625rem] text-muted mt-1">{{ socialStore.profile.display_name }} · {{ socialStore.profile.honorific }}</p>
          </div>
          <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-accent/30 text-accent">
            {{ visibilityLabel }}
          </span>
        </div>

        <div v-if="socialStore.profile.avatar_image_url" class="border border-accent/10 rounded-xs p-2 bg-bg/10">
          <p class="text-[0.625rem] text-muted mb-2">公开头像</p>
          ...
          <p v-if="socialStore.profile.avatar_image_alt" class="text-[0.625rem] text-muted mt-2 text-center">{{ socialStore.profile.avatar_image_alt }}</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <p class="text-[0.625rem] text-muted mt-1">{{ socialStore.profile.public_title }}</p>
          </div>
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted">季节进度</p>
            <p class="text-accent mt-1">{{ socialStore.profile.season_progress }}</p>
            <p class="text-[0.625rem] text-muted mt-1">{{ socialStore.profile.showcase_theme }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted">本周展示主题</p>
        ...
        <div class="border border-accent/10 rounded-xs p-2 text-xs">
          ...
          <div class="flex flex-wrap gap-1 mt-1">
            <span
              ...
              :key="tag.id"
              class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border"
              :class="tag.source === 'selected' ? 'border-accent/40 text-accent bg-accent/5' : 'border-accent/15 text-muted'"
            >
              {{ tag.label }}
            </span>
            <span v-if="socialStore.profile.public_tags.length === 0" class="text-[0.625rem] text-muted">当前还没有公开标签。</span>
          </div>
        </div>
        ...
        <div class="border border-accent/10 rounded-xs p-2 text-xs">
          ...
          <div v-if="!socialStore.profile.player_chronicle || socialStore.profile.player_chronicle.milestones.length === 0" class="text-[0.625rem] text-muted mt-2">
            当前还没有可回看的联机史册记录。
          </div>
        ...
        <div class="border border-accent/10 rounded-xs p-2 text-xs">
          ...
          <div class="space-y-2 mt-2">
            ...
            >
              ...
              <p class="text-[0.625rem] text-muted mt-1 leading-4">{{ entry.unlocked ? entry.detail || entry.summary : entry.summary }}</p>
            </div>
          </div>
        </div>
            ...
            >
              <div class="flex items-center justify-between gap-2">
                <p class="text-xs" :class="entry.unlocked ? 'text-success' : 'text-muted'">
                  {{ entry.label }}
                  <span v-if="entry.active" class="text-[0.625rem] text-accent ml-1">当前展示</span>
                </p>
                <span class="text-[0.625rem]" :class="entry.unlocked ? 'text-success' : 'text-muted'">
                  {{ entry.unlocked ? formatChronicleDate(entry.recorded_at) : '未收录' }}
                </span>
        ...
        <div class="border border-accent/10 rounded-xs p-2 text-xs">
          ...
          <div class="space-y-2 mt-2">
            ...
            >
              ...
              <p class="text-[0.625rem] text-muted mt-1 leading-4">{{ entry.detail || entry.summary }}</p>
            </div>
            <div v-if="unlockedAchievementCardCount === 0" class="text-[0.625rem] text-muted">
              当前还没有可展示的联机成就卡。
            </div>
          </div>
        </div>
      ...
      <div class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
        ...
        <div class="border border-accent/10 rounded-xs p-2 bg-bg/10 space-y-2">
          <div class="flex items-center justify-between gap-2">
            ...
            <Button class="text-[0.625rem]" :disabled="uploadingAvatar" @click="triggerAvatarUpload">
        ...
        <div class="flex items-center justify-between gap-2">
          <p class="text-[0.625rem] text-muted">保存后会同步成公开名片预览。</p>
          <Button class="text-[0.625rem]" :disabled="!socialStore.hasDirtyDraft || socialStore.saving" @click="saveProfile">
      ...
      <div class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
        <div class="flex items-center justify-between gap-2">
          ...
          <Button class="text-[0.625rem]" @click="goFriendStation">
            前往好友驿站
          </Button>
        </div>
        <p class="text-[0.625rem] text-muted leading-5">
      ...
      <div class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
        <div class="flex items-center justify-between gap-2">
          ...
          <Button class="text-[0.625rem]" :disabled="socialStore.neighborLoading || socialStore.neighborActionRunning" @click="refreshNeighbors">
        ...
        <div class="grid gap-2 md:grid-cols-3">
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <Button class="text-[0.625rem]" @click="goQuestBoard">去看委托</Button>
            </div>
            <p class="text-xs text-accent mt-2">{{ neighborTaskCard.title }}</p>
            <p class="text-[0.625rem] text-muted mt-1 leading-4">{{ neighborTaskCard.summary }}</p>
          </div>

          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted">邻里进度</p>
            <p class="text-xs text-accent mt-2">{{ neighborProgressCard.title }}</p>
            <p class="text-[0.625rem] text-muted mt-1 leading-4">{{ neighborProgressCard.summary }}</p>
          </div>

          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">邻里排行</p>
            <div v-if="neighborLeaderboard.length === 0" class="text-[0.625rem] text-muted leading-4">当前还没有可比较的公开邻里。</div>
            <div v-else class="space-y-1.5">
              <div v-for="(group, index) in neighborLeaderboard" :key="group.id" class="border border-accent/10 rounded-xs px-2 py-1.5">
                <div class="flex items-center justify-between gap-2">
                  <p class="text-[0.625rem] text-accent">{{ index + 1 }}. {{ group.name }}</p>
                  <span class="text-[0.625rem] text-muted">Lv.{{ group.level }}</span>
                </div>
                <p class="text-[0.625rem] text-muted mt-1">{{ group.member_count }}/{{ group.capacity }} 人</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <template v-if="socialStore.neighborGroup">
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between gap-2">
              <div>
                ...
                <p class="text-[0.625rem] text-muted mt-1">{{ socialStore.neighborGroup.summary || '这个邻里还没写简介。' }}</p>
              </div>
              <span class="text-[0.625rem] text-muted">Lv.{{ socialStore.neighborGroup.level }} · {{ socialStore.neighborGroup.member_count }}/{{ socialStore.neighborGroup.capacity }}</span>
            </div>
            <p class="text-[0.625rem] text-muted mt-2">我的身份：{{ neighborRoleLabel(socialStore.neighborGroup.role) }}</p>
          </div>
          ...
          <div class="flex justify-end">
            <Button class="text-[0.625rem]" :disabled="socialStore.neighborActionRunning" @click="saveNeighborNotice">
              保存公告
            </Button>
          </div>

          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">成员</p>
              ...
              <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <template v-if="socialStore.neighborGroup.role === 'leader' && member.role !== 'leader'">
                    <Button class="text-[0.625rem]" :disabled="socialStore.neighborActionRunning" @click="setNeighborRole(member.username, member.role === 'manager' ? 'member' : 'manager')">
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">邻里动态</p>
            <div v-if="(socialStore.neighborGroup.activity_log || []).length === 0" class="text-[0.625rem] text-muted">当前还没有新的邻里动态。</div>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">待处理申请 / 邀请</p>
            <div v-if="socialStore.neighborManagedRequests.length === 0 && socialStore.neighborIncomingInvites.length === 0" class="text-[0.625rem] text-muted">当前没有新的邻里申请或邀请。</div>
            <div v-for="entry in socialStore.neighborManagedRequests" :key="entry.id" class="border border-accent/10 rounded-xs p-2 mb-1.5">
              ...
              <div class="flex gap-2 mt-2">
                <Button class="text-[0.625rem]" :disabled="socialStore.neighborActionRunning" @click="acceptNeighbor(entry.id)">接受</Button>
                <Button class="text-[0.625rem]" :disabled="socialStore.neighborActionRunning" @click="rejectNeighbor(entry.id)">拒绝</Button>
              </div>
            </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">邀请成员</p>
        ...
        <template v-else>
          <div class="border border-accent/10 rounded-xs p-2 space-y-2">
            <p class="text-[0.625rem] text-muted">创建邻里</p>
            <input
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">公开邻里</p>
            <div v-if="socialStore.neighborPublicGroups.length === 0" class="text-[0.625rem] text-muted">当前还没有公开邻里。</div>
            <div v-for="group in socialStore.neighborPublicGroups" :key="group.id" class="border border-accent/10 rounded-xs p-2 mb-1.5">
              <div class="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p class="text-[0.625rem] text-muted mt-1">{{ group.summary || '这个邻里还没写简介。' }}</p>
                </div>
                <span class="text-[0.625rem] text-muted">Lv.{{ group.level }} · {{ group.member_count }}/{{ group.capacity }}</span>
              </div>
              <p class="text-[0.625rem] text-muted mt-2">公告：{{ group.notice || '暂无公告' }}</p>
              <div v-if="group.can_apply" class="flex justify-end mt-2">
                <Button class="text-[0.625rem]" :disabled="socialStore.neighborActionRunning" @click="applyNeighbor(group.id)">
                  申请加入
                </Button>
              </div>
            </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">收到的邻里邀请</p>
            <div v-if="socialStore.neighborIncomingInvites.length === 0" class="text-[0.625rem] text-muted">当前没有待处理的邻里邀请。</div>
            <div v-for="entry in socialStore.neighborIncomingInvites" :key="entry.id" class="border border-accent/10 rounded-xs p-2 mb-1.5">
      ...
      <div class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
        <div class="flex items-center justify-between gap-2">
          ...
          <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsLoading || socialStore.subscriptionsActionRunning" @click="refreshSubscriptions">
        ...
        <div class="grid gap-2 md:grid-cols-2">
          <div class="border border-accent/10 rounded-xs p-2 space-y-2">
            <p class="text-[0.625rem] text-muted">快速关注</p>
            <div class="flex flex-wrap gap-2">
              <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsActionRunning" @click="followPreset('style', socialStore.profile.showcase_theme || '本周经营展示', `庄园风格：${socialStore.profile.showcase_theme || '本周经营展示'}`)">
                关注当前庄园风格
              </Button>
              <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsActionRunning" @click="followPreset('expert', socialStore.profile.primary_route_label || '田庄经营', `玩法高手：${socialStore.profile.primary_route_label || '田庄经营'}`)">
                订阅主营方向
              </Button>
              <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsActionRunning || !socialStore.neighborGroup" @click="socialStore.neighborGroup && followPreset('neighbor_group', socialStore.neighborGroup.id, `村社 / 邻里：${socialStore.neighborGroup.name}`)">
                订阅当前邻里
              </Button>
              <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsActionRunning" @click="followPreset('festival', socialStore.profile.showcase_theme || '本周经营展示', `节庆活动：${socialStore.profile.showcase_theme || '本周经营展示'}`)">
                订阅当前节庆主题
              </Button>
            </div>
          </div>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-[0.625rem] text-muted mb-1">当前订阅</p>
            <div v-if="socialStore.subscriptions.length === 0" class="text-[0.625rem] text-muted">当前还没有任何关注或订阅。</div>
            <div v-for="entry in socialStore.subscriptions" :key="entry.id" class="border border-accent/10 rounded-xs p-2 mb-1.5">
              <div class="flex items-center justify-between gap-2">
                <div>
                  <p class="text-xs text-accent">{{ entry.label }}</p>
                  <p class="text-[0.625rem] text-muted mt-1">{{ subscriptionTypeLabel(entry.target_type) }}</p>
                </div>
                <Button class="text-[0.625rem]" :disabled="socialStore.subscriptionsActionRunning" @click="unfollow(entry.id)">
                  取消
                </Button>
              </div>
            </div>
        ...
        <div class="border border-accent/10 rounded-xs p-2">
          <p class="text-[0.625rem] text-muted mb-1">订阅动态</p>
          <div v-if="socialStore.subscriptionNotices.length === 0" class="text-[0.625rem] text-muted">当前还没有订阅提示。</div>
          <div v-for="notice in socialStore.subscriptionNotices" :key="notice.id" class="border border-accent/10 rounded-xs p-2 mb-1.5">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-[0.625rem] text-muted">{{ new Date(notice.createdAt).toLocaleTimeString('zh-CN', { hour12: false }) }}</span>
            </div>
            <p class="text-[0.625rem] text-muted mt-1">{{ notice.message }}</p>
          </div>
        </div>
      </div>
    </template>
