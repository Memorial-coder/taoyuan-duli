  ...
  <div class="border border-warning/20 rounded-xs p-3 mb-4 bg-warning/5">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-warning/20 text-warning">{{ stall.week_label }}</span>
        </div>
        <p class="text-xs text-muted mt-1 leading-5">{{ stall.bulletin }}</p>
        <p class="text-[0.625rem] text-muted mt-1">{{ stall.refresh_hint }}</p>
      </div>
    ...
    <div v-if="stall.festival_theme || categories.length > 0" class="grid gap-2 mt-3 md:grid-cols-2">
      <div v-if="stall.festival_theme" class="border border-warning/15 rounded-xs px-2 py-2 bg-warning/10">
        <p class="text-[0.625rem] text-warning">本周节庆主题</p>
        <p class="text-xs text-text mt-1">{{ stall.festival_theme.label }}</p>
        <p class="text-[0.625rem] text-muted mt-1 leading-4">{{ stall.festival_theme.bulletin }}</p>
      </div>
      <div v-if="categories.length > 0" class="border border-accent/15 rounded-xs px-2 py-2 bg-accent/5">
        <p class="text-[0.625rem] text-accent">摊位分类</p>
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="flex items-center gap-1.5 flex-wrap">
                  ...
                  <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-warning/20 text-warning">{{ offer.badge }}</span>
                  <span
                    v-for="tag in offer.tags.slice(0, 2)"
                    :key="`${offer.id}-${tag}`"
                    class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-warning/10 text-muted"
                  >
                    {{ tag }}
                  </span>
                </div>
