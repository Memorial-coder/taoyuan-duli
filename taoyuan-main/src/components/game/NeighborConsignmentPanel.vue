  ...
  <div class="border border-accent/20 rounded-xs p-3 mb-4 bg-accent/5">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-accent/20 text-accent">{{ overview.neighbor_group ? overview.neighbor_group.name : '未加入邻里' }}</span>
        </div>
    ...
    <div v-if="overview.neighbor_group" class="grid gap-2 mt-3 md:grid-cols-2">
      <div class="border border-accent/15 rounded-xs px-2 py-2 bg-bg/20">
        <p class="text-[0.625rem] text-accent">邻里信息</p>
        <p class="text-xs text-text mt-1">{{ overview.neighbor_group.name }}</p>
        <p class="text-[0.625rem] text-muted mt-1">身份：{{ overview.neighbor_group.role }} · 成员 {{ overview.neighbor_group.member_count }} 人</p>
      </div>
      <div class="border border-warning/15 rounded-xs px-2 py-2 bg-warning/5">
        <p class="text-[0.625rem] text-warning">寄售规则</p>
        <div class="flex flex-wrap gap-1.5 mt-1">
          <span v-for="option in overview.scope_options" :key="option.id" class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-warning/20 text-warning">
            {{ option.label }}
          </span>
        </div>
      </div>
    ...
    <div v-if="!overview.neighbor_group" class="border border-warning/20 rounded-xs px-2 py-2 mt-3 bg-warning/5">
      <p class="text-[0.625rem] text-warning">请先加入邻里</p>
    ...
    <div class="mt-3 border border-accent/15 rounded-xs p-2 bg-bg/20">
      <div class="flex items-center justify-between gap-2 mb-2">
        <p class="text-[0.625rem] text-accent">挂出寄售</p>
        <span class="text-[0.625rem] text-muted">固定价 · 到期回收</span>
      </div>
      <div class="grid gap-2 md:grid-cols-4">
        <label class="block">
          <span class="text-[0.625rem] text-muted">物品 ID</span>
        ...
        <label class="block">
          <span class="text-[0.625rem] text-muted">数量</span>
        ...
        <label class="block">
          <span class="text-[0.625rem] text-muted">固定价</span>
        ...
        <label class="block">
          <span class="text-[0.625rem] text-muted">范围</span>
    ...
    <div class="grid gap-2 mt-3">
      <div class="border border-accent/15 rounded-xs p-2 bg-bg/20">
        <div class="flex items-center justify-between gap-2 mb-2">
          <p class="text-[0.625rem] text-accent">当前可买</p>
          <span class="text-[0.625rem] text-muted">{{ overview.open_listings.length }} 单</span>
        </div>
        <div class="space-y-2">
          <div v-for="listing in overview.open_listings" :key="listing.id" class="border border-accent/10 rounded-xs px-2 py-2 bg-bg/10">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <button class="btn !px-2 !py-1 text-[0.625rem]" :disabled="running || !listing.can_buy" @click="$emit('buy', listing.id)">购买</button>
              <button v-if="listing.can_cancel" class="btn !px-2 !py-1 text-[0.625rem]" :disabled="running" @click="$emit('cancel', listing.id)">取消</button>
              <button v-if="listing.can_reclaim" class="btn !px-2 !py-1 text-[0.625rem]" :disabled="running" @click="$emit('reclaim', listing.id)">回收</button>
            </div>
            <p v-if="!listing.can_buy && listing.visible_to_viewer" class="text-[0.625rem] text-warning mt-1">
      ...
      <div v-if="overview.my_listings.length > 0" class="border border-accent/15 rounded-xs p-2 bg-bg/20">
        <div class="flex items-center justify-between gap-2 mb-2">
          <p class="text-[0.625rem] text-accent">我的寄售</p>
          <span class="text-[0.625rem] text-muted">{{ overview.my_listings.length }} 单</span>
        </div>
        <div class="space-y-2">
