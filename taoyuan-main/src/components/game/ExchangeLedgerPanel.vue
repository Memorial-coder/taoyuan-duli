  ...
  <div class="border border-success/20 rounded-xs p-3 mb-4 bg-success/5">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span v-if="ledger.summary" class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-success/20 text-success">
            {{ ledger.summary.trust_level.label }}
          </span>
        </div>
        <p class="text-xs text-muted mt-1 leading-5">{{ ledger.bulletin }}</p>
      </div>
      <button class="btn !px-2 !py-1 text-[0.625rem] shrink-0" :disabled="loading" @click="$emit('refresh')">
        {{ loading ? '刷新中...' : '刷新' }}
      </button>
    </div>
    ...
    <div v-if="ledger.summary" class="grid gap-2 mt-3 md:grid-cols-3">
      <div class="border border-success/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-success mb-1">常用交换来源</p>
        <div v-if="ledger.summary.source_ranks.length === 0" class="text-[0.625rem] text-muted">当前还没有可统计的交换来源。</div>
        <div v-for="entry in ledger.summary.source_ranks" :key="entry.source" class="text-[0.625rem] text-muted mb-1">
          {{ entry.label }} · {{ entry.count }} 次
        </div>
      </div>
      <div class="border border-success/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-success mb-1">常往对象</p>
        <div v-if="ledger.summary.counterparty_ranks.length === 0" class="text-[0.625rem] text-muted">当前还没有稳定往来对象。</div>
        <div v-for="entry in ledger.summary.counterparty_ranks" :key="entry.key" class="text-[0.625rem] text-muted mb-1">
          {{ entry.label }} · {{ entry.count }} 次
        </div>
      </div>
      <div class="border border-success/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-success mb-1">最常交易品类</p>
        <div v-if="ledger.summary.category_ranks.length === 0" class="text-[0.625rem] text-muted">当前还没有稳定品类分布。</div>
        <div v-for="entry in ledger.summary.category_ranks" :key="entry.label" class="text-[0.625rem] text-muted mb-1">
          {{ entry.label }} · {{ entry.count }} 次
        </div>
      </div>
    </div>
    ...
    <div class="mt-3 border border-success/10 rounded-xs p-2 bg-bg/20">
      <div class="flex items-center justify-between gap-2 mb-2">
        <p class="text-[0.625rem] text-success">最近交换流水</p>
        <span class="text-[0.625rem] text-muted">{{ ledger.entries.length }} 条</span>
      </div>
      <div v-if="ledger.entries.length === 0" class="text-[0.625rem] text-muted">当前还没有可回看的交换记录。</div>
          ...
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-success/20 text-success">{{ entry.source_label }}</span>
                <span class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-accent/10 text-muted">{{ entry.status_label }}</span>
              </div>
              ...
              <p v-if="entry.open_dispute_count > 0" class="text-[0.625rem] text-danger mt-1">
                当前有 {{ entry.open_dispute_count }} 条待处理争议
              </p>
            </div>
            <div class="shrink-0 text-right">
              <p class="text-[0.625rem] text-muted">{{ formatTime(entry.created_at) }}</p>
              <button
                v-if="entry.reportable"
                class="btn !px-2 !py-1 text-[0.625rem] mt-2"
                :disabled="running"
    ...
    <div v-if="ledger.my_disputes.length > 0" class="mt-3 border border-danger/10 rounded-xs p-2 bg-danger/5">
      <div class="flex items-center justify-between gap-2 mb-2">
        <p class="text-[0.625rem] text-danger">我的争议记录</p>
        <span class="text-[0.625rem] text-muted">{{ ledger.my_disputes.length }} 条</span>
      </div>
      <div class="space-y-1.5">
        <div v-for="dispute in ledger.my_disputes.slice(0, 4)" :key="dispute.id" class="border border-danger/10 rounded-xs px-2 py-2 bg-bg/10">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-[0.625rem] text-muted">{{ dispute.status === 'open' ? '待处理' : dispute.status === 'resolved' ? '已解决' : '已驳回' }}</span>
          </div>
          <p class="text-[0.625rem] text-danger mt-1">{{ dispute.reason_label }}</p>
          <p v-if="dispute.note" class="text-[0.625rem] text-muted mt-1">{{ dispute.note }}</p>
        </div>
      </div>
    </div>
  </div>
