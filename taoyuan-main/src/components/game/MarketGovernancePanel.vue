  ...
  <div class="border border-danger/20 rounded-xs p-3 mb-4 bg-danger/5">
    ...
    <div class="grid gap-2 mt-3 md:grid-cols-3">
      <div class="border border-danger/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-danger mb-1">价格区间</p>
        <p class="text-[0.625rem] text-muted">邻里寄售：{{ governance.price_bands.consignment.min_money }}-{{ governance.price_bands.consignment.max_money }}文</p>
        <p class="text-[0.625rem] text-muted mt-1">节庆摊位：{{ governance.price_bands.festival.min_money }}-{{ governance.price_bands.festival.max_money }}文</p>
        <p class="text-[0.625rem] text-muted mt-1">官站现金回款：{{ governance.price_bands.official_money.min_money }}-{{ governance.price_bands.official_money.max_money }}文</p>
      </div>
      <div class="border border-warning/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-warning mb-1">稀有品类限制</p>
        <p class="text-[0.625rem] text-muted">{{ governance.rare_policy.summary }}</p>
    ...
    <div class="grid gap-2 mt-3 md:grid-cols-2">
      <div class="border border-danger/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-danger mb-1">周期开关</p>
        <div class="space-y-1">
          <div v-for="source in governance.sources" :key="source.id" class="flex items-center justify-between gap-2 text-[0.625rem]">
      ...
      <div class="border border-danger/10 rounded-xs px-2 py-2 bg-bg/10">
        <p class="text-[0.625rem] text-danger mb-1">制裁与限制</p>
        <p class="text-[0.625rem] text-muted">挂单上限：{{ governance.anti_abuse.daily_consignment_listing_limit }} 次 / 日</p>
        <p class="text-[0.625rem] text-muted mt-1">寄售购买上限：{{ governance.anti_abuse.daily_consignment_purchase_limit }} 次 / 日</p>
        <p class="text-[0.625rem] text-muted mt-1">同物资在架上限：{{ governance.anti_abuse.duplicate_open_listing_limit }} 份</p>
        <p v-if="governance.sanction.blocked" class="text-[0.625rem] text-danger mt-2">
