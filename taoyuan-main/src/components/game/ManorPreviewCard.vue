<template>
  <div v-if="snapshot" class="game-panel border border-accent/10 rounded-xs p-3 space-y-2">
    ...
    <div v-if="templateId === 'showcase'" class="grid grid-cols-2 gap-2 text-xs">
      ...
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">主视觉摘要</p>
    ...
    <div v-else-if="templateId === 'operational'" class="space-y-2 text-xs">
      ...
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">本周目标</p>
        <p class="mt-1">{{ snapshot.weekly_goal }}</p>
      </div>
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">今日经营标签</p>
        <div class="flex flex-wrap gap-1 mt-1">
          <span v-for="tag in snapshot.public_tags" :key="tag.id" class="text-[0.625rem] px-1.5 py-0.5 rounded-xs border border-accent/15 text-muted">
            {{ tag.label }}
          </span>
          <span v-if="snapshot.public_tags.length === 0" class="text-[0.625rem] text-muted">当前还没有公开经营标签。</span>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="templateId === 'collection'" class="space-y-2 text-xs">
      ...
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">同主题收藏</p>
        <p class="mt-1">
          {{ sameThemeText }}
        </p>
      </div>
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">热门庄园提示</p>
    ...
    <div v-else class="space-y-2 text-xs">
      ...
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">导览路线</p>
        <p class="mt-1">{{ snapshot.guide_routes[0]?.summary || '还没有设置主题路线。' }}</p>
      </div>
      <div class="border border-accent/10 rounded-xs p-2">
        <p class="text-[0.625rem] text-muted">留言痕迹</p>
