  ...
  <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_340px]" data-testid="online-cohabitation-warehouse-panel">
    <div class="game-panel-muted p-3">
      <div class="flex items-center justify-between gap-2">
        ...
        <span class="text-[10px] text-muted">{{ totalQuantity }} 件</span>
      </div>
        ...
        <div v-for="item in items" :key="`${item.item_id}-${item.quality}`" class="border border-accent/10 bg-black/10 p-2">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              ...
              <p v-if="frozenQuantity(item) > 0" class="mt-1 text-[10px] text-amber-100">
          ...
          <div class="mt-2 flex items-center justify-between gap-2">
            <span class="text-[10px] text-muted">卖价 {{ sellUnitPriceForItem(item) || '未配置' }} 文</span>
      ...
      <p v-if="actionMessage" class="mt-2 text-[10px] leading-4" :class="actionOk ? 'text-emerald-200' : 'text-red-100'">
        {{ actionMessage }}
      </p>
    </div>
