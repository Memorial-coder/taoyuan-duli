export type GameLogCategory =
  | 'system'
  | 'quest'
  | 'economy'
  | 'goal'
  | 'market'
  | 'breeding'
  | 'hanhai'
  | 'village'
  | 'processing'
  | 'social'
  | 'guild'
  | 'museum'

export type GameLogTag =
  | 'late_game_cycle'
  | 'theme_week_started'
  | 'weekly_goals_refreshed'
  | 'weekly_goals_settled'
  | 'weekly_goals_failed'
  | 'weekly_goals_settlement_mail_failed'
  | 'weekly_activity_refreshed'
  | 'weekly_activity_task_completed'
  | 'weekly_budget_activated'
  | 'weekly_budget_expired'
  | 'weekly_budget_auto_renew_toggle'
  | 'weekly_budget_auto_renewed'
  | 'weekly_budget_auto_renew_failed'
  | 'weekly_metric_snapshot_archived'
  | 'weekly_risk_report'
  | 'resource_sink'
  | 'ui_guidance_digest_refresh'
  | 'region_map_focus_refresh'
  | 'goal_completed'
  | 'goal_unlock_hint'
  | 'economy_sink_guidance'
  | 'breeding_completed'
  | 'breeding_discovery'
  | 'breeding_mutation'
  | 'hanhai_route_investment'
  | 'hanhai_relic_exploration'
  | 'hanhai_travel_prep_sink'
  | 'hanhai_relic_milestone'
  | 'hanhai_treasure_map'
  | 'hanhai_cycle_tick'
  | 'village_project_completed'
  | 'village_project_donation'
  | 'village_project_donation_milestone'
  | 'village_project_maintenance_cycle'
  | 'village_project_npc_feedback'
  | 'village_project_weekly_summary'
  | 'village_project_world_change'
  | 'quarry_unlocked'
  | 'quarry_expanded'
  | 'quarry_mine_refreshed'
  | 'guild_season_tick'
  | 'museum_operational_tick'
  | 'museum_exhibit_set'
  | 'region_resource_turn_in_sink'
  | 'processing_seed_bonus'
  | 'potential_workshop_refund'
  | 'workshop_double_output'

export type GameLogMeta = Record<string, string | number | boolean | null | undefined>
