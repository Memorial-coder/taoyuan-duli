...
### 0618 稀有来访拜访奖励
- 稀有来访按钮不再把历史见过当成已拜访，改为按 `rare_visitor_season_visit:${visitorId}:${year}-${season}` 判断本季是否领取；旧档见过来客会显示“再访”，本季未领仍可领取。
- 行脚书生、节庆商人、巡回艺人和异乡客按人设发放轻量奖励：研究券、展陈券、当天“戏棚余韵”行动耗时 -5% 和商路票；票券直接进钱包，巡回艺人额外记录稀有来访线索。
- `qa:rare-visitor-rewards` 补充状态、奖励、季节 key 与行动速度接线守卫，避免再次用历史记录锁死本季奖励。

### 0618 桃源小助理悬浮入口拖动
- 桃源小助理悬浮入口现在支持按住拖动位置，本次页面会话内会保留移动后的位置；拖动松手不会误触发打开 / 关闭，刷新后仍回到默认左下角。
- 助手面板会跟随入口从附近弹出，并在窗口尺寸或屏幕方向变化后自动夹取到可见区域；`qa:ai-assistant-ux` 和 `qa:ai-assistant-mobile-accessibility` 补充拖动、点击抑制和非持久化守卫。

### 0618 工坊入口直达恢复
- 从 `/#/?redirect=/game/workshop` 这类深链回到主菜单后，继续旅程会消费并保留站内工坊目标，不再只停留在菜单里；新建旅程和载入存档也会优先跳回这个 `/game` 目标。
- `qa:game-deeplink-restore-guards` 和 `qa:game-deeplink-browser-smoke` 同步补上恢复守卫，覆盖主菜单 redirect 归一化和“点存档后进入工坊”的真实浏览器路径。

### 0617 节庆摊位限购状态修复
- 节庆摊位购买前会先同步当前服务端运行存档，避免后端按旧存档日历写入另一周的购买记录，导致买到票券包后面板仍显示 `0/1`。
- 摊位快照和二次购买判定优先显示个人/全局限购耗尽，再进入操作冷却治理；`qa:festival-stall-save-sync` 补充购买前同步和限购优先级守卫。

### 0617 多茬作物再生周期修复
- 多茬作物收获后不再把进度预填为“首熟天数 - 再生天数”，而是从 0 开始按 `regrowthDays` 重新计时；远古水果第二、第三茬不会再在收获后的第 2 / 第 3 天连续成熟。
- 室外农田、温室、戒指 / 钱包 / 春息成长加速和农场进度显示统一使用同一套有效生长天数计算；`qa:fertilizer-retention` 增加远古水果再生回归。

### 0617 农耕20级专精效果对齐
- 农耕 Lv.20 选择弹窗、技能页和售价明细统一口径：丰收之神为作物收获 50% 概率双倍、作物售价 +50%；土地神为农田自动浇水、农耕行动体力消耗归零。
- 室外农田、温室、普通种子和育种种子的播种/农耕操作统一读取土地神零体力；丰收之神双收与作物售价加成不再被土地神继承，加工品售价也不再误吃农耕 Lv.20 加成。
- 确认农耕精研“加工流线”已接入工坊加工耗时计算，手动投产、炼丹投产和虚空原料箱续产都会使用缩短后的有效天数；`qa:skill-mastery-effect-guards` 补充农耕 Lv.20 文案与实际效果守卫。

### 0618 铸魔炉与随机 Affix Roll
- 工坊制造区移除旧武器 / 工具 / 装备直连附魔面板，改为 Lv.7 建造 `enchanting_forge`“铸魔炉”，再从加工区点击机器打开 `processing-enchanting-forge-modal`；`spirit_forge` 继续作为“仙灵炉”独立保留。
- 新铸魔统一写入 `affixes: ForgeAffixRoll[]` 随机 affix roll，旧 `enchantmentId`、固定武器附魔和旧装备方案读档时迁移成单词条 affix；装备预设改按 `id:value:quality` 签名精确匹配。
- 铸魔模式改为随机 / 定向附魔 / 保留重铸三档：随机消耗 30000 文、青铜锭和暗影矿；定向只选大方向，不选具体词条或数值；Lv.15 保留重铸保留具体 affix id 并重掷数值与品质。
- 加工区已有铸魔炉时会把铸魔炉机器组置顶；铸魔炉弹窗内遇到锁定的武器、戒指、帽子或鞋子可直接点“解锁”，不用再回背包或角色页处理。
- 随机词条数量按工坊等级滚动：Lv.7 固定 1 条，Lv.10 为 1/2 条 70%/30%，Lv.15 为 1/2/3 条 50%/38%/12%；品质按数值百分位显示普通、优良、卓越、极品。
- 武器、镐子、戒指、帽子和鞋子词条全部改为区间数值，背包、角色、矿洞 HUD、预设详情和售价读取实际 roll；武器包含锋利、炽烈、精准、吸血、坚韧、幸运、迅捷、破甲、镇魂、虫猎、斩邪、残响和割草等词条。
- 镐子开放石屑、省力、疾手、丰采、炼矿、寻宝；装备池扩展戒指收益 / 战斗 / 探宝、帽子防御 / 农作 / 经验、鞋子移动 / 体力 / 矿洞方向，其中鞋子“轻身”按 +5%~+14% 旅行加速 roll。
- `qa:workshop-enchant-guards` 和 `qa:equipment-guards` 改为守护铸魔炉机器、成本、概率、弹窗 test id、方向池、保留重铸、affix 存档迁移、装备方案签名和 rolled value 实际生效。

### 0617 根骨危息护命实装
- 根骨末端“危息自觉”改为“危息护命”：每阶提供矿洞战斗减伤 0.4%，30 阶封顶 12%；10 阶矿洞战败不再随机掉落背包物品，20 阶击杀怪物会清退相邻普通怪物，30 阶 Boss 开战时属性降低 20%。
- 满阶危息护命在 Boss 战失败时会触发撤离回入口并保住本次探索战利品；失败仍保留铜钱惩罚和回血结算，`qa:potential-effect-guards` 同步守护根骨护命数值和矿洞接线。

### 0617 巧作潜能实装
- 巧作“仓中有序”改为每阶 +1 仓库箱位，最高 +30；仓库页显示基础箱位与潜能箱位来源，商店仓库扩容仍按基础箱位结算，避免潜能箱位误卡常规扩容。
- “工坊手记”改为机器完工时每阶 +1% 概率返还 1 份真实投入原料，30 阶封顶 30%；手动收取、日结自动收取和虚空原料箱续作路径都会尝试返还，并优先放入虚空成品箱。
- “炉火耐心”改为炼丹结果容错，每阶 +1%、最高 30%，通过成丹 / 奇丹 / 废丹权重调整提高高价值丹方稳定性；`qa:potential-effect-guards` 同步守护三条巧作潜能的真实接线。

### 0617 哲学家料理增益调整
- 采集 Lv.20 炼金哲人不再让料理增益永久保留，改为食物恢复仍 +200%，料理增益强度 +25% 且持续 2 天；送礼倍率只放大超过 1 的增量，避免倍率失控。
- 技能页和专精选择弹窗同步更新说明，并新增 `qa:cooking-philosopher-guards` 守护哲学家两天持续、存档读写与到期清空行为。

### 0617 工坊配方区去重渲染
- 工坊展开机器组时，同一组的配方列表改为只渲染一份，并默认给该组第一个空闲机器开工；空闲机器卡片保留轻量状态提示，避免多台空闲同类机器重复生成整组配方 DOM 导致卡顿。
- “仅显示有材料的配方”切换改为复用已构建配方选项做轻量过滤；丹炉配方卡复用同一份炼丹材料计划做可用判定和替代提示，材料行仍按候选材料总库存统计，避免显示被单次消耗量截断。
- 点开单个配方详情时直接复用已点击的配方选项，不再为丹炉详情弹窗重新构建整张配方卡，减少材料很多时的点击卡顿。
- 展开机器组时配方列表改为按小批次异步整理，先显示“正在整理配方...”并逐批渲染卡片；收起组会取消未完成加载，避免丹炉或大配方组一次性同步计算造成展开 / 收起卡顿。
- 合并库存查询改为共享响应式索引，工坊制造弹窗、丹炉用途替代材料统计和配方刷新签名不再按每个材料反复扫描全部仓库箱子；制造区“已有 N”徽标改用轻量机器计数，避免切到制造页时误触发加工区配方整理。
- 折叠的机器组继续跳过配方计算，`qa:processing-recipe-detail-guards` 同步守护组级配方卡仍进入详情确认弹窗与丹炉材料计划复用。

### 0617 博物馆学者委托开放提示
- 学者委托列表现在会在未开放条目下直接显示开放条件，包含当前/所需展陈等级、捐赠数量和对应馆区等级，避免只看到“未开放”却不知道差什么。`qa:museum-scholar-commission-unlock-hints` 同步守护这段提示不再丢失。

### 0617 工坊后期扩建与里程碑奖励
- 工坊等级上限从 Lv.7 扩展到 Lv.15，设备上限随原公式提升到 90 台；Lv.7+ 扩建开始消耗青铜锭，Lv.9+ 追加秘银锭，并保留高额铜钱、铱锭、精制石英、石材 / 木材消耗，作为后期资源出口。
- 工坊 Lv.10 解锁加工加速，所有新开工加工时间缩短 15%；Lv.15 解锁工坊精研，加工完成收取时 10% 概率额外产出一份，并覆盖手动收取、自动收取和虚空原料箱续作路径。
- 工坊扩建弹窗现在显示最高等级、当前生效加成、里程碑奖励列表和满级状态；升到里程碑等级时日志会直接提示解锁的被动奖励。

### 0617 种子品质影响收获品质
- 种植普通种子时会记录种子自身品质，并在收获品质判定中叠加品质加成：优良 +0.06、精品 +0.12、极品 +0.20，和肥料、戒指、烹饪全技能增益一起参与 `rollCropQualityWithBonus`，但不会保证必定极品。
- 普通农田、温室、单株种植、一键种植和抽象收获入口同步接入种子品质；旧存档与示例存档补齐 `seedQuality` 默认值，收获后清空本次种子品质，避免多茬作物无限复用一次种子的品质加成。
- 新增 `qa:seed-quality-harvest-guards`，并同步资源消耗守卫，防止播种扣除的种子品质再次丢失或退款时品质不一致。

### 0617 放牧与畜牧产出解耦
- 放牧与日结自动产出改为两条完全独立的计时路径（daysSinceGrazingProduct + daysSinceAutoProduct），互不干涉：放牧每天都能收产品（只要够周期），日结每天自动产出也独立运行，不管今天是否放过牧。
- 两条路径都会结算农耕经验（3-12 XP/产品）；雨天/雷雨天的日结产出仍然执行并给经验。
- `Animal` 类型拆分 `daysSinceProduct` 为双计时器字段，旧存档自动迁移；`pastureProducedToday` 字段已移除。
- `qa:animal-production-guards` 重写覆盖独立计时器架构，包括类型定义、双路径读写、经验发放和旧档兼容。
- 修复 `daysSinceGrazingProduct` 在日结中从未递增的 bug（2天以上周期牲口通过放牧永远无法产出）。
- 修复放牧就绪判断中的 `+1` 偏移（修复递增后会导致2天周期牲口每天产出）。

### 0617 背包同物品同品质合并收口
- 背包物品页改为复用 `useInventoryStore.visibleItems`，按物品 id + 品质合并可见堆叠；点击详情、作物用途建议、空格占位、锁定/解锁、食用、使用和丢弃都会对准同一条可见堆叠，并在数量归零后自动关闭详情弹窗。
- NPC 送礼、配偶送礼、隐士供奉、家园箱子 / 虚空箱存入和地窖陈酿选择列表同步改读合并后的可见背包，避免商圈来源批次或普通批次把同类物品拆回多条；相关消耗路径改用未锁定堆叠扣除，锁定物品不会被误吃、误用或误交付。
- `qa:inventory-guards` 补充可见堆叠合并、锁定消耗、详情弹窗关闭、作物建议品质 key，以及礼物 / 供奉 / 箱子 / 地窖入口不回退到 raw 批次列表的守卫。

### 0617 童心灵息周板委托收口
- 阿花/石头挚友后的童心灵息特殊订单现在会在限时活动窗口开启时仍保留进候选池，不再被活动源白名单误筛掉；特殊订单反重复冷却会先从候选池排除仍在冷却的标签，避免周板连续回落到同一批老委托。
- `qa:quest-guards` 补充童心灵息委托运行回归，覆盖挚友门槛、活动窗口候选、反重复过滤、完成后灵息发放、潜能来源回执、特殊订单回执和完成历史记录。

### 0617 行旅图首领入口并入开放地图
- 开放行旅地图的首领地标格现在直接显示首领名称、路线门槛、体力耗时、奖励家族和“发起首领远征 / 再次挑战首领”按钮；旧远征页的重复首领按钮与长篇“首领准备”说明降级为简短指引，主入口回到新版地图格子。
- `qa:region-open-world-guards` 增加首领地标 CTA 守卫，确认 `boss_landmark` 不再只是观察格，开放地图会把点击路由到同一套首领远征与交战弹窗流程。

### 0617 潜能 / 研究券体验线收尾
- 研究券兑换百工札记的 QA 从静态接线扩展为运行态兑换：验证 5 张研究券扣减、百工札记写入潜能材料、补齐潜能心得后巧作节点可参悟并实际提升加工耗时潜能；潜能页资源格会提示百工札记最近可投入的巧作节点。
- 百科搜索的券 / 卷误输过滤与排序收口，`qa:glossary-coverage` 守护研究卷、建设卷、展陈卷等误搜仍命中正式票券词条。

### 0617 工坊与装饰图标补位
- 工坊制造卡片、装饰商店和已放置装饰列表现在会复用百科物品图标资产，按物品 id 或装饰名称显示 128/256 WebP 图标；自动抚摸机、箱子变体和目录装饰等无独立背包物品的条目会回落到对应基础名称，避免已接入图片的内容在这些入口仍只显示文字。

### 0617 技能专精显示文案对齐
- 农耕、钓鱼 15/20 级专精在技能页、角色页和售价明细中统一使用选择弹窗里的名称与真实售价效果文案；牲畜大亨会显示为动物产品售价 +30%，售价明细不再泛化成“高阶牧养 / 高阶渔业”。`qa:skill-mastery-effect-guards` 补充对应文案守卫。

### 0617 每日委托池扩容
- 村民每日委托池扩展到 77 条模板、43 种请求物品，新增鸡蛋、牛奶、蜂蜜、柴火、野蘑菇、鱼饲料、绿茶、桂花茶、铁锭、蛋黄酱、奶酪、羊毛、兔毛等常见经营产物，并加入 16 条传闻轻任务，让告示板不再总围着鲫鱼、桂花、稻米、木材、竹子、铁矿循环。
- 委托生成会降低近期同 NPC / 同物品 / 同签名重复权重，传闻任务可绕过正式 NPC 类别过滤并保留读档后的传闻标签；`qa:quest-guards` 同步守护委托池多样性、传闻轮换和读档归一化。

### 0617 补充物品图标接入
- 物品图标准备脚本现在会在默认 `images/item` 源之外自动并入 `images/0615补充`，按既有 item/NPC 资产规则生成 128/256 WebP 透明方形图标、刷新 `item-icon-manifest.json` 与 QA 报告；同名变体保留主源图，补充源只填补缺失图标。

### 0617 凤凰套装经验奖励修正
- 凤凰套装不再提供额外 3 件套经验加成，避免“经验加成+15%”作为套装奖励展示并实际计入技能经验；凤冠、凤鸣靴等单件装备自带经验词条保持不变。`qa:equipment-guards` 同步守护凤凰套完整装备后只保留单件经验来源，并确认套装目录不再显示经验套装奖励。

### 0617 淘金盘满级耗时对齐
- 淘金基础耗时从 1 小时调整为 30 分钟，继续叠加淘金盘等级减时、钓鱼技能减时和 10 分钟最低行动时间；铱金淘金盘配合满级钓鱼时会和钓鱼、竹林采集一样显示 10 分钟。`qa:resource-consumption-guards` 同步守护钓鱼、采集、淘金三条基础耗时不再分叉。

### 0617 节庆摊位限购与重复回执修复
- 节庆摊位购买限购现在会从个人计数、购买记录和成功回执共同重建，不再只依赖 `user_usage` 派生表；即使旧记录缺少计数，同一周同一票券包也会被正确判定为已达个人限购。
- 购买成功回执被网络重试或浏览器重复提交时，前端只确认该回执，不再把本次奖励差量重复合并进当前运行态，避免研究券、展陈券等票券被重复入账。`qa:festival-stall-save-sync` 同步守护重复回执与服务端限购计数路径。

### 0615 行旅图首领交战弹窗
- 行旅图首领远征推进到最终节点时，不再一次性模拟胜败，而是进入可存档的 `bossCombat` 交战态；页面会弹出专属首领交战窗口，玩家可逐回合选择攻击、防御或前线压制，胜利后再回到原有远征收束奖励流程。
- `qa:combat-flow-guards` 增加行旅图 Boss 交战回归，守护最终节点创建交战态、交战中不能继续推进绕过弹窗、逐阶段击破后进入可收束状态。

### 0615 节会小游戏潜能沉淀
- 互动节日小游戏在有效结算且奖金大于 0 时，会通过独立“节会小游戏”潜能来源沉淀潜能心得×1、灵息×1；该来源拥有独立每周 2 次 / 4 份材料上限，不再占用仙灵记忆或节庆出货的灵息机缘额度。`qa:potential-resource-guards` 同步守护小游戏结算入口、奖励内容和周上限。

### 0615 发现类目标可完成性修复
- 每日 / 季节发现目标刷新前会检查剩余未发现物品数量，图鉴已探完或剩余数量不足时不再刷出“新增发现”类目标；行旅图探索周的“记录见闻”改为可重复的行旅路线、区域资源交付或首领结算进度，不再要求本周新增 3 项见闻。`qa:weekly-board-activity-guards` 同步守护目标池和行旅计数入口。

### 0615 百科票券与系统覆盖
- 百科补齐票券、奖池阶段、票券兑换、密匣、潜能分支/材料/来源、钱包物/钱包流派、周预算渠道/档位和村庄建设项目词条；搜索“建设卷/展陈卷”等常见误写时也会把对应票券词条排到前列。
- 新增 `qa:glossary-coverage`，守护上述数据表的百科词条、搜索关键词、分类颜色和券/卷误写排序不再脱钩。

### 0615 矿洞首领潜能复战收益
- 矿洞首领潜能材料改为“首次潜能首获必定沉淀，复战按层数逐步提高概率并在深层封顶”；120 层主矿 Boss 复战不再因为楼层奖励已领取而完全跳过潜能结算，骷髅矿穴高层首领也复用同一套首获 / 复战凭据。
- 潜能页材料来源文案改为“随层数有一定概率”，不直接暴露复战精确概率；`qa:potential-resource-guards` 补充首获记录、复战概率曲线和凭据区分守卫。

### 0615 潜能材料百科来源说明
- 百科里的潜能材料与潜能来源词条补充具体玩法入口：灵息会说明仙灵结缘记忆归档、阿花/石头挚友后的童心甜点委托和节日当天出货箱有效结算，潜能心得、百工札记、山行玉也会写明对应订单、考据、行旅和首领结算路径，避免只显示抽象来源名。

### 0615 果树荔枝脱水接入
- 脱水机新增果树荔枝入口：荔枝树和果蝠洞产出的 `tree_lychee` 现在也能加工成 `dried_lychee`，不再只有田地作物版 `lychee` 能制作荔枝干；`qa:item-data-guards` 同步守护两条荔枝干配方。

### 0615 研究券兑换百工札记
- 钱袋票券兑换新增“匠作研修札匣”：可用 5 张研究券兑换百工札记×1，直接写入潜能材料而不是背包物品；钱包兑换卡会展示“潜能材料：百工札记×1”，并新增 `qa:reward-ticket-potential-exchange` 守护研究券、百工札记与潜能发放路径不漂移。

### 0615 博物馆考据满捐后续产
- 博物馆“考据”潜能来源不再只依赖一次性捐赠里程碑；学者委托领奖现在也会沉淀博物馆考据材料，捐满 43/43 后仍可通过博物馆经营拿到本季 3 次来源进度。`qa:potential-resource-guards` 同步守护满捐后来源不再断供。

### 0615 潜能页分区浅框整理
- 潜能页资源、分线、节点、重修和材料来源改为分区浅框布局，提升宽屏下的信息边界感；`qa:potential-ui-structure` 增加分区结构守卫。

### 0615 浅色主题深色背景清理
- 公告弹窗、公告历史、记录中心空态和主菜单在线入口改为读取主题 surface / RGB 变量，避免浅色与高对比主题下继续出现固定黑底或暗绿底；`qa:theme-contrast` 补充这些玩家侧入口的源码守卫。

### 0615 公告正文换行放宽
- 后台更新公告正文不再限制换行数量，长 Markdown 公告仍保留 8000 字长度上限、CTA URL 安全校验和内容审核；`qa:announcement-flow` 新增 360 行正文草稿保存回归用例。

### 0615 商圈购入品回购防倒卖
- 商圈直接购入的库存物品现在会记录来源、购入日期和实际购入单价；同名同品质物品按来源批次分格，仓库存取、临时背包、排序和存档读档都会保留这些字段，避免商圈购入品和玩家自制产物混堆。
- 商圈来源物品出售时只走“商圈回购”，按购入单价的 80% 结算，不再吃品质、技能、装备、祝福、仙缘、行情或节庆倍率；回购收入不计入市场出货历史、行情压力或累计出售收入，普通产物仍按原售价逻辑结算。
- 出售弹窗和一键出售改为按库存格结算并展示回购说明；出货箱会拒绝商圈来源物品并提示只能商店回购。新增 `qa:shop-buyback-guards`，并扩展 `qa:inventory-guards` 覆盖来源字段合堆、仓库存取、存读档和出货箱防洗。
- 背包物品页改为按物品 + 品质合并展示同名批次，玩家侧不再看到商圈来源/普通来源拆成多张卡；锁定按钮会同步锁定或解锁同物品同品质的全部底层批次，`qa:inventory-guards` 增加可见合并与锁定回归守卫。

### 0615 目标页版面顺序与宽屏铺满修复
- 目标页根面板移除居中宽度上限，进入游戏内容区后随父容器铺满；版面顺序调整为本周活动、目标清单、目标摘要、经营线索，避免活动和目标列表在宽屏 / 全屏下仍缩成中间窄列或被经营提示挤到后面。
- `qa:weekly-board-activity-guards` 补充目标页根宽度与模块顺序源码守卫；本轮浏览器验证在 2048px 视口下 `goals-page` 与 `.game-layout-body` 同宽，左右 gap 为 0。

### 0615 页面宽度 1% 微调
- 设置里的“页面宽度”自定义模式现在支持 1% 步进，减号、加号和滑杆都从统一的 `PAGE_WIDTH_PERCENT_STEP` 读取；`qa:settings-dialog-touch-target` 补充源码守卫，防止回退到 5% 一档。

### 0615 游戏视窗外层滚动锁定
- 游戏路由现在会给 `html` / `body` / `#app` 加上游戏壳层类，进入游戏后外层页面固定在 `100dvh` 并关闭页面级滚动槽；游戏框贴合视窗边缘，中间内容面板继续独立滚动，避免瀚海等长页面拖动时整个视窗跟着上下滑动。
- `qa:desktop-layout-smoke` 补充外层纵向溢出断言，防止 `100vh + #app padding` 回归。

### 0615 瀚海商路经济回收
- 瀚海货架把仙人掌种子、椰枣种子、炸弹补给和巨型炸弹配方接入绿松石兑换，并在商路商店补齐材料成本、持有量、背包容量和限购禁用提示；遗迹探索保留铜钱解锁但不再直接返还铜钱，奖励改为西域材料、藏宝图和商路 / 研究 / 展陈票券。
- 藏宝图现金期望从高额返现下调到约 488 文，主要价值转向绿松石、西域香料、丝绸和票券；仙人掌、椰枣新增两条 T10 高阶育种支线并补齐作物、种子和用途资料。新增 `qa:hanhai-economy-guards` 守卫绿松石成本、遗迹票券到账、藏宝图现金范围和新育种分支。

### 0615 虚空原料箱续产扣料防护
- 虚空箱设为作坊原料箱时，自动续产的箱内成组扣料现在会先在同一份箱内快照上模拟逐条扣除，再执行真实扣除，避免重复/重叠材料需求被按单条库存误判，导致材料箱看似有料却不上料。`qa:resource-consumption-guards` 同步守护虚空箱成组扣料不再退回逐条原始库存判断。

### 0615 虚空箱弹窗高度收束
- 虚空箱远程存取弹窗现在限制在视口内，箱子列表改为内部滚动，避免大量虚空箱或展开满箱时上下溢出屏幕；存入列表与道具详情弹窗同步收束滚动边界。

### 0615 委托同物品库存进度修复
- 进行中的钓鱼 / 挖矿 / 采集等库存交付类委托进度现在以当前可交付库存为准，不再用历史 `collectedQuantity` 顶满进度条；同一物品的多个委托在其中一个提交并消耗库存后，剩余委托会立即回退到真实数量。
- `qa:quest-guards` 新增两个鲫鱼委托共享库存的回归用例，守护 7 条鲫鱼提交大委托后 4 条委托不再继续显示满进度 / 可提交；本轮验证：`qa:quest-guards`、目标 `useQuestStore.ts` ESLint、`node --check scripts/qa-quest-guards.mjs` 通过。

### 0615 柳村长票据转换入口
- 柳村长互动页新增“村务票据转换”：柳村长好感达到 friendly / 1000 且完成 2 项村庄建设后，可按 `3 张来源券 + 1200 文 -> 1 张目标券` 在建设券、展陈券、商路票、研究券之间折损转换。
- 转换按每周总次数限制，四券任意组合合计最多 6 次；转换所得只增加当前票券余额，不计入累计入账和阶段奖池进度，避免折损循环刷奖池。
- 聊天 / 送礼跨过柳村长好感门槛、完成第 2 项村庄建设时会给出村务票据提示；新增 `qa:mayor-ticket-conversion` 守卫转换范围、成本、周上限、解锁条件、lifetime 隔离和两处提示入口。

### 0615 料理增益满状态可食用
- 背包内带当天增益的烹饪品现在即使体力和 HP 已满也可以食用以开启增益；普通恢复品仍会在无恢复收益时阻止误吃。`qa:inventory-guards` 同步守护按钮禁用与点击处理入口。

### 0615 农场温室收获提示回补
- 农场页顶部“温室”入口在有成熟作物时直接显示 `可收N`，恢复原来温室卡片能看到可收获作物数量的提示。
- 温室入口悬停摘要补充地块数、可收获数量和果树数量，`qa:greenhouse-fruit-trees` 同步守护顶部入口提示不再丢失。

### 0615 商路通商稳价缓冲
- 瀚海商路投入后会按匹配品类降低近 7 天出货量的行情压力：每条已运转商路提供 25% 有效销量缓冲，多条最高折到原销量 50%，让通商表现为市场容量扩大而不是直接抬高基础售价。
- 商店行情看板、售价明细、直接出售与出货箱结算共用同一套缓冲口径，售价明细会显示商路缓冲前后的出货量；新增 `qa:market-commerce-relief` 守卫商路映射、缓冲封顶和结算入口一致性。

### 0615 百科图标备选切换修复
- 百科详情里的物品大图恢复为可点击切换图标备选，点击会按 01 / 02 / 03 循环，并在标题下补回小图选择器；背包、图鉴和百科继续共用同一份图标偏好。
- 本轮验证：`src/components/game/GlossaryTab.vue` ESLint 与 `npm run type-check` 通过。

### 0615 周预算自动续投
- 周预算已投入槽位新增“自动续投”开关；跨周时会按原档位重新计算目标声望减免并扣除当周实付价，余额不足或档位失效则跳过续投并提示手动重选。
- 日结日志会区分已自动续投、未开启续投和续投失败的槽位；新增 `scripts/qa-weekly-budget-auto-renew.mjs` 守卫保存字段、扣款续投、钱包开关和跨周提示。

### 0615 邮箱一键已读与桌面布局整理
- 邮箱页恢复独立“一键已读”入口，批量标记会走服务端 `/api/taoyuan/mail/read-all`，只清理当前账号邮箱未读状态，不会影响私聊隐藏礼物、奖励领取或已领取清理。
- 电脑端邮箱改为更宽的列表 / 详情两栏，刷新、一键已读、一键领取、清空已领取固定在列表顶部；详情区在宽屏下将正文与奖励 / 操作分栏展示，减少长邮件和领奖按钮互相挤压。
- 邮箱列表面板、卡片和桌面置顶工具条改为读取主题面板 / surface 变量，浅色和高对比主题下不再保留固定深色背景。
- `qa:mailbox-reward-idempotency` 覆盖批量已读幂等与“已读不消耗可领奖励”，`qa:online-ui-structure` 补充邮箱批量操作和桌面布局静态守卫。

### 0615 种植季末成熟提醒前置
- 露天田地在单格 / 一键播种普通种子或育种种子前，会先判断本季剩余天数是否足以成熟；不足且下季无法继续生长时弹出“目前季节剩余天数不足以成熟，是否确认种植”，取消不会消耗种子、体力或时间。
- 判断复用地块肥料和当前作物生长加成，并移除播种后才出现的旧日志提醒；新增 `scripts/qa-farm-season-risk-confirm.mjs` 守卫前置确认入口和旧提醒清理。

### 0615 全屏模式自定义界面宽度修复
- 右下角进入全屏后，游戏根容器现在继续读取设置里的自定义页面宽度比例，并在全屏视口中居中显示，避免全屏态强制铺满 `100vw` 导致“自定义大小”看似失效。
- `qa:desktop-layout-smoke` 补充全屏 70% 页面宽度断言，守护全屏、跨页面跳转和桌面自适应布局同时生效。

### 0615 目标页经营提示归位
- 告示板顶部“经营提示”抽成共享组件后改为只在目标页展示；告示板保持委托和本周活动主入口，目标页集中承载本周主路线、活动编排、限时收尾、特殊订单风向、关系联动和今日随机目标。
- `qa:quest-guards` 改为守护经营提示仅出现在目标页，不再回灌到告示板；本轮验证：目标文件 ESLint、`qa:quest-guards`、`type-check` 通过。

### 0615 后20级通用精通池与旧档迁移
- 满级后的各项技能经验现在汇入同一个通用精通池，统一按 60000 经验兑换 1 点；通用精通点可用于解锁任意已满级技能的精研节点，不再按技能分别累积精研点。
- 旧档读入时会把各技能既有精研点、剩余精研经验和已解锁精研节点成本折算成累计经验后汇入通用池；合法已解锁节点保留，技能上的旧精研余量会清空以避免重复迁移。

### 0615 游学书肆空货架提示
- 行脚书生到场但藏书已收齐时，商圈总览现在会显示“本档藏书已收齐”的空货架提示，避免玩家误以为游学书肆未接入或入口丢失。

### 0615 技能专精重修入口
- 技能页已选 Lv5 / Lv10 / Lv15 / Lv20 专精现在提供“重修”入口，确认后会清空所选层级及下游分支，并复用原有天赋选择弹窗重新选择，避免选错后只能长期背负旧分支。
- 重修按实际清空层级累计收费（Lv5 12000 文、Lv10 18000 文、Lv15 28000 文、Lv20 40000 文），每项技能每季限重修一次；旧档会自动补齐并归一化本季重修记录，新增 `qa:skill-perk-respec-guards` 守卫成本、清空范围、扣款和季节限制。

### 0615 潜能正式版 30 阶与参悟预览
- 潜能页 20 个节点全部开放为 30 阶成长，原本的预留 / 首版文案改为正式能力描述；成本改为按阶生成的长线曲线，后段材料需求更高，生命、体力、加工、工具、行旅、节庆等效果继续保留上限防止数值失控。
- 参悟按钮改为先打开预览弹窗，展示当前阶、下一级、效果变化、材料拥有量和阻塞原因；确认后才扣材料升阶，成功后继续显示本次提升结果提示条。`qa:potential-save-guards`、`qa:potential-effect-guards` 和 `qa:potential-ui-structure` 同步守住 30 阶、全节点开放和弹窗结构。
- 服务端存档异常字段校验同步接受潜能节点 30 阶，避免 `craft_processing_flow` 等节点 4-30 阶被误判为越界并强制修回 3；`qa:potential-save-guards` 补充服务端校验 / 修复回归。

### 0615 潜能随机来访参与门槛
- 潜能解锁条件新增随机 NPC 里程碑读取：会从熟人册、随机小委托、长住名册、长住故事、家人线与正式关系线里汇总当前进度，并在未解锁提示里显示“当前 N/M”和对应推进方向。
- 根骨线保留生命上限根节点不加硬门槛，只让体力节点和后续护身节点轻度读取随机来访参与；人和线则承担更多 NPC 关联门槛，让随机来客系统能自然影响委托、节会和村社方向。
- 潜能页在卡住随机 NPC 门槛时新增“去桃源村”入口，并补充 `qa:potential-random-npc-guards` 守卫类型、节点配置、进度提示和页面跳转。

### 0615 云存档冲突保存当前进度修复
- 云存档冲突面板选择“保存当前进度”时，现在会在确认覆盖前重新序列化当前运行态，避免玩家冲突后继续游玩再保存时写回旧的待同步副本；导入存档冲突仍会保留导入文件本身作为待处理副本。
- `qa-save-load-feedback` 补充运行态冲突刷新与导入冲突来源隔离断言，防止“当前页”和“导入副本”再次混用。

### 0615 钓鱼断线与挣扎机制
- 钓鱼小游戏新增真实断线与挣扎事件：进度曾推进后被磨到 0 会断线，陷阱浮漂可自动续线 1 次；鱼会按难度偶发挣扎，捕手、精致鱼饵和软木浮漂会提高稳住挣扎的概率。
- 软木浮漂和陷阱浮漂文案同步到实际效果，并新增 `qa:fishing-mini-game-guards` 守卫小游戏参数、事件触发、结算失败原因和玩家文案。

### 0615 矿洞移动端全屏状态条
- 矿洞探索和战斗弹窗内置时间、体力、HP 轻量状态条，移动端全屏时不再依赖容易被弹层遮住的全局顶栏；弹窗顶部留白同步兼容安全区。
- 新增 `qa:mining-hud-structure` 守卫探索 / 战斗两处状态条、危险状态着色和移动端窄屏布局，防止矿洞 HUD 回退。

### 0615 温室收获工具占用修正
- 温室单块收获与一键收获现在会遵守镰刀升级占用状态；镰刀升级中时无法收获温室成熟作物，避免温室入口绕过工具占用规则。

### 0615 灶台制作耗时提示
- 灶台烹饪弹窗在材料下方补充“制作耗时”，批量制作时显示本次总耗时与每份耗时；料理 QA 守卫同步防止耗时提示回退。

### 0614 潜能页入口、来源与天赋精研扩容
- 新增“桃源潜能”页面与移动端个人页入口，潜能页可查看根骨、巧作、山行、人和四条分线的资源、节点、效果、消耗、重修预览和来源上限。
- 潜能系统接入存档序列化与读档归一化，并落到实际玩法：生命 / 体力上限、工具体力消耗、昏倒铜钱损失、加工耗时、行旅压险、矿洞进层提示、节庆出货小加成、任务板偏向提示和区域图标记提示。
- 潜能材料现在可从矿洞首领、高风险行旅、特殊订单、主题周结算、博物馆里程碑、节庆出货和仙灵记忆中沉淀，并通过统一发放 helper 做凭据去重与日 / 周 / 季上限控制。
- 现有天赋精研每个技能补到 7 个节点，新增节点优先承担信息、路线规划和预留分支感，不额外堆攻击、暴击或全局售价倍率。
- 终局综合样本档默认落点改为潜能页，并补充潜能资源与已点节点；新增潜能规格、数值审计文档，以及 `qa:potential-save-guards`、`qa:potential-effect-guards`、`qa:potential-resource-guards`、`qa:potential-ui-structure` 守卫。
- 农场出货箱弹窗改为固定视口高度，避免移动端和桌面端列表增减时弹窗高度跳动；`qa:mobile-ui-smoke` 补充对应结构守卫。

### 0614 随机 NPC 家族节点用途说明
- 随机 NPC 长住后的关系页补充“家族节点有什么用”、见面 / 深线进度和每个家族节点的用途说明，让师门、旧债、商队等节点能直接看出会影响家人线、委托、核心深线和后续门槛。
- `qa:random-npc-content-guard` 已补充对应页面文案和工具函数守卫，防止用途说明回退成只显示节点名称。

### 0614 农场温室入口位置
- 农场页顶部页签新增“温室”入口，温室解锁后会和“田庄 / 林木”并排显示，避免玩家在田庄内容区滚动查找温室。
- 田庄内容区移除重复的温室卡片入口；`qa:greenhouse-fruit-trees` 补充页签位置守卫，并用 Playwright 截图确认“田庄 / 林木 / 温室”三项同排展示。

### 0614 矿洞雷火弹与炸弹返还收敛
- 雷火弹不再覆盖整层矿洞，改为有限大范围爆破；火药包 / 雷火弹只清除普通怪物，感染层怪物会被炸开显形但仍需正面清除，BOSS 仍不能被炸弹跳过。
- 挖矿 15 级“深渊挖掘者”调整为 50% 炸弹返还；20 级“深渊矿工”改为每层首次炸弹必定返还、后续 60% 概率返还，不再无限连发。稳压爆破也不再把揭出怪物、BOSS、陷阱或楼梯当作空爆返还。
- 技能页、专精选项、炸弹道具描述和百科说明已同步收窄，并新增 `qa:mining-bomb-balance-guards` 守护雷火弹范围、返还上限、感染层限制和玩家文案；`qa:skill-mastery-effect-guards` 同步更新挖矿炸弹返还断言。

### 0614 鱼神传说鱼权重修复
- 鱼神不再把传说鱼权重推成近似必出：海洋商人只保留售价路线，不再参与传说鱼权重；鱼神传说鱼基础权重从 5.0 下调并加占比封顶，默认窗口最高约 45%，定向鱼饵下最高约 50%。
- 鱼神新增短期调节：刚钓上传说鱼后会短暂降低传说鱼权重，连续未中传说鱼时逐步提高保底，但仍受硬上限保护，避免普通鱼 / 困难鱼长期消失。
- 技能页和专精选项文案改为“传说鱼窗口内出现率大幅提升”，不再承诺“必定出现”或“任何时间任何天气”；`qa:skill-mastery-effect-guards` 补充鱼神权重封顶、冷却、保底和海洋商人解耦守卫。本轮目标 ESLint 与完整 `qa-skill-mastery-effect-guards` 均通过。
- 同类扫描补齐 20 级“诱饵神”：诱饵师 / 诱饵宗师 / 诱饵神现在分别按 2 / 4 / 8 倍放大鱼饵权重与野生鱼饵双倍鱼获概率，技能页文案同步改为“鱼饵效果×8，且不消耗鱼饵”，并补充对应 QA 模型守卫。

### 0614 NPC 随机来访审计展示收敛
- NPC 页“陪伴总览”里的随机来访关系记录改为玩家可读的“关系回看”摘要，不再直接展示 `family_special_event_progressed`、`privacyScope`、`idempotencyKey` 等技术字段。
- 随机来访生成异常记录改为“存档修复”提示；原始 action、目标引用和幂等键仍保留在内置样例 / QA 运行态的折叠技术详情里，便于存档护栏回看但不干扰普通游玩。
- 同类扫描后继续收敛共同庄园玩家页：权限审计、共同仓库流水和共同基金流水改为中文动作名，不再在主路径直接显示 `permissions_updated`、`deposit`、`spend` 等内部枚举。
- `qa:random-npc-save-guard` 已同步改为守护“玩家摘要 + 折叠技术详情”的展示边界。

### 0615 土地与温室肥料消耗修正
- 收获普通作物、多茬作物单次收获、巨型作物收获，以及虫害 / 杂草 / 缺水 / 雷击 / 乌鸦 / 换季枯萎导致作物被清掉时，已施肥料会随这一批作物消耗，不能留给下一批复用。
- 温室作物收获后同样消耗地块肥料；复收作物会继续生长，但下一轮若要加成需要重新施肥。
- `qa:fertilizer-retention` 调整为验证普通地块、复收作物、巨型作物与温室收获后的肥料不复用行为。

### 0614 矿洞弹窗位置与拖动
- 矿洞探索和战斗弹窗在电脑端默认下移，避免遮住顶部日期与时间；标题栏支持鼠标拖动调整位置，并会限制在当前窗口可见范围内。移动端保持原有滚动体验，避免误触拖拽。

### 0614 温室作物耗时对齐
- 温室一键收获改为沿用露天田地的批量收获计时：每 6 株结算一段批量收获耗时，并继续读取镰刀工具倍率，不再按每株成熟作物线性扣时间。
- 温室作物日结成长现在吃全局作物成长加速：钱包图腾、春季仙灵成长加成和戒指成长进度会与露天田地保持一致；成熟天数显示也同步读取同一套成长加成。
- `scripts/qa-tool-upgrade-guards.mjs` 补充温室批量收获计时守卫，防止回退到按单株收获时间累加。

### 0614 育种失败突破进度
- 作物杂交失败现在会按目标品系累计“谱系突破进度”，同一已知配方进度满后，下次同目标杂交可消耗失败积累补足甜度 / 产量门槛并完成突破；无配方乱配不会累计。
- 育种规划器、当前推荐目标和选种弹窗会显示突破进度与“已可突破”状态，失败回收摘要也会同步展示本次进度增量。
- `qa:breeding-preview-guards` 已扩展守护失败进度类型、存档字段、突破消耗逻辑和 UI 提示，防止回退到只有材料回收的失败保底。

### 0614 镖局押镖承接
- 商圈里的镖局新增“镖局承接”区，不再只卖武器；现在可每日限办押镖路签、护送前压包和驿路回执整理，分别承接商路票、行旅 / 战斗补给和古驿关券回流。
- 镖局承接区提供任务板、行旅图和瀚海快捷入口，并新增 `scripts/qa-biaoju-escort-services.mjs` 守护服务卡、每日限办、奖励和跳转锚点。

### 0614 目标声望阶位与周预算减免
- 目标声望现在会按 0 / 100 / 300 / 600 / 1000 / 1500 进入阶位，并在目标面板显示当前名望与下阶差距。
- 周预算投入会读取目标声望阶位，按 0%-10% 给出受上限控制的筹备减免；钱包周预算入口会显示实付、原价和声望减免来源。
- 新增 `scripts/qa-goal-reputation-utility.mjs`，守护阶位阈值、折扣上限、实付扣款、sink 统计与 UI 文案。

### 0614 鱼塘繁殖世代提示
- 鱼塘鱼列表现在直接显示每条鱼的世代；选择第一条亲本后，打开第二条鱼详情会显示配对预览，明确标出“可升代”或“保代滚基因”，避免同种繁殖被误解为每次都会提高世代。
- 繁殖空态和已选亲本提示补充“升代取决于品系配方”，并新增 `scripts/qa-fishpond-generation-visibility.mjs` 守护配方升代、非配方保代、世代评分 / 订单门槛和 UI 预览文案。

### 0614 村庄建设线索提示修复
- 村庄建设的待线索提示现在会直接说明真实来源：赵木匠 / 柳娘等委托线索会标出公告板委托与交付物，关系线索会标出需要达到好友或挚友，不再只写“从某某处获得”。
- 节庆暖房现在兼容柳娘好友阶段给出的旧温室线索 `liu_niang_clue`，达到好友后不必再误等随机委托；`qa-village-project-clue-guards` 已补充柳娘线索别名和各建设提示门槛守卫。

### 0614 骷髅矿穴深层产出曲线
- 骷髅矿穴普通层产出改为 50 层一档：1-50 层保持基础矿脉，51-100 层开始增加普通矿格和少量矿量 +1 概率，101 / 151 层后继续富集，201 层后进入封顶档，避免 120 层附近过早吃满收益。
- 深层矿池的铱矿权重改为跟随 50 层档位提升，五彩碎片权重概率改为受控封顶，不再随楼层无限增长；矿洞入口会显示当前可进入的深度档、矿脉加成、铱矿富集和矿量 +1 提示。
- 骷髅矿穴随机 Boss 新增深层压力倍率：25-100 层逐步拉满，泥岩巨兽、冰霜女王、熔岩君主、水晶之王和暗影君主在深层不再远低于深渊龙王，300 层时粗略强度约为龙王的 57% / 63% / 68% / 80% / 89%。
- 新增 `qa:skull-cavern-depth-loot`，守护 1、50、51、100、101、151、201 和 500 层的档位参数、铱矿权重、五彩碎片封顶，以及普通层真实格子生成的矿格数量和矿量 +1 接入。

### 0614 钓鱼经验按鱼获数量与品质结算
- 钓鱼成功经验从单条基础经验改为按鱼基础售价、难度、鱼获数量、品质、溪流田庄、完美评级与传奇称重共同结算；野生鱼饵触发双倍鱼获时会同步获得双倍经验，优良 / 优质 / 极品鱼按 1.25 / 1.5 / 2 倍经验结算，并新增共享公式 `getFishingCatchExperience()` 与 `qa:skill-mastery-effect-guards` 守卫。

### 0614 灵物封匣开匣反馈
- 灵物封匣开出「珍礼匣 / 战备匣」时，现在会直接告诉你实际拿到的物品和背包分类去向，例如樟脑香、丝帕会提示去背包礼物里找，铁锭、木炭会提示去背包材料里找。
- 钱袋页的密匣 / 遗箱预览也会提前列出每个奖励档的具体物品与入包位置，避免只看到奖励档名却不知道该去哪查收。
- 新增 `qa:mystery-box-feedback` 静态守卫，防止开匣反馈退回只显示「珍礼匣 / 战备匣」。

### 0614 票券命名统一
- 周预算和钱包摘要统一使用正式票券名称：商路票、展陈券、研究券与后勤票，避免玩家误以为存在独立的“商路票券 / 展馆票券 / 学舍票券 / 学社票 / 后勤券”背包物品；新增 `qa:reward-ticket-copy` 守护预算奖励仍指向原有票种且玩家文案不再混用旧名称。
...
- 混合精通新增水田匠师、工台匠师、险境行者、地下水脉师和桃源全才 5 条终局方向；高级工台权限改由工台匠师解锁，并实际限制仙灵炉制造，store 与加工页都会拦截未解锁状态。
- 稀有资源转化配方已落到炼丹炉隐藏丹方“灵脉转彩丹”：消耗灵脉碎晶、风蚀晶核、奇丹晶和铱矿，复用每日主丹限次与炼丹结果表，失败仍会消耗材料，避免变成无风险资源复制。
- 特殊地图标记能力已接入行旅图摘要和新手路线提示，会优先提示本周未完成事件、路线承接与风险信号；该能力只提供规划信息，不直接增加奖励。
- `qa:skill-mastery-effect-guards` 已覆盖 25 个精研 `effectKey`、5 条混合精通、每日祝福 / 护符位 / 高级工台 / 稀有转化 / 地图标记等功能性奖励、仙灵炉权限、隐藏转化丹方和行旅图标记；本轮验证还复跑了技能存档归一化、目标文件 ESLint、目标文件 `git diff --check`，`type-check` 仍被既有 `AchievementView.vue(847,142)` 类型错误阻塞。
...
- 主人查看自己的照料页时，不再把访客照料/轻采限制显示成失败原因；`qa:online-ui-structure` 补充权限入口和轻采权限选择框守卫。本轮验证：目标文件 ESLint、源码断言与 `git diff --check` 通过；`qa:online-ui-structure` 仍被既有 `FarmView.vue` 控件断言阻塞，`type-check` 仍被既有 `AchievementView.vue(847,142)` 类型错误阻塞。

### 0613 好友私聊未读提示修复
- 修复收到好友私聊通知后，实时刷新会在非私聊页沿用上次会话并自动标记已读，导致地图按钮、地图联机快捷入口和好友驿站看不到未读数的问题；现在只有停留在 `/game/chat` 时才会自动刷新当前消息流并清读。
- 好友驿站新增“私聊未读”统计，好友列表里的“私聊”按钮会对对应好友显示 `N 未读`，并与移动端地图入口复用同一份会话未读数。
- `qa:friend-lobby-guards` 补充私聊未读提示守卫，防止后续把聊天通知再次后台自动读掉。

### 0613 好友私聊聊天化
- 新增 `/game/chat` 好友私聊页，会话列表、气泡消息、时间分隔、未读数、发送失败提示和聊天礼物卡片统一承载好友文字、附图与送礼，好友驿站的“私聊 / 送礼”入口改为进入聊天页。
- 服务端新增私聊运行时和 `taoyuan_private_chat.json` 数据面，只允许互为好友且未拉黑的双方通信；聊天礼物通过隐藏的 mailbox delivery 复用既有扣物资、领取和幂等结算，但不再出现在邮箱列表或未读数里。
- WebSocket 复用 `notification.created` 的 `chat` 分类推送在线与离线补收通知；邮箱页保留旧玩家书信历史，并隐藏未来手动写信 / 邮件式送礼入口。
- 本轮验证：`npm --prefix server run qa:server-node-check`、`npm --prefix server run qa:private-chat-flow`、`npm --prefix taoyuan-main run qa:friend-lobby-guards`、私聊相关 changed-file ESLint 与 `node --check` 通过；`qa:online-ui-structure` 仍被既有 `FarmView.vue` 控件断言阻塞，`qa:realtime-smoke` 的私聊段通过后被既有节日摊位断言阻塞。
...
- 桌面端开放行旅视窗现在支持鼠标悬停滚轮缩放，复用既有缩放上下限与加减按钮逻辑，并在地图内阻止页面跟随滚动。
...
### 0602 同居共同仓库 11.4 林间补药 / 灵草秘药稀有料理
- 共同工坊配方下拉新增共同灶台林间补药和灵草秘药，可从共同仓库扣人参、野蘑菇、草药和蜂蜜，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v42，两类人参系后续料理成品进入稀有目录；服务端在消费人参前要求 `storage.withdraw_rare`，契约 QA 覆盖无权限阻断、授权后 consume / deposit 流水、稀有来源资产和个人 / 基金边界。

### 0602 同居共同仓库 11.4 雪莲 / 瀚海稀有料理
- 共同工坊配方下拉新增共同灶台雪梅羹、仙人掌汤和枣糕，可从共同仓库扣雪莲、蜂蜜、瀚海仙人掌、西域香料、瀚海椰枣和稻米，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v41，三类雪莲 / 瀚海料理成品进入稀有目录；服务端在消费雪莲、瀚海仙人掌或瀚海椰枣前要求 `storage.withdraw_rare`，契约 QA 覆盖无权限阻断、授权后 consume / deposit 流水、稀有来源资产和个人 / 基金边界。

### 0602 同居共同仓库 11.4 矿石系高价值料理
...
### 0602 同居共同仓库 11.4 鲟鱼 / 龙鱼高价值鱼获料理
- 共同工坊配方下拉新增共同灶台月下刺身、鲟鱼羹、龙鱼刺身和收藏家宴，可从共同仓库扣鲟鱼、龙鱼、生姜、草药、木柴、人参、南瓜和稻米，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v39，鲟鱼、龙鱼和四类高价值鱼获料理成品进入稀有目录；服务端在消费鲟鱼、龙鱼或人参前要求 `storage.withdraw_rare`，契约 QA 覆盖无权限阻断、授权后 consume / deposit 流水、稀有来源资产和个人 / 基金边界。

### 0601 同居共同仓库 11.4 人参系稀有料理
- 共同工坊配方下拉新增共同灶台人参汤、百草丹、长生汤、文曲星粥和鹿茸汤，可从共同仓库扣人参、草药、木柴、蜂蜜、稻米、茶叶和鹿茸，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v38，人参和五类人参系料理成品进入稀有目录；服务端在消费人参前要求 `storage.withdraw_rare`，契约 QA 覆盖无权限阻断、授权后 consume / deposit 流水、稀有来源资产和个人 / 基金边界。
...
- 丹炉运行槽新增 `consumedInputs` 记录真实投入材料，取消加工或拆机时退回替代后的实际材料；`qa:crop-use-entry-guard` 补运行态守护料理 / 炼丹替代计划。

### 0601 同居共同仓库 11.4 丝绸 / 香料普通料理补齐
- 共同工坊配方下拉新增共同灶台锦囊玉饺、孔雀宴和香料烤羊，可从共同仓库扣普通蚕丝、孔雀羽、西域香料、羊奶、稻米、白菜与桂花，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v36，蚕丝、西域香料和三类料理成品进入普通目录；服务端契约 QA 覆盖 consume / deposit 流水、协作升品来源资产和个人 / 基金边界，高价值瀚海仙人掌、雪莲、鲟鱼、龙鱼与虚空矿材料继续留给后续分级批次。

### 0601 20.1 分居冷却时间收口
- 分居最终剧情 / 家庭回执在执行 ledger 无剩余待办后会把契约关闭为 `closed`，读回 `closed_at`、`closed_by`、`separation_state=closed` 与 `separation_contract_closed` 审计，并用关闭时间刷新恋爱 / 婚姻 72 小时关系切换冷却。
- 已关闭契约上的最终回执幂等回放仍只读返回原回执，不重复关闭或改写个人资产；庄园退出交接记录的幂等读回也会保留 `manor_exit_handover_record`。

### 0601 同居共同仓库 11.4 面粉 / 干菜加工料理链
- 共同工坊配方下拉新增共同晒架干菜、共同石磨面粉、共同灶台馒头和共同灶台干菜米粉汤，可从共同仓库扣普通白菜 / 冬小麦加工为优质干菜 / 面粉，再继续作为料理材料追溯来源流水。
- 共同仓库策略版本升到 v35，面粉、干菜、馒头和干菜米粉汤进入普通目录；服务端契约 QA 覆盖加工产物入仓、料理产物入仓、来源 ledger 反链和个人 / 基金边界。

### 0601 雇工日结反馈增强
- 睡后摘要 / 记录中心新增“雇工帮忙”分区，集中展示雇佣 NPC 当天喂食、浇水、收获、除草除虫、没活可做和工资不足离职等日结结果。
- 普通日志仍沿用原有雇工消息，不额外刷屏；本轮只增强可见性，不改变雇工效率、工资、执行时机、饲料 / 背包条件或雨天自动浇水规则。

### 0601 同居玩家申诉与恢复
- 共同庄园权限面板新增成员级“恢复默认”入口，展示默认模板和偏离默认的权限数量；总览新增玩家申诉与恢复面板，可提交恢复申诉、查看安全版本并输入固定确认文案回滚契约。
- 分居预览新增资产争议来源清单，统一读回仓库来源 / 未知来源、高价值取出冻结、基金未知贡献、装修拆除、装修来源、建筑来源和关系剧情回看，展示目标、来源、草案 / ledger、金额 / 数量、冻结 / 待确认状态和后续动作。
- 新增 `/permissions/default-restore`、`/recovery-appeals` 和 `/safe-versions/rollback` 前端 API / store 动作；剧情演出播放提交固定确认文案，恢复与回滚均保持不自动改个人存档、不自动还仓。
- `qa:online-ui-structure` 和 `qa:cohabitation-recovery-20-3` 覆盖按钮、API / store 接线、幂等回放、非 owner 恢复拒绝、申诉读回、安全版本回滚和分居来源清单。

### 0601 同居共同仓库 11.4 节令 / 成就普通料理补齐
- 共同工坊配方下拉新增腊八粥、冬至饺、花生汤圆、百收粥、大厨特供和交际花茶，可从共同仓库扣普通稻米、花生、野果、冬小麦、冬白菜、蜂蜜、红枣、鸡蛋、芝麻和桂花，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v34，六类节令 / 成就料理成品进入普通目录；面粉、干菜、人参、雪莲、鲟鱼、龙鱼和虚空矿等未分级或高价值材料继续保留给后续分级批次。

### 0601 工具升级工作耗时与矿洞战斗节奏
- 工具升级新增统一工作耗时倍率：基础 / 铁制 / 精钢 / 铱金分别为 1 / 0.9 / 0.8 / 0.7，不新增存档字段；开垦、播种、批量播种、浇水、一键浇水、收获、一键收获、钓鱼、淘金、竹林采集和矿洞翻格等对应工具动作都会按等级缩短耗时。
- 钓鱼、淘金和竹林采集从旧的固定减分钟改为百分比减时，并继续叠加技能减时和最低行动时间规则；矿洞普通翻格吃镐倍率，炸弹、下楼和工具升级等待天数不受影响。
- 矿洞战斗基础耗时改为按实际战斗碾压程度计算，快速、优势、普通和长战斗分档；攻击、防御、逃跑以及 BOSS 战无法逃跑提示，只要产生战斗耗时，都会在基础耗时后叠加镐升级倍率，再交给 `advanceTime()` 叠加料理 / 丹药速度 buff。
- 工具升级页新增“工作耗时”词条，镰刀、鱼竿保留原有范围 / 时限属性并额外展示耗时变化；角色信息工具一览改为显示体力减免和耗时减免，指南同步说明工具升级会减少体力消耗并缩短对应动作耗时。

### 0601 同居共同仓库 11.4 山药 / 蒜香 / 苦瓜家常料理补齐
- 共同工坊配方下拉新增山药团圆粥、蒜香萝卜和苦瓜清暑汤，可从共同仓库扣普通山药、大蒜、苦瓜、萝卜、稻米和木柴，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v33，普通大蒜、苦瓜、三类家常料理成品进入普通目录；人参、雪莲、鲟鱼、龙鱼和虚空矿等高价值材料继续保留给后续分级批次。

### 0601 同居共同仓库 11.4 野蘑菇 / 松果普通料理补齐
- 共同工坊配方下拉新增蘑菇炖和猎人烤，可从共同仓库扣普通野蘑菇、松果、草药和木柴，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v32，普通野蘑菇、松果、蘑菇炖和猎人烤进入普通目录；人参、暗影矿和虚空矿相关料理继续保留给后续稀材分级批次。
...
### 0601 同居共同仓库 11.4 钓鱼技能普通料理补齐
- 共同工坊配方下拉新增红烧鲶鱼，可从共同仓库扣普通鲶鱼和辣椒，并把钓鱼技能料理产物回存共同仓库。
- 共同仓库策略版本升到 v31，普通鲶鱼和红烧鲶鱼进入普通目录；鲟鱼和龙鱼料理继续保留给后续高价值鱼获分级批次。

### 0601 共同资产跨账号 / 设备防搬空
- 共同仓库与共同基金快照新增跨账号 / 设备治理读回：`governance.suspicious_networks`、网络动作计数和阻断原因会暴露同 IP/设备多成员短窗风险。
- 共同仓库普通入仓、普通取出 / 卖出、高价值取出草案，以及共同基金小 / 中额预算支出和商店采购会在后端命中跨账号 / 设备风险时阻断并写审计；现有前端读取治理摘要即可展示阻断状态。

### 0601 共同仓库治理申诉入口
- 新增 `/warehouse/governance/appeals` record-only 写链；被入仓 / 出仓高频或同 IP/设备网络簇阻断的成员可提交申诉，申诉记录阻断原因、ledger、网络风险与审计证据，不自动恢复共同仓库或个人背包。
- 共同仓库治理快照新增 `recent_appeals`、`pending_appeal` 和 `last_appeal`，管理恢复会把匹配申诉标记为 `recovery_applied` 并在恢复审计里反链申诉 ID。
- 共同仓库高频治理面板新增玩家申诉表单与 pending 读回，保留 owner / 高价值仓库权限成员的直接恢复入口；`qa:cohabitation-contract` 和 `qa:online-ui-structure` 固定非 owner 申诉、幂等和资产不变边界。

### 0601 同居共同仓库 11.4 采集 / 战斗技能普通料理批次
- 共同工坊配方下拉新增野菜沙拉、战士口粮、战斗炖菜和铁拳汤，可从共同仓库扣普通草药、野果、土豆、鸡蛋、辣椒、姜、铁矿石和木柴，并把采集 / 战斗技能料理产物回存共同仓库。
- 共同仓库策略版本升到 v30，四类采集 / 战斗技能料理成品进入普通目录；野蘑菇、人参、暗影矿和虚空矿相关料理继续保留给后续分级批次。

### 0601 同居共同仓库 11.4 采矿技能普通料理批次
- 共同工坊配方下拉新增矿石汤和铁骨汤，可从共同仓库扣普通铜矿石、铁矿石、萝卜、草药和木柴，并把采矿技能料理产物回存共同仓库。
- 共同仓库策略版本升到 v29，两类采矿技能料理成品进入普通目录；水晶矿、金矿和虚空矿相关料理继续保留给后续分级批次。


### 0601 同居关系切换冷却
- 恋爱 / 婚姻同居契约创建和接受会阻断成员已有其他进行中恋爱关系；已关闭关系会按 72 小时 `relationship_switch_cooldown_until` 冷却阻断新关系，错误码区分进行中关系与冷却中关系。
- 分居个人剧情回执会写入关系切换冷却字段和共同审计证据；前端契约类型补齐冷却结束时间、冷却原因、来源契约和关闭信息，`qa:cohabitation-contract` 覆盖创建、接受、关闭冷却和分居回执路径。

### 0601 随机 NPC 日常成长线收口
- 新增 `RANDOM_NPC_RELATIONSHIP_GROWTH_BEATS`，统一描述日常熟面、熟人长住、暧昧邀约、长住恋爱和家人线五段成长门槛，读取好感、关系方向信号和见家人状态生成进度文案。
- NPC 页短访 / 熟人 / 长住卡片新增“自然成长 / 日常长出关系线”回看，`qa:random-npc-content-guard` 与 `qa:random-npc-save-guard` 固定内容入口、家人线门槛和存档上限。
...
- 深关系长住在无婚约 / 成婚、112 天低活跃且旧档锁定名额可用时，会转为锁定旧日长住摘要并保留轻量长住快照；锁定位不足时继续留在长住名册，避免被旧档池裁剪。

### 0601 炼丹与料理内容表收尾
- 新增山药团圆粥、蒜香萝卜、苦瓜清暑汤三道低幅 buff + 剧情触发料理，灶台默认可见，并继续写入最近料理线索供 NPC、宠物、节会、行旅和订单消费。
- 新增固元山药丹、蒜辛驱寒丹、苦瓜清暑丹三枚辅丹，均保留主材 / 辅材 / 引子 / 火候、成丹 / 偏丹 / 废丹 / 奇丹结果和短效经营定位；百科 / 搜索会对山药、蒜、苦瓜展示料理价值与炼丹价值双路径。
- 新增 `qa-alchemy-cooking-content-table` 运行态守护料理剧情消费入口、作物双路径、丹药物品表和四类炼丹结果，支撑 19.2 验收收口。

### 0601 同居共同仓库 11.4 农耕技能普通料理批次
- 共同工坊配方下拉新增南瓜饼、黄金炒饭和田园盛筵，可从共同仓库扣普通南瓜、稻米、鸡蛋、玉米和西瓜，并把农耕技能料理产物回存共同仓库。
- 共同仓库策略版本升到 v28，普通西瓜和三类农耕技能料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。
...
### 0601 同居共同仓库 11.4 结婚普通料理批次
- 共同工坊配方下拉新增凤凰糕、熔岩铁锅、茶宴八珍、铁匠炖、牧场鲜奶汤和月下茶泡饭，可从共同仓库扣普通材料并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v27，六类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 挚友普通料理批次
- 共同工坊配方下拉新增农家盛宴、秋月宴、鸳鸯酥、锻造师套餐和灵果酒，可从共同仓库扣普通材料并把料理 / 酒坊产物回存共同仓库。
- 共同仓库策略版本升到 v26，五类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 相知普通料理批次
- 共同工坊配方下拉新增老萝卜炖肉、绣囊糕、深矿炖菜和野果酱，可从共同仓库扣普通材料并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v25，野果和四类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 家常与相识料理批次
- 共同工坊配方下拉新增柿饼、鱼汤面、矿工铁锅饭、炒土豆丝、芝麻糊、韭菜炒蛋、花生糖和桂花甜茶，可从共同仓库扣普通材料并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v24，柿子、韭菜、花生和八类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 物品分级策略 v23
- 雪莲、瀚海仙人掌和瀚海椰枣进入显式稀有目录；普通入仓按 `rare_item_policy` 阻断，后续走稀有高价值草案。
- 基础山药补入普通目录；莲子、茶叶和莲藕继续保持普通目录，避免把作物用途里的 `valuable` 直接等同为仓库稀有。

### 0601 同居共同仓库 11.4 物品分级策略 v22
- 二代 / 三代遗留高价值杂交作物进入显式稀有目录，包括桃花茶、莲心茶、蜜桃瓜、晶山药、桂花茶、雪茶、双莲、山莲、桂莲、冰火椒、秋桂瓜、冬南瓜、雪菊、雪南瓜、风瓜、云豆、雨稻、霜薯、雷菜、虹果、晨茶、暮笋、翠玉茶、寒香菊和珠桃。
- 共同仓库普通入仓会按 `rare_item_policy` 阻断，策略快照读回 v22 和稀有高价值草案许可；基础莲子、茶叶、莲藕等普通物不随本批次升档。

### 0601 同居共同仓库 11.4 物品分级策略 v21
- 风华 / 玉系三代二十类高价值杂交作物进入显式稀有目录；共同仓库普通放入、取出和卖出会按 `rare_item_policy` 阻断，后续只能走稀有高价值草案。
- 仓库策略快照读回 v21、目录包含和高价值草案许可；本批次只扩展高价值杂交作物，不把基础普通作物误升为稀有。

### 0601 同居 11.6 离线经营第一版收口
- TODO 将“任一成员上线可经营”“所有操作写入流水”“冲突以服务端状态为准”按第一版关闭；当前已接入的离线经营入口继续读回服务端状态、幂等键、审计 / 业务 ledger 和分域冲突证据。
- 前端已具备离线经营摘要、本地队列缓存、批量预检 / 自动解决、离线自动收益领取、共同日结以及农田 / 动物 / 宠物 / 工坊 / 装饰 / 高风险回执 / 基金购买 / 仓库卖出入口；后续新增经营动作继续按同一服务端写链扩展。

### 0601 同居共同仓库 11.4 高阶杂交稀材丹方批次
- 共同工坊配方下拉新增雪莲清心丹、露花凝神丹、星莲安神丹及各自偏丹 / 废丹 / 奇丹分支，可从共同仓库扣雪莲子、露花、星莲和优质炼丹材料，并把丹炉结果回存共同仓库。
- 共同仓库策略版本升到 v20，三类高阶杂交稀材输入和三类新丹药输出进入稀有目录；处理回执继续读回 consume / deposit 流水、协作成功率、高价值 / 稀有保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 高阶动物产物料理批次
- 共同工坊配方下拉新增鸽蛋药草汤、牦牛奶麦粥、驴奶姜茶、鸵鸟蛋宴和鹿茸药草汤，可从共同仓库扣高阶动物产物、草药、冬小麦、蜂蜜、茶叶和精加工芝麻油，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v19，五类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 节令点心料理批次
- 共同工坊配方下拉新增年糕、花糕、青团、月饼、粽子、巧果、斗茶饮和纸鸢糕，可从共同仓库扣普通农产、蜂蜜、茶叶和精加工芝麻油，并把节令点心 / 饮品回存共同仓库。
- 共同仓库策略版本升到 v18，八类节令料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。
### 0601 同居分居个人关系主状态前端读回
- 分居预览执行区新增只读“个人关系主状态写回”证据面板，读取 `personal_relationship_mutation_summary` 展示 receipt 数量、写回适配器、影响 NPC 和清理动作。
- 前端 API 类型补齐关系主状态写回摘要，`qa:online-ui-structure` 固定读回面板、summary 字段和影响 NPC / 清理动作证据；本轮只读展示，不新增个人存档写链。

### 0601 同居共同仓库 11.4 日常农家料理批次
- 共同工坊配方下拉新增麻辣火锅、冬笋炒肉、莲子羹、玉米烙和桂花藕粉，可从共同仓库扣普通农产、精加工芝麻油与燃料，并把料理产物回存共同仓库。
- 共同仓库策略版本升到 v17，五类料理成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0601 同居共同仓库 11.4 普通点心料理批次
- 共同工坊配方下拉新增桂花糕、红枣糕和桃花饼，可从共同仓库扣桂花 / 红枣 / 桃子与稻米，并把点心产物回存共同仓库。
- 共同仓库策略版本升到 v16，红枣和三类点心成品进入普通目录；处理回执继续读回 consume / deposit 流水、协作升品保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 自动产出分级证据
- 仓库最近流水会展示服务端返回的分级证据：普通 / 高品质保护 / 稀有保护、策略分类和策略版本，并提示是否需要高价值取出确认。
- 前端 API 类型补齐仓库 ledger 的 `withdrawal_risk_level`、`high_value_withdrawal_required` 与 `item_policy` 字段，方便共同农田升品、协作料理 / 加工、共同炼丹自动结果和高品质卖出回执统一读回保护边界。

### 0531 同居分居个人关系主状态写回
- 分居个人剧情回执现在会在恋人 / 婚姻 / 知己拆分后清理个人存档里可识别的 `dating / married / zhiji`、婚龄 / 知己天数和待婚 / 孕育 / 待孩子轻量字段。
- 预览执行请求、执行 ledger、个人剧情 receipt 和审计会读回 `personal_relationship_mutation_summary`；孩子、铜币、背包、农田、房屋和家庭资产保持不改写，结拜 / 合伙庄园仍只记录契约级交接证据。

### 0531 同居分居家族庄园退出交接记录
- 分居剧情拆分后端会为结拜 / 合伙庄园生成 `manor_exit_handover_record`，读回成员职位、交接状态、家主、资产域收口状态和 `family_role_handover_executed`。
- 该交接记录只写共同契约 ledger / 预览 / 审计，不改个人存档，也不静默重排契约职位；庄园退出 UI、个人 / 家族主状态迁移和美术级动画资源仍留后续。

### 0531 同居共同仓库 11.4 稀莲 / 翠桃稀有丹方
- 共同工坊配方下拉新增稀莲护心丹、翠桃醒神丹及各自偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质草药膏 / 莲心粉 / 蜜桃脯 / 桂花蜜和稀有莲子 / 翠桃，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v15，稀莲护心丹和翠桃醒神丹输出进入稀有保护目录；前端可预览稀莲 / 翠桃稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 玉兰稀有丹方
- 共同工坊配方下拉新增玉兰凝心丹及偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质桂花蜜、优质莲心粉和稀有玉兰，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v14，玉兰凝心丹输出进入稀有保护目录；前端可预览玉兰稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 月珠稀有丹方
- 共同工坊配方下拉新增月珠安神丹及偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质绿茶、优质莲心粉和稀有月珠，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v13，月珠安神丹输出进入稀有保护目录；前端可预览月珠稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 泽地稀有丹方
- 共同工坊配方下拉新增泽光净息丹及偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质草药膏、稀有湿地孢样和夜光藻团，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v12，泽光净息丹输出进入稀有保护目录；前端可预览泽光稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 风蚀稀有丹方
- 共同工坊配方下拉新增风蚀护脉丸及偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质腌姜、优质精制石英和稀有风蚀晶核，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v11，风蚀护脉丸输出进入稀有保护目录；前端可预览风蚀稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居共同仓库 11.4 灵脉稀有丹方
- 共同工坊配方下拉新增灵脉凝神丹及偏丹 / 废丹 / 奇丹分支，可从共同仓库扣优质绿茶、优质精制石英和稀有灵脉碎晶，并把丹药或结果产物回存共同仓库。
- 共同仓库策略版本升到 v10，灵脉凝神丹输出进入稀有保护目录；前端可预览灵脉稀材自动炼丹权重，处理回执继续读回 consume / deposit 流水、稀有保护和个人 / 共同基金不变边界。

### 0531 同居离线共同庄园日结队列
- 离线队列新增 `settle_shared_daily` 入口，可把共同庄园日结加入本地离线缓存，并参与批量预检 / 自动合并。
- 合并结果会展示农田成长、成熟、健康 / 心情加成消耗、共同地图 / 动物变更，以及个人存档、共同仓库、共同基金不变边界。

### 0531 同居分居关系剧情轻量演出时间线
- 分居剧情面板会把 `dialogue_lines`、`animation_cues` 和场景调度组装成前端播放步骤，恋人告别、婚姻家庭、知己约定和家族交接都能先播放轻量时间线。
- “播放演出”完成本地时间线后再写 `/record-story-cinematic` 回执，继续保持个人 NPC、家庭、孩子与资产主状态不变。

### 0531 同居共同仓库 11.4 物品分级策略 v9
- 仓库 `item_policy` 升级到 v9，稀有目录新增房间 / 区域稀有材料和稀有物采购目标，任务保护目录新增运单、拓片、活动房间回执与钱包成就印记。
- 前端仓库策略卡会随快照读回新的普通 / 稀有 / 任务保护计数；普通出入仓继续阻断稀有物和任务凭证，任务凭证仍禁止高价值草案。

### 0531 同居分居共同基金未知经营贡献人工分配
- 分居基金争议确认现在可随首次确认提交未知经营贡献人工分配；后端锁定分配 hash 后重算返还权重，后续成员确认复用同一方案，冲突方案会被拒绝。
- 共同庄园分居面板新增按成员输入的人工分配区，合计未等于未知经营贡献争议额时禁用确认；返还读回会展示人工分配金额、hash 和确认状态。

### 0531 同居共同仓库 11.4 季节节会料理
- 共同工坊配方下拉新增春卷、荷灯糕、丰收盛宴和年夜饺；共同仓库物品策略版本升到 v8，并把冬小麦、冬白菜与这些节会料理成品列入普通目录。
- 处理回执继续展示共同仓库扣料、料理入仓、协作升品来源资产保护，以及个人存档 / 共同基金不变边界。

### 0531 同居分居共同基金争议确认前端收口
- 分居面板的共同基金拆分读回会展示未知经营贡献总额、组数、稳定 hash 和来源流水；“确认争议”入口同时覆盖消费差额与未知经营贡献争议。
- 前端仅在后端进入确认阶段且任一争议存在时放行提交，确认仍只写审计，不改个人铜币、共同基金或共同仓库。
...
### 0531 同居离线家庭重大事件回执
- 离线队列新增 `record_family_major_event_receipt` 与 `record_family_major_event_refund_receipt` 前端入口，可把已扣款且待回执的家庭重大事件交付或退款草案加入本地离线缓存。
- 批量预检 / 自动合并会按服务端高风险回执写链提交 delivered / refunded 回执，交付结果读回 `family_state.major_event_ledger` 证据，退款结果读回共同基金退款流水，两条路径都展示个人家庭主状态 / 共同仓库 / 共同装饰状态不变边界。
...
- NPC 页会标注“长住旧档”，召回时恢复到长住名册并受长住 3 人上限约束；深关系长住在后续收尾中改为 112 天低活跃且锁定位可用时锁定归档。
