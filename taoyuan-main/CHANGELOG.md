...
### 0623 装备配件系统
- 铁匠铺新增“配件”入口，首版支持兵刃、护具、采具 3 条线共 9 个槽位；配件有一至四阶、普通/精良/卓越/极品品质和 1-20 级调校等级，三件同线会按最低阶触发套装摘要。
- 配件可通过工坊定向打造、矿洞掉落、深层矿洞稀有掉落和每日限购材料推进；四阶制作需要高级蓝图，孙铁匠与阿石可分别提供二阶打造和矿洞提示相关联动。
- 配件升级消耗配件材料、调校石、矿物和铜钱，1->20 总成本按一个游戏年约养成一件满级配件设计；拆解会返还升级投入材料和少量基础返材，但不返还铜钱。
- 护具配件的生命上限会直接计入玩家最大生命，卸下后会自动校正当前生命；四阶配件在高级蓝图解锁后制作，并在来源上标记为蓝图制作。
- 同名同阶同品质 3 件配件可合成升品质，成功产物固定为 Lv.1，不继承平均等级；成功、普通失败和保护失败都会返还被消耗配件的升级投入，使用稳固石失败时会保住 1 件材料配件并累计保底。
- 新增 `qa:equipment-accessory-guards` 并补注册 `qa:durability-guards`，覆盖配件数据、旧档归一化、制作/掉落、升级/拆解、三合一、保护失败、套装摘要、铁匠铺前端和装备/耐久兼容路径。

### 0623 装备耐久与坚固值
- 装备寿命扩展为“耐久 + 坚固”双层规则，武器、戒指、帽子、鞋子都会保存坚固值；旧档缺失坚固值时按最大坚固值补满。
- 修理台新增精修、简修、翻新和拆解：精修材料/铜钱更贵但坚固损耗低，简修只花铜钱但坚固损耗高，翻新可恢复满耐久并补回部分坚固，失固装备可选择拆解返材。
- 背包装备列表、装备详情、已装备槽位和修理台都显示耐久与坚固；修理台禁用原因改为中文提示，并默认选中可用修理方式，避免满耐久装备显示内部状态码。
- 修理和铸魔入口迁入铁匠铺服务区，不再制造或占用工坊机器格；旧档中的修理台任务会迁移为铁匠铺修理队列，修理面板改为装备列表、双条状态、方式预览和领取队列。
- 扩展 `qa-durability-guards`，守住坚固旧档兼容、修理模式费用关系、坚固损耗、失固拆解、修理台文案和默认选中可用项。

### 0623 育种果酒十阶分档
- 隐藏酒坊的育种果酒改为按 T1-T10 固定映射到灵果清酿 / 玄果清酿 / 天成果酿及普通 / 优良 / 精品 / 极品品质，T7 不再直接封顶后压平 T8-T10 价值。
- 固定世代酒品会保留产物品质，出售、出货箱、酒窖和库存堆叠继续走现有品质倍率；远古果酒仍保持专属出口。
- 扩展 `qa-hidden-processing-guards` 守住十阶映射、远古果酒和隐藏酒坊收益底线。

### 0623 育种谱系突破保底收紧
- 谱系突破进度阈值从 100 提高到 180，单次失败进度增量从 8-38 下调到 5-24，并降低近门槛、世代和研究加成，让失败保底从少数几次触发改为更长期的兜底。
- 旧档中已保存的谱系突破进度会按新阈值归一化，不会继续沿用旧的 100 点快速突破；`qa:breeding-preview-guards` 增加保底数值和旧档归一化守卫。

### 0623 商圈出售列表卡顿修复
- 商圈出售物品列表改为虚拟滚动，只渲染当前视口附近的物品行；长背包滑动时不再把全部可售物品、图标和行情徽标一次性挂到 DOM。
- 出售行的单价与行情徽标在列表数据阶段预先计算，滚动过程不再反复调用售价/行情查询；`qa:shop-buyback-guards` 增加虚拟列表回归检查。

### 0622 行旅图入口卡顿修复
- 开放行旅图 100x100 地图改为虚拟生成格子，只保留地标常驻数据，进入页面时不再一次性构建 4 张 10000 格地图和全量索引。
- 行旅图旧页签改为按需挂载，拖拽视窗合并到每帧派发，并恢复玩家标记的短线性移动动画，减少进入、移动、点击时的主线程卡顿。
- 地图路线页签改为预组装区域/路线视图模型，并在首次访问后保留挂载、分帧补齐路线卡与探索树，避免每次渲染重复计算路线预览、决策摘要和探索树，减少点击页签后拖慢上方开放地图。
- `qa:region-open-world-guards` 增加虚拟格子和禁止全量索引回归检查，键盘冒烟脚本同步改用懒查格子。

### 0622 睡后摘要工坊卡片
- 睡后摘要新增“工坊与加工”卡片，会汇总工坊自动收取、加工完成、隐藏配方发现、种子制造机额外产出、工坊手记返还和工坊精研双倍产出日志，避免加工产物只留在日志里看不见。

### 0622 联动最终验收护栏
- 新增 `qa:linkage-final-acceptance`，把联动计划最终验收标准固化成一键检查：重点物品、加工品大类、NPC 效果、采石场下游、家庭心愿、线上订单、物品卡用途和结算防重都会被集中验证。
- 当前验收结果覆盖 9 个高优先物品、16/16 个加工品大类、41 个 NPC effectType 和 5 条采石场下游联动，避免后续只看 TODO 勾选而忽略真实证据。
- 补齐 `qa:pet-feed-consumption` 和 `qa:online-order-consumption`，让计划 QA 清单中的宠物点心真实扣物、线上订单周锁扣物也具备独立脚本入口。

### 0622 联动旧档兼容护栏
- 新增 `qa:linkage-save-compatibility`，集中守住联动物品矩阵、家庭心愿、博物馆展组、物种笔记、采石场旧支道和 NPC 功能效果的旧档兼容边界。
- 家庭心愿材料需求继续由静态心愿定义和共享需求池重建，旧档只需要保存心愿板进度；`itemLinkage` 保持静态数据，不写入玩家存档。
- 博物馆展组、物种笔记和采石场联动状态会从现有存档归一化或派生；服务端存档异常检测已验证不会把这些新增联动容器误判为非法字段。

### 0622 联动经济护栏
- 新增 `qa:linkage-economy-guards`，集中检查线上弱用途订单、家庭心愿、博物馆展组、瀚海出行准备、采石场丹药准备和订单结算的防套利边界。
- 线上弱用途订单被锁定为周轮换、小额票券、真实扣料且不发直接铜钱；家庭心愿和博物馆展组继续守住一次性完成/领奖，不会变成重复刷票券或刷钱入口。
- 瀚海准备物和采石场丹药准备新增奖励倍率、额外票券、真实消耗和不直接发钱的 QA 红线；特殊订单评分和结算回执继续受倍率上限、回滚和去重检查保护。

### 0622 日志和结算反馈统一
- 新增 `qa:linkage-settlement-log-feedback`，守住家庭心愿、订单、博物馆展组、行旅图公共交付、瀚海出行准备、采石场丹药准备和线上订单的成功/失败日志闭环。
- 加工品订单和 NPC 功能高级订单结算会记录本次真实提交的物品与数量；订单失败或重复提交仍走原结算锁/回滚，不会写重复成功日志。
- 博物馆展组、行旅图公共交付、瀚海遗迹勘探和采石场旧支道准备补齐系统标签失败日志；采石场丹药消耗成功由 store 统一写全局日志，页面不再重复写同一成功记录。

### 0622 公会目标生活联动
- 长期目标新增家庭心愿、加工品订单、博物馆专题展组、物种笔记和 NPC 功能高级订单目标，并复用既有采石场周管护目标，覆盖生活系统而不只看战斗/采集。
- 目标进度接真实事件源：家庭心愿完成、加工品订单提交、采石场管护领奖、展组完成、物种笔记完成、NPC/服务承接特殊订单都会进入目标或本周活动统计。
- 本周活动新增“生活联动周”，公会经营联动面板会显示生活目标和周活动承接状态；新增 `qa:goal-guild-life-linkage` 守住指标、目标、周活动、事件计数、奖励克制和公会入口。

### 0622 区域地图需求池交付
- 区域资源整备接入共享需求池，古驿残卷、生态样本、灵脉结晶的公共交付会分别消耗驿路关券、夜光藻团和灵脉碎晶。
- `recordResourceTurnIn` 不再只是记账：交付会同时扣区域资源账本和背包/临时背包物品，失败时回滚账本与背包快照，避免半成功吞材料。
- 资源整备面板新增交付需求、库存、来源提示和影响说明；新增 `qa:region-demand-pool-sinks` 守住需求池、扣物路径、UI 与 package 命令。

### 0622 瀚海出行准备消耗
- 瀚海遗迹新增可选出行准备物，行旅干粮包、茶酒腌品箱、丹药测绘箱和深脉定砂锚会分别消耗冒险口粮/草药膏、茶饮/腌制品、高阶丹药、黑曜石/龙玉。
- 选择准备物后，遗迹勘探会真实扣料，并按配置降低探索费用、提高素材回收或追加票券；不带准备物仍可正常出发，低库存玩家不会被单一耗材卡死。
- 瀚海页新增出行准备面板，实时显示库存缺口、成功率补正、风险降低和预计收获变化；新增 `qa:hanhai-travel-prep-sinks` 守住数据、store、UI 和非必备兜底。

### 0622 物种笔记联动
- 新增 `speciesNotes.ts` 轻量物种笔记数据，把育种谱系、鱼塘品系、区域生态、采石遗物和博物馆专题展示汇总为长期图鉴进度。
- 鱼塘图鉴页新增物种笔记进度，博物馆经营总览新增跨系统物种笔记面板，能同时读回育种、鱼塘、行旅图、采石场遗物捐赠和专题展组完成情况。
- 物种笔记只读取既有发现/捐赠/展组状态，不扣鱼、不吞育种奖励、不改一次性捐赠；新增 `qa:species-note-linkage` 守住数据、store、UI 和不消费边界。

### 0622 装饰评分需求风向
- 装饰系统新增“需求风向”规则，庭前待客、水畔游赏、书香馆藏三类主题会根据总美观度、主题美观度和已放置装饰，为对应家庭心愿提供有限候选权重。
- 家庭心愿的自动周安排和手动安排会读取装饰风向排序；该效果只影响未完成候选优先级，不绕过伴侣/知己、关系阶段、每周心愿上限，也不直接发放铜钱、票券或物品。
- 装饰页新增需求风向面板，小屋页会显示当前/下一条心愿受到的装饰来源；扩展 `qa:decoration-linkage-effects` 守住规则、store 输出、NPC 排序和 UI 读回。

### 0622 博物馆专题展组
- 博物馆新增 4 组专题展组：乡土风味展、深脉采石展、鱼塘物种展、灵物供奉展，把加工副本、采石场副本遗物、鱼塘样本、共同丹炉高阶丹药和供奉物转成长期展示需求。
- 展组提交会真实扣除背包/临时背包材料并保存进度；标记为副本的藏品必须先完成首件捐赠，避免破坏一次性 `donatedItems` 捐赠规则。
- 完成展组会沉淀为展示评分或访客热度等经营收益，博物馆页新增专题展组面板展示材料来源、背包数量、提交进度和奖励确认；新增 `qa:museum-exhibit-set-guards` 守住数据、存档、UI 和扣料边界。

### 0622 采石场扩建门槛修复
- 采石场 10x10 之后的扩建不再要求主矿洞达到 130 层以上；主矿洞门槛统一封顶为实际最大 120 层，后续深度推进继续读取骷髅矿穴层数。
- 扩展采石场刷新/扩建守卫，防止后续数据再次写出超过主矿洞上限的不可达门槛。

### 0622 共同丹炉高阶丹药回流
- 共同庄园的共同工坊新增高阶丹药“单人回流”读回，选中灵脉凝神丹、风蚀护脉丸、泽光净息丹等 7 个共同丹炉产物时，会显示取出到单人背包后的可用去向。
- 共同工坊执行结果会回读单人用途；这些丹药已接入背包服用、当天丹药槽、远征、采石场、宠物安抚、社交和节会等消费链路。
- 新增 `qa:elite-elixir-use-guards`，并扩展在线 UI 结构守卫，锁住共同丹炉产物、背包扣料、效果消费端和回流提示。

### 0622 公共仓与家庭心愿共享需求池
- 联动需求池新增家庭心愿映射、反重复标签和来源提示，`family_breakfast`、`public_storage`、`festival_prep` 等标签现在同时服务家庭心愿和线上公共仓备料单。
- 家庭心愿需求规范化会读取共享需求池，保留需求 ID、标签和来源提示；在线委托页读取当前家庭心愿后，会避开同周家庭正在消耗的同类物资，并显示反重复提示。
- 扩展 `qa:family-wish-consumption`、`qa:online-order-weak-item-sinks` 和 `qa:item-linkage-matrix`，守住共享池查询、公共仓复用和家庭/线上同周反重复。

### 0622 线上订单弱用途物品池
- 在线委托页新增弱用途物品周轮换备料单，覆盖庄园边角作物包、杂籽油、米粉、田园干货包、什锦果干、鱼饲料和普通鱼饵。
- 订单会显示当前库存、所需数量、票券奖励、本周轮换原因和下周预告；提交时真实扣除未锁定背包物品，库存不足或本周已交不会吞材料或重复领奖。
- 轮换池复用统一需求池的 `onlineOrder + weak_item_sink` 条目，并新增 `qa:online-order-weak-item-sinks` 守住轮换、扣料、周锁、票券反馈和物品卡用途。

### 0622 管理端日志中心完善
- 日志中心升级为统一检索入口，新增全部、管理审计、在线审计、内容审核、游戏日志、内容发布标签，并展示各日志源总数、最近写入、留存策略和重大证据保留说明。
- 游戏长期日志默认容量调整为 30 天留存、最多 500000 条、单用户单存档最多 12000 条；后台列表返回真实 total、分页和 retention 元数据。
- 统一筛选支持时间范围、用户名、动作、结果、分类/场景和关键词，并补接在线审计、内容审核事件/风险信号、内容发布记录和公告发布审计。
- 扩展 `qa:admin-audit-retention` 与新增 `qa:admin-log-center-structure`，守住日志容量默认值、统一筛选、分页总数、空态和权限态入口。

### 0622 家务分工真实效果
- 婚后/知己家务分工升级为可结算的低强度家庭协作效果：宅院照料会减免家庭心愿材料并推进心愿，田务分工会稳定动物心情，家业协作会小幅加速工坊，人情往来每周带回家人情分券。
- 小屋页新增“家务分工影响”面板，家庭心愿材料行会显示家务分工带来的原需求到实际需求变化；NPC 陪伴总览同步展示当前激活分工效果。
- 孩子训练可读取宅院照料的训练收益加成；新增 `qa:household-role-effects` 守住分工效果表、周结、家庭心愿扣料、动物/工坊日结和 UI 入口。

### 0622 装备耐久显示与修理费用优化
- 背包装备页的武器、戒指、帽子、鞋子会统一从装备实例计算并显示耐久，旧档满耐久装备也不再因为缺少 `durability` 字段而隐藏耐久条。
- 装备详情弹窗和已装备槽位摘要补充耐久数字与耐久条，修理台列表同步显示缺失耐久和损耗百分比。
- 修理台材料与铜钱消耗改为按缺失耐久比例计算，`90/100` 的轻度损耗不会再和 `0/100` 的完全破损收取同样费用；开工时会重新读取当前耐久再扣费。

### 0622 家庭心愿真实需求扩容
- 现有 7 条家庭心愿全部补齐真实物品需求，除了杂籽油早餐和集市共宴外，湖畔相伴、家业传承、馆藏赞助、月下看塘和灵缘札记也会消耗庄园边角菜包、鱼饵、纸张、木炭、布匹、鱼饲料和松香等材料。
- 小屋家庭心愿面板会展示材料库存、品质和来源提示，下一条待安排心愿也会预览需要准备的材料；自动周结只推进进度，材料不足时保留心愿并写出具体缺口，不会绕过背包消耗领奖。
- `manor_edge_bundle` 联动矩阵从计划用途升级为当前家庭心愿消耗口，统一需求池新增家庭出游、家庭档案和鱼塘照料条目；扩展 `qa:family-wish-consumption` 覆盖全量心愿、扣料顺序、失败回滚和 UI 反馈。

### 0622 孩子训练真实资源消耗
- 小屋孩子互动升级为训练结算，学识、体魄/社交、自然、手作四类训练会分别消耗纸张、饭团/冒险口粮、鱼饲料/白菜种子、布匹/木材等真实物资。
- 训练会先检查并扣除背包/临时背包材料，扣料成功后才记录今日已训练、本周课程数、训练方向和好感；材料不足或扣料失败时不会增加课程次数，也不会吞物品。
- 小屋孩子卡片新增训练方向、周课程、材料库存和来源提示；物品联动矩阵新增 `childTraining` 系统和 7 条训练需求池，并新增 `qa:child-training-consumption` 守住扣料、UI 与联动登记。

### 0622 联机入口与邀请提示优化
- 在线中心继续强化“联机活动中心”结构，集中展示今日小游戏、房间状态、待处理邀请、本周奖励进度、好友动向和小游戏种类目录。
- 在线中心新增首屏“今日三局”优先行动区，从今日推荐里固定收束 3 个可推进入口，并直接展示行动顺序、推荐原因、耗时、奖励、协作方式和下一步按钮。
- 在线中心三套主题舞台补充“当前焦点”读回，节会、远征、庄园入口会直接展示今日主行动、下一步、好友动向和奖励落点，减少普通信息卡堆叠感。
- 在线中心新增“周末大型活动”看板，把全服节庆目标、限时远征首领、村社建设赛和主题排行榜从小游戏目录提升为可行动入口，并直接展示本周目标、限定奖励、组队线索和进度条。
- 右下角地图入口在收到节会/远征房间邀请时新增可读浮动提示，配合既有角标与地图推荐入口，让被邀请玩家更容易发现并处理邀请。
- 右下角地图邀请提示升级为可点击入口，会按当前节会/远征邀请直接跳到对应房间页并聚焦待处理邀请，减少“看见提示但还要自己找”的断点。
- 统一邀请面板新增“推荐一起开局”，优先展示在线好友、好友、近期队友和推荐玩家，并支持一键加入推荐名单，让房主不必只靠手输 ID 约人。
- 在线中心“离线也能帮”新增离线协作队列，把留下委托、派出帮手、借用庄园设施、领取异步协作奖励串成 4 步下一步行动，并展示状态、影响和奖励。
- 在线邻里好友页新增“离线轻协作”看板，把发布委托、派出帮手、进入好友庄园设施和领取异步奖励接到已有委托、好友驿站、庄园、邮箱入口，好友不在线时也能看见可推进路径。
- 在线邻里好友页新增“好友榜 / 队伍榜”承接面板，会读取活动中心的好友挑战和主题榜入口，展示可复玩的好友/队伍条目、奖励预期、轻差距规则和再次约人动作。
- 在线中心新增“联机赛季进度”面板，把短局回执、周目标、好友协作、收藏凭证和限定奖励合成一条赛季轨道，展示联机等级、下一档奖励、进度来源和五档里程碑。
- 节会/远征玩法板新增事件变化与隐藏目标展示，短局主舞台会读回天气、季节/NPC/随机事件、护送风险、撤离抉择和隐藏奖励线索。
- 节会/远征统一房间壳新增“队伍脉搏”首屏读回，把下一步行动、岗位分工、连携反馈、共同进度和奖励落点压成四拍信息，进房后能立刻看见队伍正在一起推进什么。
- 节会/远征统一房间壳新增“四岗位协作线”，把队友分工自动整理成采集、护送/战斗、加工/提交、加成/支援四条赛道，并展示负责人、补位状态和下一步行动。
- 节会/远征统一房间壳新增“协作节奏”板，把分工、下一步行动、队伍连携和领奖落点压成四拍读回，移动端和读屏摘要也会同步显示当前协作节奏。
- 节会/远征统一房间壳新增“队友动态高光”，把最近行动、连携加成和今日协作评分从普通反馈列表提升为三条即时高光，移动端和读屏摘要也会同步读回。
- 节会/远征短局计划新增开局爽点预告，灯谜竞猜和资源护送会在开局前集中展示事件变化、隐藏目标、奖励追求和失败收益，让玩家先看到这一局为什么值得玩。
- 节会/远征结算后新增“结算爽点总览”，集中展示贡献、奖励、稀有/收集、队友表现、失败保底，并提供按本局配置直接再来一局的复开按钮。
- 节会/远征房移动端底部操作区新增固定五键轨道，邀请、准备、开始、结算、领奖会始终占位显示，并按当前房间状态读回可操作或未就绪原因。
- 节会/远征纪念记录页新增“联机领奖收口”，集中汇总结算回执、铜钱、奖券、物品线索、本周奖励进度和再开一局入口，让移动端“领奖”不只是跳到记录列表。
- 在线活动中心的领奖、收集、周进度和复玩回看入口会带到纪念记录页的“联机领奖收口”，移动端固定“领奖”按钮也会直接聚焦该面板，减少结算后找奖励的断点。
- 庄园照料页新增“协作短局”主舞台，把共同浇灌、动物护理、作坊接力、访客订单包装成带状态、协作进度、奖励预览、失败保底、事件变化、隐藏目标和再开入口的小游戏卡片。
- 庄园照料页把玩家可见的“反刷”表述调整为“频次限制”，并扩展在线 UI 结构守卫，防止联机邀请提示退回成只有数字角标。

### 0622 采石场接入目标、公会与饰物
- 长期目标新增“旧场清道”“深脉入手”“采石管护”，分别读取采石场累计清理格、深脉清理数和周管护领取次数，让采石场行为能推进目标页。
- 新增采石场饰物“护符·岩遗”，沿用采石场矿洞终点/战斗精通解锁规则，效果偏宝箱发现与额外矿石发现；解锁后可通过既有饰物槽装备。
- 公会页经营联动区会读取旧采石场状态，提示本周管护还差多少格、旧支道是否可进入，以及累计清理/深脉情况；新增 `qa:quarry-goal-guild-trinket-linkage` 守住这条链路。

### 0622 采石场周清理接入潜能反馈
- 采石场本周管护面板补充潜能奖励说明，会显示“潜能心得 + 山野玉”、本周已领取次数、距离下一次奖励还差多少格，以及周上限后的继续清理提示。
- 采石场周管护继续走统一潜能来源发奖，并修正为只有潜能材料发放成功后才标记本周里程碑已领取，避免周上限或异常时误吞下一次机会。
- 潜能页材料来源卡支持来源跳转，“采石场管护”现在可直接指向旧采石场；新增 `qa:quarry-potential-linkage` 并扩展潜能资源/UI 守卫。

### 0622 采石场供材接入村庄项目
- 旧采石场复开后新增“采石场维护供材”捐赠计划，可持续消耗石材、木材、铁锭、黑曜石和龙玉，形成采石场材料回流村庄建设的长期出口。
- 供材里程碑领取后会启用 7 天采石场维护窗口，维护期内采石场每日生成上限 +2；该效果沿用现有维护状态，不会永久无限叠加。
- 村庄建设页的捐赠面板补充需求说明、回报说明、进度和里程碑状态；新增 `qa:quarry-village-project-linkage` 守住材料消耗、限时维护和采石场刷新读回。

### 0622 采石场深脉接入铸魔炉
- 铸魔炉新增“深脉精锻”模式，工坊 Lv.12 后可对镐子消耗黑曜石、龙玉、稀莲守息露和铜钱进行采石场专向铸魔。
- 新增镐子深脉方向词条：岩鸣可让旧采石场非木材资源概率额外主产物，脉握降低深脉点和旧支道推进体力消耗，遗感让深脉/宝箱/古物点概率额外发现采石场遗物。
- 深脉精锻继续复用原铸魔炉事务，材料/铜钱会真实扣除并写入镐子词条，失败会回滚库存；旧 `enchantmentId` 迁移路径保持不变。
- 新增 `qa:quarry-forge-linkage`，并继续通过铸魔炉与采石场守卫，防止采石场深脉词条退回只显示不结算。

### 0622 采石场遗物接入博物馆学者委托
- 旧采石场古物池现在显式接入博物馆线索，三叶虫化石、古陶片、古碑拓片、铜镜和甲骨片的来源提示会标明“旧采石场古物点”。
- 博物馆学者委托新增副本遗物消耗：化石修复笔记会消耗采石场副本化石/陶片，先民遗珍田野报告会消耗古碑拓片、铜镜和甲骨片；首件藏品仍只需一次性捐赠，不要求重复捐同一藏品。
- 学者委托领奖会预检材料和背包空间，并在同一结算里扣除副本遗物、发放声望/铜钱/潜能考据材料；材料不足时不会吞物品，委托卡会显示研究材料当前库存。
- 采石场页新增博物馆线索提示，说明古物点首件可捐、副本可交给学者委托；新增 `qa:quarry-museum-linkage` 守住采石场 -> 博物馆联动。

### 0622 作物成长加速上限修复
- 作物成长总加速增加上限保护，叠加肥料、钱袋、仙缘和装备效果时不会再把所有作物压成一天成熟。
- 扩展农业成长回归守卫，覆盖高额加速下短周期作物不会首日成熟、第二次日结后才可收获。

### 0622 装饰书画演出 NPC 功能联动
- 雪芹、丹青、墨白、赵木匠的装饰/书画/演出类 NPC 功能对齐真实系统：画像计入美观度，景观点缀进入每日好感，题字墨由药碾隐藏配方门控，信件代笔影响邻里创建容量，乐曲与特别演出继续影响节会好感结算。
- 装修页新增 NPC 装饰效果摘要，说明画像、景观点缀和定制家具的来源、数值与上限；目录限定家具锁定提示会指向赵木匠“定制家具”或商店目录，不再只像收藏品。
- 在线邻里/旧社交创建入口显示丹青“信件代笔”的实际容量加成；在线节会房显示墨白乐曲、特别演出和丹青邀请函等已解锁协作效果。
- 新增 `qa:decoration-linkage-effects`，守住装饰、加工、社交、节会、商店目录和 UI 来源读回，防止 LINK-036 退回只显示不结算。

### 0622 料理酒茶药材 NPC 功能联动
- 王大婶“烹饪指导”现在会影响料理页真实成品品质，烹饪时有受限概率升 1 档，料理卡和弹窗会提示可能升品，成功时日志标明 NPC 指导来源。
- 老陆“陈酿指导”接入小屋酒窖，陈酿进度和日结升品统一读取 NPC 调整后的有效天数；药材代购、酒窖储藏等 NPC 功能文案改为已接入的真实货架/仓储效果。
- 新增 `qa:npc-cooking-processing-effects`，守住秘方、每日豆腐、豆腐坊、节庆豆腐宴、酒窖、酒坊加速、稀有酒、茶艺、私藏茶和药材渠道的真实读取点。

### 0622 钓鱼与鱼塘 NPC 功能联动
- 钓鱼类 NPC 功能完成真实接入：鱼获情报显示当前鱼池概率，渔具维护影响浮漂耐久扣减，钓鱼指导降低小游戏压力，秘传钓法和同舟共钓进入稀有鱼权重且保留上限链路。
- 李渔“深水线索”现在会真正解锁瀑布和沼泽深水钓点；未解锁时地点按钮显示锁定原因，钓鱼候选池不会提前放出深水鱼，解锁后显示当日深水鱼候选数。
- 鱼塘页新增深水样本承接提示，把虹鳟、沼泽泥鳅、娃娃鱼、黄鳝等深水可养鱼导向繁育、展示池和周赛；新增 `qa:npc-fishing-pond-effects` 守住钓鱼/鱼塘 NPC 效果接线。

### 0622 背包装备页移动端布局修复
- 背包装备页的武器、帽子、鞋子、戒指和饰品列表改为移动端安全收缩布局，长名称、耐久条和装备按钮会在行内截断或换行，不再把右侧操作区挤出屏幕。
- 扩展 `qa:inventory-guards`，守住装备行的收缩结构，避免后续样式调整重新引入手机横向溢出。

### 0622 矿洞锻造 NPC 功能联动
- 矿洞与锻造类 NPC 功能完成闭环确认：额外矿点、层位提示、掉落加成、熔炉品质、精锻门槛、锻造加速、工具升级加速和工具奖励词条都接入真实探索、加工或升级结算。
- 阿铁“免费工具修理”接入修理台，每周首次修理会把材料和铜钱成本降为 0，UI 显示“阿铁本周免费”，开工成功后写入周 ledger 防止重复领取。
- 新增 `qa:npc-mining-forge-effects`，并继续通过矿洞 HUD、体力减免和工坊铸魔守卫，覆盖 NPC 功能来源、免费修理防重复和铸魔炉优先展示。

### 0622 动物与育种 NPC 功能守卫
- 动物类 NPC 功能接线完成确认：心情衰减、自动好感、配偶畜牧品质、牧场追踪和放牧发现都会影响真实日结、产物或背包收益，并保留好感、品质和概率上限。
- 育种类 NPC 功能确认接入育种台耗时，`breeding_boost` 会缩短真实育种天数且保留最少 1 天下限。
- 扩展 `qa:animal-production-guards` 与 `qa:breeding-preview-guards`，并继续通过 `qa:breeding-capacity-guards`，防止动物/育种 NPC 效果退回只显示不结算。

### 0622 商店与订单 NPC 功能联动
- 商店类 NPC 功能形成闭环：折扣继续真实影响扣钱，批量购买、代购货架、稀有货架和商队预购统一从 NPC 效果读取，并保留库存、容量和旅行商去重保护。
- 何掌柜“稀有委托”现在会把本周寄售加价品类同步推入特殊订单偏好，订单提示会标明 NPC 功能来源，仍复用周刷新、冷却标签和反重复历史。
- 新增 `qa:npc-shop-function-effects`，守住 `shop_discount_bonus`、`rare_commission`、`bulk_buy`、`rare_shop_stock`、`caravan_preorder`、`proxy_buy` 六条商店/订单效果接线。

### 0622 NPC 功能效果注册表
- 新增 NPC 功能效果注册表，统一登记每个 `effectType` 的系统归属、显示名、数值类型和聚合方式，后续商店、订单、动物、矿洞、料理等系统不再各自手写效果判断。
- `useNpcStore` 保留解锁与旧档开放职责，但效果值、生效判断和已解锁效果摘要统一走注册表 helper，方便后续 UI 和 QA 展示“效果来自哪个 NPC 功能”。
- 扩展 `qa:npc-function-unlock-guards`，守住注册表覆盖、store 接线和禁止回退到旧手写聚合；`qa:npc-active-services` 继续覆盖既有 NPC 主动服务链路。

### 0622 节会加工品备料提交
- 节庆摊位新增“节前备料”分类，灯会、秋宴、冬集会分别消耗杂籽油、桂花茶/桂花蜜、米粉、腌姜、田园干货包等加工品，并以现有票券作为回礼。
- 摊位提交路径改为按通用 `costs/rewards` 结算，服务端会真实扣除加工品、保留 item 成本流水，并继续复用个人周上限、全局库存、节庆窗口和幂等回执。
- 前端节庆摊位同步会在当前会话扣除提交材料后再发回礼，面板和日志区分“提交备料”和普通购买；新增 `qa:festival-stall-supply-consumption` 并扩展 `qa:festival-stall-save-sync` 守住这条链路。

### 0622 加工品消耗分组
- 新增加工品分组数据源，把油料、粉料、腌制、干货、甜品材料、动物加工、鱼加工、药材加工、酒醋发酵、茶饮、豆制、饲料点心、布料、香品、精炼材料和灵锻产物统一登记，后续订单、家庭心愿和节会备料可按分组抽取候选。
- 特殊订单生成会写入加工分组来源、需求提示和调试 trace，联动需求池支持按 `processedGroupId` 反查；家庭心愿材料补充来源分组，百科详情和搜索关键词会显示“加工分组”。
- 新增 `qa:processed-item-groups`，守住加工机器产物分组覆盖、订单/家庭/节会候选池、特殊订单 trace、需求池反查和百科接线。

### 0622 联机小游戏入口和短局提示
- 在线中心新增“今日可玩小游戏”卡组，把灯谜竞猜、摆摊接力、资源护送、共同浇灌和每日互助直接放到活动中心首屏，并展示可加入 / 可邀请状态、事件变化和短局奖励提示。
- 在线中心新增“小游戏种类目录”，按节会、远征、庄园、好友轻互动和周末大型活动展示 20 个活动挂点；每个条目都标出短局耗时、事件变化、奖励预期和可跳转入口，先把玩法池丰富度露出来。
- 房间创建向导的备用模板对齐服务端真实节会 / 远征模板，显式支持灯谜抢答和资源护送；玩法卡补充 2-5 分钟短局、天气 / NPC / 随机事件 / 隐藏目标和失败保底收益说明。
- 节会与远征最近结算新增“再来一局”入口，会按上局房型预填创建向导；结算卡补充奖励、队友贡献、连携 / 纪念 / 护送完整度和路线段数等短局高光。
- 节会房和远征房新增短局“玩法板”：灯谜抢答局会把抢答、题签整理、观众秩序、现场资源和队友反馈放到主舞台；资源护送局会把路线推进、稳固货物、途中事件、路线风险和奖励 / 失败保底放到同一张可操作面板。
- 节会房和远征房的协作面板新增态势提示，集中显示今日协作评分、活跃队友、分工缺口、目标差距、事件加成、现场压力、远征风险和队伍连携次数。
- 节会房和远征房的协作面板升级为四职能“队伍分工台”，固定展示采集、护送 / 控场或战斗、加工 / 提交、加成 / 支援，并给每类职责补下一步提示和即时反馈流。
- 节会房和远征房统一房间壳新增“本局战术”HUD，固定展示下一步行动、岗位缺口、奖励落袋、共同进度和队伍活跃度，让房间更像正在进行的一局短玩法。
- 在线中心新增“离线也能帮”轻协作区，把留下委托、派出帮手、借用庄园设施和领取异步协作奖励集中成四个可跳转入口。
- 在线中心侧栏新增“好友榜 / 队伍榜”，优先展示好友、节会结算和远征结算，用轻量榜单把短局回看和再次组队串起来。
- 在线中心新增“联机奖励路线”，把每日、每周、协作和长线奖励分层展示，明确铜钱 / 材料 / 好友币 / 联机经验、稀有材料 / 潜能资源 / 装饰称号、队伍连携和小游戏熟练度 / 稀有外观家具等追求目标。
- 在线中心新增“奖励兑现清单”，把今日可领、本周缺口、协作收益和长期收藏拆成下一步、奖励内容、凭证来源三段，玩家不用猜联机短局到底能拿到什么。
- 在线中心新增“联机收集目标”，把节会纪念册、远征图鉴、好友徽章和庄园协作称号做成可跳转的长期目标卡，读回现有纪念、结算、好友和共同庄园进度。
- 在线中心新增“每周主题与赛季限定”，把灯会周、远征周、丰收周和老带新奖励做成可跳转主题卡，提前展示限定家具、称号、战利品陈列和新人协助奖励。
- 今日可玩小游戏卡补充可加入、可邀请、可结算、今日完成和本周奖励剩余矩阵，并把本局奖励进度和长期收集目标固定展示，玩家进活动中心就能判断哪一局最值得开。
- 在线中心首屏新增节会 / 远征 / 庄园三套“主题舞台”卡，把灯会市集、路线地图 / 战利品、协作经营 / 共同建设的视觉线索前置展示，不再只像普通表单信息卡。
- 今日可玩小游戏和灯会周 / 远征周入口会直接打开已预填的创建向导，灯谜竞猜、摆摊接力和资源护送不再需要玩家进页后二次找创建入口。
- 房间创建向导的邀请步骤新增好友 / 近期玩家候选卡，开房时可以直接点选队友；房间创建成功后会按向导名单继续发送节会 / 远征邀请。
- 在线节会页进入后会主动启动联机实时连接，并在连接断开时对节会房 / 远征房做当前页静默兜底刷新；准备大厅和主舞台人数改为区分“已加入”和“已邀请”，不再把待接受邀请误显示成已占满成员。
- 房主取消未开局的节会 / 远征房时会撤销未响应邀请，受邀方实时刷新后不再保留已失效邀请。
- 在线中心“今日推荐”改为固定 3 个行动入口，会优先展示待处理邀请、进行中的房间、可领取奖励、接力委托和庄园照料，并为兜底短局展示推荐原因、预计耗时、奖励预期和协作人数。
- 节会房和远征房新增“短局计划”与“短局结算高光”面板，会直接展示 2-5 分钟短局进度、当前事件、推荐下一步、结算奖励预览、失败 / 中断保底说明、贡献高光、奖励落账、队友表现和再来一局提示。
- 节会房和远征房新增结算后“短局战报”横幅，把本局状态、最高贡献、奖励落袋、纪念 / 图鉴进度、队友表现和再来一局入口压到第一眼可读的位置。
- 节会房和远征房新增“短局奖励爽点”战报层，把贡献爆点、奖励落袋、稀有 / 收集进度、队友表现、再来一局和失败保底固定成 6 个格子，结算后能更快看到本局值不值得复开。
- 在线中心新增“再开一局 / 约人复玩”区，把节会回执复开、远征路线复开、好友 / 队伍榜挑战和老带新入口集中展示，并说明复玩奖励与榜单轻差距规则。
- 在线中心新增“今日事件变化”面板，把天气影响、季节限定、NPC 乱入、随机事件和隐藏目标五类变化集中展示，并说明触发条件与事件收益。
- `qa:online-ui-structure` 增加今日推荐三入口、今日可玩小游戏、灯谜竞猜、资源护送、事件变化、失败保底、小游戏状态矩阵、奖励追求区、三主题舞台、轻协作入口、好友榜 / 队伍榜、四层奖励路线、收集目标、每周主题与赛季限定、复玩区、轻差距榜单说明、今日事件变化、结算战报、结算高光、奖励爽点、再来一局、本局战术 HUD、协作态势和创建向导真实模板守卫。

### 0621 在线庄园互动前端优化
- 在线庄园概览重排为庄园封面、访客身份、今日可做和访问 / 留言 / 照料 / 轻采四个主动作，访客首屏能直接完成串门核心操作。
- 到访记录改成轻量到访卡片，留言入口改为概览主按钮打开弹窗；最近留言、来访、照料和轻采汇总为最近互动流，规则和凭证默认收进折叠明细。
- 照料与轻采场景板强化为可点击热点体验，物件选中后直接显示可做动作、收益 / 限制摘要和行动反馈；移动端烟测补充首屏、留言、照料入场景和轻采成功刷新断言。

### 0621 钱包页前端优化
- 钱包页改为总览、预算、票券、额度、流派、被动六个分区入口，默认只展示当前分区，减少预算、奖券、额度兑换和被动清单长堆叠带来的阅读压力。
- 移动端保留“当前推荐动作”卡片，并让推荐动作、Prompt 跳转和外部引导自动切到对应分区；旧钱袋被动列表改为双列卡片入口。
- 新增 `qa:wallet-ui-structure`，守住分区入口、Prompt handlers、移动端主动作卡和被动区挂点。

### 0621 物品系统联动第一批
- 新增物品联动矩阵和统一需求池，先登记杂籽油、庄园边角作物包和 7 个共同丹炉高阶丹药的来源、消耗口与后续联动方向。
- 杂籽油接入“杂油拌面”“节庆油糕”两道料理，并新增“家常油料补给”特殊订单，形成料理与周期订单双出口。
- 家庭心愿新增真实物品需求：庭前共食会消耗杂籽油和鸡蛋，集市共宴会消耗杂籽油和米粉；小屋页会展示需求库存，完成时先扣材料，奖励失败会回滚。
- 庄园边角作物包接入在线委托页本地“邻里边角备料单”和宠物特别喂食；线上提交会真实扣除背包物品、按周锁定并奖励少量商队票券。
- 共同丹炉高阶丹药现在可从背包直接使用，复用当天单丹药 Buff 槽，并回流到矿洞、采石场、行旅图、NPC 对话、节会和宠物安抚等既有结算读取点。
- 高阶丹药新增探索准备物入口：采石场旧支道可显式消耗风蚀护脉丹/稀莲守息露降低本段压力，行旅图路线和首领远征可出发即消耗晶辉、沼光或月珠类丹药并写入旅程日志。
- 物品卡和百科用途详情都会读取联动矩阵，显示料理、订单、家庭、宠物、线上订单、背包使用、采石场和远征等真实用途；背包和图鉴里的用途标签可直接跳往对应场景。新增 `qa:item-linkage-matrix` 守住这批弱用途物品不再退回孤儿状态。

### 0621 瀚海沙海图前端改版
- 瀚海页从三段长列表改为“沙海路线图 + 节点详情”：驿站商店、商路投资、遗迹勘探、瀚海赌坊和本周轮换以节点形式呈现，顶部集中显示循环阶段、本周首领、遗迹清理、活跃投资、赌坊次数和轮换货架。
- 商路、遗迹、赌坊和轮换联动保留原玩法规则与结算逻辑，只调整信息层级、首屏决策摘要和异域旅途视觉；`qa:hanhai-economy-guards` 增加地图化结构守卫。

### 0621 家族关系图谱交互优化
- NPC 页家族关系图谱改为地图式关系网：默认收束到核心关系，并提供核心、来客、村中、旧档、全图筛选，弱关系、家族旁支和旧档可按需展开。
- 关系节点新增焦点模式、选中路径高亮和更清晰的右侧详情层级；查看人物、聊天、送礼快捷入口和本地存档 / 隐私边界保持不变。
- 图谱默认视图改为轻量渲染：短访、熟人、村中 NPC、旧档和家族旁支只在对应筛选打开后构建；同时移除黑底网格、厚重外圈和选中暗描边，减轻进入 NPC 页时的卡顿与视觉压迫感。

### 0621 商圈二级货架界面
- 商圈各子店铺改为“进店 → 货架目录 → 货架详情”的二级界面：万物铺、铁匠铺、镖局、渔具铺、药铺、绸缎庄和酒馆会先展示货架卡片，再进入单个商品列表。
- 货架目录统一显示说明、商品数量和进入箭头，详情页提供“返回货架”按钮；原有批量购买、限购、熟客折扣、回购和镖局承接结算逻辑保持不变。

### 0621 联机中心与共同庄园体验收束
- 在线中心新增“联机活动中心”：把今日推荐、节会 / 远征 / 庄园三类主题活动、房间状态、协作分工、奖励预览、好友正在玩的内容和本周奖励进度集中到首屏。
- 节会房和远征房的统一房间壳新增协作推进面板，集中展示共同进度、队友分工、连携 / 行动反馈；移动端补固定底部操作条，把邀请、准备、开始、结算和领奖记录入口从抽屉里提前出来。
- 在线中心新增“今日联机待办”聚合区，优先提示房间邀请、共同庄园待确认、好友庄园照料 / 轻采、互助委托和可领取邮件奖励，活动排期与功能开关仍保留在折叠详情里。
- 共同庄园默认主入口收束为总览、共同地图、仓库、基金、权限、离线 / 安全六项；桌面三栏结构保留，移动端新增顶部状态摘要和横向契约切换，仓库 / 基金 / 权限入口保持 44px 触控目标。
- `scripts/qa-online-ui-structure.mjs` 增加联机活动中心、房间协作推进、移动端固定操作条、今日待办、六入口主流程、移动端共同庄园状态摘要和契约横向切换守卫。

### 0621 村庄建设嵌入区优化
- 村民页里的“村庄建设”区块改为摘要优先、宽屏双列的卡片布局，完工项目不再重复铺开铜钱和材料清单，待建设、待线索与维护异常状态也会在顶部集中提示。

### 0621 联机邀请入口优化
- 节会房与远征房邀请面板新增好友 / 近期玩家快捷选择，不再只能手动输入玩家 ID；已在当前房间的玩家会直接标记为不可重复邀请。
- 右下角地图入口新增节会 / 远征房间邀请角标；移动端地图菜单会把待处理联机邀请提升为推荐入口，并在联机快捷区展示待确认数量。
- `scripts/qa-online-ui-structure.mjs` 增加邀请候选来源、地图角标和移动端联机邀请入口静态守卫。

### 0621 作物小数成长阈值修复
- 作物有效成熟天数不再被向下/向上整天化，肥料、钱包、春息和装备生长加速统一作用于同一套小数成熟阈值；例如 5 天作物吃到 33% 生长加速时按 3.35 天阈值结算，不再显示成 `1.33/5` 后又按整天口径成熟。
- 农场详情、地块提示和季末种植风险提示同步格式化小数成熟天数；`qa:fertilizer-retention` 与 `qa-tool-upgrade-guards` 增加小数阈值和装备成长加速口径守卫。

### 0621 鱼塘取出鱼状态修复
- 修复鱼塘取出鱼后再卖掉，后续新放入同种鱼仍复用旧鱼品种与入塘天数的问题；取出的鱼个体状态改为跟随背包槽位保存，卖出或消耗后不会残留在鱼塘隐藏队列里。

### 0621 背包种子详情
- 背包查看种子时会显示该种子的适宜季节、基础成熟天数；多茬作物额外显示再收周期，方便播种前判断是否适季。

### 0621 钓鱼难度颜色
- 钓鱼页「传说」难度鱼类改用更醒目的紫红高稀有度文字样式，不再弱于「困难」难度提示。

### 0621 仓库矿石堆叠修复
- 仓库与快捷虚空箱的物品列表改为按物品 ID + 品质合并展示，龙玉、五彩碎片等因来源字段不同产生的隐藏批次不再拆成多行；底层来源字段仍保留给出售/回购结算使用。

### 0621 育种作物冬季适应性
- 8 阶以上终局育种作物新增一批冬季适应品系，覆盖瓜、薯、菜、茶、麦、根、芽、兰、栗、梨等高代谱系；保留部分高温/瀚海分支的非冬季定位，避免全部终局作物四季通用。
- 更新 `qa:breeding-preview-guards`，守住 T8/T9/T10 都存在冬季适应高代作物，且不会把全部终局育种作物统一冬季化。

### 0621 旧采石场古物收取修复
- 修复采石场紫色古物点抽到「古碑拓片」时，因奖励物品 ID 未注册而被误报为「背包空间不足」的问题；该古物现在可正常入包并可捐入博物馆。

### 0621 工坊自动收取反馈
- 工坊每日自动收取日志改为展示实际入库产物、数量和品质，例如虚空原料箱续产、无原料机器和精研双倍产出都会汇总为具体物品明细，不再只显示配方/机器完成名。

### 装备耐久系统
- 武器/戒指/帽子/鞋子全部加入耐久上限，由品质档位（common/fine/excellent/supreme）+ 配方成本动态计算，锻造词条和附魔均可影响耐久上限与消耗速率。
- 武器+戒指在战斗中每次攻击扣 1 点耐久；帽子+鞋子在日常农事操作中每天扣 1 点（批量操作不叠加）。
- 装备耐久归零后自动锁定，无法装备但不消失，修理后恢复。
- 新增「修理台」加工机器，消耗材料+时间恢复装备全部耐久；苏苏 T2 提供 NPC 修理费折扣和耐久上限加成。
- 秋月 T3「渔具维修」效果上线：钓鱼浮漂耐久消耗 -30%（30% 概率跳过扣除）。
- 耐久减耗改为小数消耗累积，耐磨、附魔和 NPC 减耗在“单次扣 1”的攻击/日常动作中也会真实延缓掉耐久，并保留存档进度。
- 锻造词条新增：坚韧（耐久上限 +10%~30%）、耐磨（消耗降低 +10%~25%）、易损（负面，耐久上限 -10%~20%）。
- 附魔新增：恒久（戒指耐久 +15%）、坚甲（帽子消耗 -15%）、韧行（鞋子消耗 -12%）。
- 背包装备卡片显示耐久条，归零装备显示「破损」标记并灰置。
- 修复修理台存档字段恢复逻辑的作用域错误，避免加工店 store 热更新失败后读档时报 `processingStore.serialize is not a function`。
- QA 守卫脚本 scripts/qa-durability-guards.mjs 验证 65 项耐久系统完整性。

### 0620 快捷键 — 矿洞下一层
- 矿洞页新增 `E` 键快捷下一层（`miningDescend`），仅在楼梯已发现且可用时生效；战斗中、动画锁、弹窗打开时不响应。下一层按钮旁显示当前绑定键位标识。

### 0618 旧采石场复开
- 新增「旧采石场复开」终局村庄工程，前置「矿料棚与支架」，费用 80000 文、石 500、木 200、铁矿 100、金矿 60、铱矿 20、虚空矿 10；要求主矿洞 120 层、骷髅矿穴 100 层、采矿 Lv20、至少 1 个采矿精研节点、村庄工程等级 ≥6。
- 采石场改为 8×8 起步的开放可见资源场，建成当天会直接露出 9 个石头、矿脉、木桩、裂隙或宝箱/古物点；之后每日早晨只在空地按 12% 概率慢速生长新点位，已有资源会阻挡新刷。日刷新上限随年份从 7 逐步增至 16，维护生效 +2、骷髅矿穴 150 层 +2、采矿精研节点 ≥2 时 +1。
- 资源池：普通岩石 52%、普通矿 28%、宝石 10%、枯木/竹材 8%、深脉点 2%；深脉点仅出虚空矿/铱矿/黑曜石/龙玉，不放五彩碎片。宝箱、古物和怪物裂隙作为少量惊喜点直接显示在场上，不再支配整盘盲探体验。
- 采石场改为独立页面与独立导航入口，位于野外菜单「行旅图」后方；不再挂在矿洞页。
- 普通石头、矿脉、宝石、木竹、宝箱和古物点可直接点击收取；深脉点保留更高体力消耗；怪物遭遇继续使用矿洞同款弹窗战斗，夜晚怪物会获得额外强化。背包不足时资源格会保留在原地。
- 新增一次性「采石场矿洞」短路线：在采石场页进入，按顺序清理少量矿点、怪物和旧矿工箱，终点奖励解锁 `trinket_quarry_shard` 的采石场矿洞凭据；终点奖励不可重复领取，矿洞不作为可反复刷材料来源。
- 新增后期扩建链：采石场从 8×8 起步，可按阶段花费铜钱、矿材与累计清理进度逐步扩至 32×32。
- 新增潜能来源「采石场管护」，每周清理满 12 格资源可领取一次，每周最多 2 次、每次最多 4 份材料，奖励潜能心得 + 山野玉。
- 采石场页锁定时显示完整门槛清单；解锁后显示可见资源网格、可清理资源、稀有点、空地、今日已生成/上限、维护状态、周清理进度、夜晚提示、扩建条件和采石场矿洞入口。
- 更新 `qa:quarry-respawn-guards`，覆盖高门槛、旧档 hidden 迁移为可见状态、独立页面入口、12% 日刷只落空地、已有资源阻挡新刷、8→32 扩建、每周潜能上限、矿洞一次性终点奖励和饰物解锁凭据。

### 0618 电脑端快捷键绑定
- 设置弹窗新增“快捷键”页，支持总开关、重新绑定、清除、单项恢复默认和全部恢复默认；快捷键配置随设置存档版本化保存，旧档默认启用 v1 键位。
- 游戏内新增全局键位：`O` 设置、`L` 记录中心、`F9` 存档管理，`F/I/J/M/C/K/P/G` 分别跳转农场、背包、任务板、行旅图、角色、技能、潜能、目标，`V` 在虚空箱解锁后打开虚空箱；输入框、录入状态和阻塞弹窗会自动忽略快捷键。
- 矿洞战斗弹窗新增 `1/2/3/4/5`：攻击、防御、逃跑、使用战斗道具、切换装备方案，并在按钮上显示当前键位；Boss 战按逃跑键只提示不可逃跑，不走战斗结算也不消耗时间。
- 矿洞探索与行旅图地图页新增 `↑/↓/←/→` 方向键移动：长按方向键可连续移动，矿洞光标高亮带平滑过渡动画，行旅图棋子用 CSS transition 丝滑滑动到目标格，不再跳切镜头；战斗、弹窗、远征进行中和输入焦点下不触发。
- 第二批补齐 `N/H/B/R/T/U/Y/Q` 默认导航键，并加入 `[`/`]` 切换当前界面分组、`/` 聚焦搜索、`PageUp/PageDown` 翻页；背包、工坊、育种、鱼塘、公会、瀚海、图鉴、博物馆、装饰、商圈、邮箱和百科页已接入上下文快捷键，弹窗或购买/提交流程中会自动暂停。
- 新增 `qa:keyboard-shortcut-guards` 与 `qa:keyboard-shortcut-browser-smoke`，覆盖默认键冲突、保留组合键、设置页/矿洞稳定测试入口、导航键、总开关、输入聚焦、重绑和矿洞战斗快捷键路径。
### 0620 快捷键 v4 — 补齐剩余页面接入
- 农场页接入 `[`/`]` 切换田庄/林木标签、`PageUp/PageDown` 翻页，弹窗（地块操作、温室、批量操作、出货箱、季节风险确认、果/野树砍伐）期间自动暂停。
- 灶台、技能、钱包页接入 `PageUp/PageDown` 翻页，对应弹窗（菜谱详情、精研洗点、票券详情）期间自动暂停。
- 旧采石场接入矿洞战斗快捷键 `1/2/3`（攻击/防御/逃跑）复用 `miningAttack/miningDefend/miningFlee`，以及 `PageUp/PageDown` 翻页。
- 更新 `qa:keyboard-shortcut-guards` 覆盖 v4 新增 5 个页面的快捷键连线验证。
### 0619 通用交互快捷键 v3
- 新增通用交互快捷键分组：`F8` 打开睡觉确认弹窗（不直接过日）、`Enter` 确认/主动作、`Esc` 取消/关闭最上层弹窗；`uiFocusPrimary`（聚焦主按钮）和 `uiQtyDecrease/uiQtyIncrease`（数量弹窗调节）预注册但默认不绑定，供玩家自定义。
- 睡觉弹窗内 `Enter` 才执行最终过日、`Esc` 关闭弹窗；有设置、存档管理、记录中心、公告、矿洞战斗或数量弹窗等阻塞弹窗时不响应 `F8`。
- 设置快捷键页新增「通用交互」分组；`Esc` 作为系统级 UI 键保留，不进入普通玩家重绑池。
- 弹窗按钮显示 `Enter`/`Esc` 小标识，睡觉按钮显示 `F8` 小标识；虚空箱数量弹窗接入 `uiQtyDecrease/uiQtyIncrease`。
- 更新 `qa:keyboard-shortcut-guards` 覆盖 v3 动作定义、`uiInteraction` 分类标签、睡觉弹窗/虚空箱数量弹窗快捷键连线和 `Esc` 例外白名单。

### 0618 育种后期容量扩展
- 育种台上限从 3 台扩展到 10 台，第 4~10 台继续通过制作获得，并按编号使用更高的铜钱与材料阶梯成本，强化后期育种并行发展空间。
- 种子箱在原 Lv.5 基础上继续开放 20 级，最高 Lv.25；容量公式保持每级 +15 格，最终可扩至 405 格，旧存档读入时会把异常等级夹到 0~25。
- 新增 `qa:breeding-capacity-guards`，并让 `qa:resource-consumption-guards` 覆盖全部育种台阶梯成本与 Lv.1~25 种子箱材料重复检查。

### 0618 固定村民帮办服务
- NPC 详情页新增“村民帮办”周限服务：陈伯、柳娘、阿石、秋月、林老、素素、赵木匠、孙铁匠、云飞、王大婶、红豆、春兰、雪芹、丹青、墨白、大牛会按各自专长提供补给、材料或票券，把好感推进转成可重复使用的实际收益。
- 帮办服务改为每周至少与对应 NPC 交谈 3 次后才能委托；委托当天只扣费并写入待办，不立即发放奖励，次日结算时再送达物品或票券并写入日志。
- 委托时继续按每周账本、关系阶段、NPC 是否在场和铜钱校验；次日结算按账本与背包空间发放，若物品送达失败会自动退款，票券发放不吃后期倍率，避免把社交服务变成刷票漏洞。
- 新增 `qa:npc-active-services`，守住服务数据覆盖、每周账本 key、3 次交谈门槛、委托排队、次日结算、失败退款、票券发放和 NPC 页测试入口。

### 0618 酒坊酒品收益对齐
- 红枣酒售价从 360 调整为 450，按红枣实际出售价值满足酒坊“原料收益 ×3”的收益规律。
- 桃源米酒售价从 80 调整为 180，两壶产出总值不再低于稻米原料三倍；米醋继续作为料理材料链处理，不套用酒品三倍售价。
- 远古水果隐藏酒坊配方重新产出专属「远古果酒」`ancient_fruit_wine`，不再和多年育种链顶端作物共用「天成果酿」；天化薯等高代作物继续保留顶档果酒出口。
- `scripts/qa-hidden-processing-guards.mjs` 补充公开酒坊酒品收益、远古专属果酒和天化薯顶档果酒守卫，防止手写酒坊配方再次低于原料三倍，或让优质肥料远古压过多年育种链收益。

### 0618 节庆摊位限购账本裁剪修复
- 节庆摊位周账本不再按插入顺序保留最早 16 周，改为按游戏周 / 旧现实周 / 交易时间保留最新周，并在购买、失败回执和回滚写入时强制保护当前周，避免当前周限购记录刚写入就被裁掉。
- 修复真实数据累计超过 16 周后，票券包当次购买返回 `1/1`，重新读取又变回 `0/1`，从而可以反复买到 1500 文资金波动上限的问题。
- 新增 `qa:festival-stall-personal-limit`，覆盖 16 个旧周后购买秋宴票券包、重读仍保持个人限购、换新幂等键二购被拒、重复回执不重复发奖和治理资金不重复累计。

### 0618 温室种子选择器分组
- 温室单块种植和一键种植改为按作物分组展示种子；普通种子在组内保留品质按钮，育种种子按作物折叠，展开后再选择具体基因种子，避免同类种子过多时刷屏难选。
- 温室种植弹窗补充搜索和“全部 / 普通 / 育种”筛选，并改为固定视口高度加内部滚动；`qa:seed-quality-harvest-guards` 同步守卫品质传递、育种分组和弹窗滚动结构。

### 0618 工坊机器批量拆除
- 工坊加工页的机器分组新增“批量拆除”入口，可按机器类型一次性拆除同类机器；确认弹窗会展示空闲、加工中、已完成数量，以及退回背包、进入虚空成品箱和返还铜钱的明细。
- 批量拆除改为先做整批容量预检，再一次性执行；若背包无法完整接收拆除材料和产物，确认按钮会禁用并提示，不会出现拆到一半才失败的状态。
- `qa:workshop-batch-remove-guards` 补充批量拆除入口、预览弹窗、虚空成品箱模拟、单拆复用共享执行器和原子回滚守卫。

### 0618 育种种子同种归类
- 育种页种子箱新增“同种”排序，按作物把同类育种种子排在一起；同一作物内优先显示高世代、高总属性种子，选择两颗种子的育种弹窗同步复用该排序。
- `qa:breeding-preview-guards` 补充同种归类排序、类型枚举和弹窗列表复用守卫，避免回退成两个亲本隔很远的默认顺序。

### 0618 委托品质化
- 部分正式村民委托和特殊订单现在会要求最低品质：木材、柴火、竹子类订单开始区分良品/精品及以上，高品质材料有了稳定交付用途。
- 品质门槛按“该品质及以上”结算，提交时只统计并消耗达标品质；良品单可交精品/极品，普通品质不会被误扣。
- 带品质门槛的委托会按现有品质倍率提高基础铜钱和好感奖励；任务板、任务详情和特殊订单组合目标同步显示品质要求与达标数量。
- `qa:quest-guards` 和 `qa:inventory-guards` 补充最低品质统计、提交扣除、组合订单和 UI 文案守卫。

### 0618 矿石宝石品质归一与虚空箱合并
- 矿石 / 宝石类物品入背包、临时背包、仓库和虚空箱时统一归为普通品质，修复龙玉、五彩碎片等无实际品质差异物品因隐藏品质来源被拆成多格的问题。
- 仓库 / 虚空箱列表继续按物品 + 品质合并显示；底层仍保留商圈来源、购买日和购入单价等批次元数据用于回购 / 售价计算，读档会压实旧存档里的同批次碎堆。`qa:inventory-guards` 补充品质归一、旧虚空箱迁移、商圈批次保留和显示合并守卫。

### 0618 更新公告置顶
- 后台更新公告新增“置顶公告”开关；已发布公告同一时间只保留一条置顶，发布新置顶时会自动取消旧置顶，下线公告会同步取消置顶状态。
- 玩家公告弹窗和历史公告都会把置顶公告排在最上方并显示置顶高亮；置顶公告已读后不会单独弹窗，但有其他未读公告时仍会跟随展示在弹窗顶部。
- `qa:announcement-flow`、`qa:announcement-ui-structure` 和 `qa:announcement-visual-smoke` 补充置顶保存、自动替换、已读置顶跟随未读公告展示和置顶视觉标识守卫。

### 0618 更新公告保存更新按钮开关
- 更新公告弹窗的“保存存档并更新”按钮改为公告配置控制：版本更新和热修复说明默认显示，维护、活动预告、补偿说明等普通公告默认只显示“知道了”或“知道并领取”。
- 后台公告管理新增“显示保存存档并更新按钮”手动开关，可对任意公告覆盖模板默认值；旧公告缺少该字段时继续按模板类型兼容推导。
- `qa:announcement-flow`、`qa:announcement-ui-structure` 和 `qa:announcement-visual-smoke` 补充默认值、手动开关、公开接口回传与普通公告不显示更新按钮的回归验证。

### 0618 桃源田庄专属 16×16 扩建
- 农场扩建上限改为按田庄类型区分：溪流、竹林、山丘、荒野和草甸田庄仍最高 12×12；桃源田庄在 12×12 后可继续扩到 16×16。
- 选田庄文案同步写明桃源田庄初始 6×6、可扩至 16×16，其他田庄初始 4×4、可扩至 12×12；12×12 → 16×16 作为后期工程，需要 120000 文、采集 Lv.20、建设券×12、木材×2400、石材×1800。
- `qa:farm-expansion-guards` 补充普通田庄专属上限、商店显示门槛、旧存档按田庄类型归一化、共同庄园 16×16 读入和选择文案守卫。
...
### 0618 桃源小助理口语理解与降噪
- 桃源小助理新增“现在干啥 / 咋办 / 去哪弄 / 为啥卡了 / 帮我看看”等玩家口语意图识别，当前页面有上下文时会直接进入今日规划、任务诊断、资源来源或下一步建议，不再轻易退回泛问答。
- 远程模型提示词改为先归纳玩家真实意图、第一句给直接结论，并限制正文密度；欢迎语、本地草稿和三步建议正文同步去掉 fallback、等待说明、收益长段和重复安全声明。
- 补充 `qa:ai-assistant-intent-slots`、`qa:ai-assistant-model-client`、`qa:ai-assistant-configured-quick-questions`、`qa:ai-assistant-local-draft` 和三步建议 guard，覆盖口语识别、短欢迎语、短本地草稿与紧凑行动建议。
- 远程模型成功返回后，聊天区会移除同次请求的本地草稿，并默认不再展示内置知识库来源块；新增大模型语义预解析，把“咋整 / 缺一个去哪弄”等口语先转成受控 intent、question type 和检索词，再回到本地公开知识库校验。
- 本地任务诊断和今日规划答案改为“结论 / 为什么 / 下一步 / 注意”短结构，隐藏内部评分、诊断信号数和长安全声明，只保留玩家能立即执行的卡点、路线和可见状态依据。
- 本地资源反查、泛知识和源码线索回答继续降噪，正文不再输出“结构化公开资料回答 / 资源索引 / 依据 / 证据 / 命中公开资料”等内部流程词；相关 QA 改为检查玩家可见结论、来源路径和 `evidence` 结构字段分离。
