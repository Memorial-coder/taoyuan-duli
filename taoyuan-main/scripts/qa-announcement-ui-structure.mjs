...
assert.match(gameLayout, /:claim-rewards="announcementStore\.canClaimRewardsInCurrentSession"/, 'game layout should pass runtime reward claim capability')
...
assert.match(gameLayout, /skippedRewardCount/, 'local announcement close should surface skipped server rewards')
...
assert.match(store, /mode === 'local'/, 'announcement suppress scope should support local runtime saves')
assert.match(store, /`local_slot_\$\{slot\}`/, 'announcement suppress scope should include the local save slot')
...
assert.match(store, /runtimeSessionMode === 'server'/, 'announcement rewards should only be claimed from server runtime sessions')
assert.match(store, /skippedRewardCount/, 'announcement close result should report rewards skipped for local saves')
