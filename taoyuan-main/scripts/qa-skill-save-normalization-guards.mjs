...
const createSkill = type => ({ type, exp: 0, level: 0, perk5: null, perk10: null, perk15: null, perk20: null })
...
const normalizeProgress = skill => {
  ...
  if (skill.level >= 20 && skill.exp > expTable[20]) skill.exp = expTable[20]
}
...
const normalized = normalizeSave([
  { type: 'farming', exp: -50, level: -3, perk5: 'harvester', perk10: 'artisan' },
  { type: 'mining', exp: 999999, level: 99, perk5: 'fighter', perk10: 'warrior', perk15: 'sword_saint', perk20: 'war_god' },
...
assert(mining.level === 20 && mining.exp === 155000, '模型用例：超过 20 级和过量经验必须裁剪到满级边界。')
...
console.log('技能读档归一化守卫通过：level/exp/perk 异常档会被归一化并留下迁移日志。')
