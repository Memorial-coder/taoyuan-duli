...
assert(/class="game-modal-overlay mining-dialog-overlay fixed inset-0 bg-black\/60 flex items-start justify-center overflow-y-auto z-50 p-4"/.test(miningViewSource), 'mining explore dialog should use the draggable overlay shell.')
assert(/data-testid="mining-explore-drag-handle"/.test(miningViewSource), 'mining explore dialog should expose a desktop drag handle.')
assert(/@pointerdown="handleMiningDialogPointerDown\(\$event, 'explore'\)"/.test(miningViewSource), 'mining explore drag handle should wire pointer dragging.')
...
assert(/class="game-modal-overlay mining-dialog-overlay fixed inset-0 bg-black\/60 flex items-start justify-center overflow-y-auto z-60 p-4"/.test(miningViewSource), 'mining combat dialog should use the draggable overlay shell.')
assert(/data-testid="mining-combat-drag-handle"/.test(miningViewSource), 'mining combat dialog should expose a desktop drag handle.')
assert(/@pointerdown="handleMiningDialogPointerDown\(\$event, 'combat'\)"/.test(miningViewSource), 'mining combat drag handle should wire pointer dragging.')
...
assert(/const MINING_DIALOG_DESKTOP_QUERY = '\(min-width: 768px\) and \(pointer: fine\)'/.test(miningViewSource), 'mining dialog dragging should be desktop fine-pointer only.')
assert(/const MINING_DIALOG_DESKTOP_TOP_GAP = 76/.test(miningViewSource), 'mining dialog should leave desktop status-bar space above the panel.')
assert(/const MINING_DIALOG_VIEWPORT_MARGIN = 16/.test(miningViewSource), 'mining dialog dragging should keep a viewport margin.')
assert(/const clampMiningDialogOffset = \([\s\S]*MINING_DIALOG_DESKTOP_TOP_GAP[\s\S]*window\.innerWidth[\s\S]*window\.innerHeight/.test(miningViewSource), 'mining dialog dragging should clamp to viewport bounds.')
assert(/const clearMiningDialogDrag = \(\) => \{[\s\S]*window\.removeEventListener\('pointermove', handleMiningDialogPointerMove\)[\s\S]*window\.removeEventListener\('pointerup', handleMiningDialogPointerEnd\)[\s\S]*window\.removeEventListener\('pointercancel', handleMiningDialogPointerEnd\)/.test(miningViewSource), 'mining dialog dragging should clean up pointer listeners.')
assert(/window\.addEventListener\('resize', clampVisibleMiningDialogs\)/.test(miningViewSource), 'mining dialog should reclamp after resize.')
assert(/window\.removeEventListener\('resize', clampVisibleMiningDialogs\)/.test(miningViewSource), 'mining dialog should remove resize listener on unmount.')
assert(/@media \(min-width: 768px\) and \(pointer: fine\)[\s\S]*\.mining-dialog-overlay[\s\S]*padding-top: 76px/.test(miningViewSource), 'mining dialog desktop CSS should push the panel below the status bar.')
