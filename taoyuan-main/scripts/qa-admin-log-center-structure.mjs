...
for (const label of ['全部', '管理审计', '在线审计', '内容审核', '游戏日志', '内容发布']) {
...
assert.match(panel, /500000/, 'gameplay policy copy should state the 500000 total cap')
assert.match(panel, /12000/, 'gameplay policy copy should state the 12000 per user-slot cap')
