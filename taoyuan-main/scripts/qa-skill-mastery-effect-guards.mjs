...
const perkSelectDialogSource = readSource('src/components/game/PerkSelectDialog.vue')
...
assert(perkSelectDialogSource.includes("deity_of_harvest', name: '丰收之神', description: '作物收获时50%概率双倍，作物售价+50%'"), '农耕20级选择弹窗必须说明丰收之神的真实双收与作物售价效果。')
assert(perkSelectDialogSource.includes("land_god', name: '土地神', description: '农田自动浇水，农耕体力消耗归零'"), '农耕20级选择弹窗必须说明土地神的自动浇水与农耕零体力效果。')
assert(skillViewSource.includes("deity_of_harvest: '作物收获时50%概率双倍，作物售价+50%'"), '技能页必须与丰收之神选择弹窗和实际效果保持一致。')
assert(skillViewSource.includes("land_god: '农田自动浇水，农耕体力消耗归零'"), '技能页必须与土地神选择弹窗和实际效果保持一致。')
assert(!skillViewSource.includes('品质必定神圣'), '丰收之神不应再承诺未接入的作物品质必定神圣。')
assert(!skillViewSource.includes('所有农产品产量+100%'), '土地神不应再承诺未接入的所有农产品产量翻倍。')
assert(!perkSelectDialogSource.includes('售价再+15%'), '农耕20级选择弹窗不得保留旧的 +15% 售价文案。')
assert(!farmActionsSource.includes("perk20 === 'deity_of_harvest' || farmingSkill.perk20 === 'land_god'"), '室外收获不得把土地神接入丰收之神双收。')
assert(!farmHarvestSource.includes("perk20 === 'deity_of_harvest' || farmingSkill.perk20 === 'land_god'"), '抽象收获入口不得把土地神接入丰收之神双收。')
assert(farmActionsSource.includes("skillStore.getSkill('farming').perk20 === 'land_god'"), '农耕体力 helper 必须读取土地神分支。')
assert(farmActionsSource.includes("if (skillStore.getSkill('farming').perk20 === 'land_god') return 0"), '土地神必须让农耕行动体力消耗归零。')
assert(farmActionsSource.includes('getFarmingStaminaCostLabel'), '农耕体力日志必须提供土地神零消耗标签。')
assert(farmViewSource.includes('getFarmingActionStaminaCost') && farmViewSource.includes('getFarmingStaminaCostLabel'), '农田页温室与育种播种路径必须复用农耕体力 helper 和日志标签。')
assert(countOccurrences(farmViewSource, 'getFarmingActionStaminaCost(3, [') >= 6, '农田页普通/温室/育种播种路径必须统一接入土地神零体力 helper。')
assert(shopStoreSource.includes("deity_of_harvest: '丰收之神'"), '售价明细必须能显示丰收之神名称。')
assert(shopStoreSource.includes("itemDef.category === 'crop' && farmSkill.perk20 === 'deity_of_harvest'"), '丰收之神售价加成必须只作用于作物。')
assert(!shopStoreSource.includes("farmSkill.perk20 === 'deity_of_harvest' || farmSkill.perk20 === 'land_god'"), '土地神不得继承丰收之神的售价加成。')
assert(!shopStoreSource.includes('farming_perk20_processed'), '农耕20级丰收之神不得再给加工品售价明细。')
