...
const regionViewTemplate = regionView.match(/<template>[\s\S]*?<\/template>/)?.[0] ?? ''
...
addCheck('Open world uses virtual generated empty tiles', /const createOpenWorldEmptyTile/.test(regionData) && /getOpenWorldTileDefAtCoord/.test(regionData) && /const createOpenWorldLargeRegionTiles[\s\S]*return profile\.specialTiles/.test(regionData))
addCheck('Store does not build full open-world tile indexes', !/OPEN_WORLD_TILE_DEFS_BY_(?:REGION|COORD)/.test(regionStore) && !/REGION_OPEN_WORLD_DEFS\.map\([^)]*def[\s\S]{0,240}def\.tiles\.map/.test(regionStore))
...
addCheck('Component drag pan emits float tile deltas', /const deltaX = -dragX \/ cell\.width/.test(openWorldComponent) && /const deltaY = -dragY \/ cell\.height/.test(openWorldComponent) && /emitPanViewport\(\{ deltaX, deltaY \}\)/.test(openWorldComponent))
...
addCheck('Component renders responsive player marker overlay', /playerTokenStyle/.test(openWorldComponent) && /transition:\s*left 0\.12s linear,\s*top 0\.12s linear/.test(openWorldComponent))
addCheck('Component batches drag pan updates by frame', /pendingPanDelta/.test(openWorldComponent) && /requestAnimationFrame\(flushPanDelta\)/.test(openWorldComponent) && /emitPanViewport\(\{ deltaX, deltaY \}\)/.test(openWorldComponent))
...
addCheck('RegionMapView handles focus current event', /@focus-current="handleFocusCurrentOpenWorldTile"/.test(regionView) && /const handleFocusCurrentOpenWorldTile/.test(regionView) && /view\.playerTile/.test(regionView))
...
addCheck('Map route tab uses cached region view models', /visibleRegionMapViews = computed<RegionMapTabRegionView\[\]>/.test(regionView) && /hasVisitedRegionMapTab\.value/.test(regionView) && /buildRegionRouteCardView/.test(regionView))
addCheck('Map route tab template consumes route view cache', /v-for="routeView in regionView\.routeCards"/.test(regionView) && /routeView\.preview\.title/.test(regionView) && /routeView\.decision\.headline/.test(regionView))
addCheck('Map route tab reuses exploration tree build', /const tree = regionMapTreeReady\.value \? getRegionExplorationTree\(region\.id\) : createEmptyRegionExplorationTree\(\)/.test(regionView) && /getRegionTreeSummary\(region\.id, tree, boardSummary\)/.test(regionView) && /:nodes="regionView\.tree\.nodes"/.test(regionView) && /:links="regionView\.tree\.links"/.test(regionView))
addCheck('Map route tab keeps heavy content mounted after first visit', /v-if="hasVisitedRegionMapTab"[\s\S]{0,120}v-show="activeRegionMapTab === 'map'"/.test(regionView) && /scheduleRegionMapDeferredMount/.test(regionView) && /regionMapRouteCardsReady/.test(regionView) && /regionMapTreeReady/.test(regionView))
addCheck('Map route tab is memoized away from open-world camera pans', /v-memo="regionMapTabMemoDeps"/.test(regionView) && /regionMapTabMemoDeps = computed\(\(\) => \[/.test(regionView) && /visibleRegionMapViews\.value/.test(regionView) && !/regionMapTabMemoDeps[\s\S]{0,500}openWorldViewportCameras/.test(regionView))
addCheck('Map route tab template avoids repeated route preview helpers', !/getRouteMapPreview\(route\)/.test(regionViewTemplate) && !/getRouteDecisionSummary\(route\)/.test(regionViewTemplate))
