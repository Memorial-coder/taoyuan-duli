...
- 当前已内置 4 套中后期样例存档：
  - `late_economy_foundation`：后期富裕经营档
  - `breeding_specialist`：育种专项档
  - `fishpond_operator`：鱼塘经营档
  - `endgame_showcase`：终局综合档
- 样例元信息清单位于 `../data-defaults/taoyuan_saves/manifest.json`
- 开发态可直接在浏览器控制台执行：

```js
await __TAOYUAN_SAMPLE_SAVES__.load('endgame_showcase')
```

- 如需查看全部样例列表：

```js
__TAOYUAN_SAMPLE_SAVES__.list()
```

- 该入口用于 QA / AI / 策划快速进入后期状态，无需手改存档或重复跑前期流程。
...
- 调试面板支持：
  - 一键载入 4 套中后期样例档
  - 加钱、强制日结、推进到下周、切换调试日期与主题周
  - 投放测试特殊订单、注入票券快照、查看最近结构化日志
  - 切换后期 feature flag 与套用部分平衡参数覆盖

- 进入该页后，也可在控制台使用：

```js
await __TAOYUAN_LATE_GAME_DEBUG__.loadSample('endgame_showcase')
__TAOYUAN_LATE_GAME_DEBUG__.advanceToNextWeek()
__TAOYUAN_LATE_GAME_DEBUG__.getSummary()
```
