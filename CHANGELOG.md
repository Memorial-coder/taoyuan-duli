...
- 共同庄园仓库主列表拆分：新增 `CohabitationWarehousePanel` 承接共同仓库物品列表、冻结 / 可用数量、取出 / 高价值申请 / 卖出按钮；父页面继续持有仓库策略、治理、草案和补偿审计 slot，原按钮 test id 与 store action 接线保留。
...
- 在线中心首屏行动区：在线中心新增 `online-center-hero-actions`，首屏展示活动房间、村社待办和接力委托摘要，并按继续当前房间、处理邀请、创建活动房间、村社待办、接力委托派生最多 3 个主行动；移动端接入 `OnlineStickyActionBar` 固定主 CTA，mobile smoke 已覆盖 390x844 / 360x780 hero 与 sticky 可见。
- 在线节会旧玩法按钮备用操作：节会场景 / 赛道已有可视化主行动时不再同时渲染旧按钮堆；无可用热区时旧 `online-festival-room-gameplay-action-*` 入口进入「旧节会按钮备用操作」，保留键盘与回归测试路径。
- 共同庄园基金与仓库高风险确认：共同基金大额草案创建 / 确认 / 执行扣款 / 高风险回执，以及共同仓库高价值取用申请 / 确认 / 执行都会先进入 `OnlineConfirmActionDialog`；弹窗要求输入确认文字，并展示共同基金、共同仓库、个人铜币 / 背包和成员确认影响，浏览器 smoke 覆盖完整确认门槛。
- 在线节会房结算 / 关闭确认接入：节会房准备大厅和旧备用按钮现在都会先打开 `OnlineConfirmActionDialog`，展示房间状态、成员影响、奖励预览 / 结算记录和失败恢复提示；运行中关闭要求输入「确认关闭房间」，确认后仍调用原 `festivalRoomStore.settleRoomAction` / `closeRoomAction`，灯会结算 smoke 已迁移到确认弹窗路径。
...
- 共同庄园总览首屏降噪：共同庄园总览移除旧 6 项头部统计条，只保留契约状态、共同基金、共同仓库、今日建议 / 风险待办 4 张主卡；契约表单、宠物照料、恢复、安全边界等细节默认折叠，并保留旧 test id 供回归展开定位。
- 共同庄园标签分组：共同庄园页把 12 个并列标签改成「总览 / 共同经营 / 家庭与节会 / 资产与治理 / 更多」五组，默认只露出总览、地图、仓库、基金、节会五个常用入口；权限、离线、建筑、关系、公开等低频入口进入折叠分组，同时保留旧 `online-module-tab-*` test id 和 `festival/public/family/separation` query 映射。
...
- 在线玩家文案扫描严格 gate：`qa:online-player-copy` 已从 warn-only 切为严格阻断，当前扫描 59 个 Vue 文件、14 个 denylist 词和 1 条 `OnlineTechnicalDetails` allowlist 通过；共同庄园审计摘要、在线庄园照料说明和旧节会页网络异常测试已不再暴露玩家禁用词。
- 在线房间邀请面板与旧 E2E 迁移：新增 `OnlineInvitePanel`，支持批量粘贴拆分、最近联机快捷选择、已在房 / 邀请失败 / 重试 / 移除状态；龙舟浏览器 smoke 会先展开折叠后的备用创建表单再断言旧人数 test id，并把房间壳奖励断言迁到「奖励已记录」玩家态文案。
- 在线远征房创建向导接入：独立远征页和在线节会远征标签新增 `online-expedition-room-create-trigger` 主入口，复用统一 `OnlineRoomWizard` 配置路线、玩法、人数和标题；旧远征创建表单收进「备用创建表单」折叠区，并保留原路线 / 玩法 / 标题 / 人数 / 创建 test id 供旧 QA 和 E2E 迁移。
- 在线玩家文案首批收口：`qa:online-player-copy` 改用 Vue SFC 解析完整模板，避免内层 slot 模板漏扫；在线中心、统一房间壳、独立远征页和在线节会页已把「降级入口 / 开始 ready / per-member receipt / 服务端落账」替换为玩家态文案，并把远征 / 节会网络异常测试入口收进默认折叠「调试操作」，结构 QA 同步改查「备用操作」和「奖励已记录」。
- 在线节会房创建向导接入：节会房间页新增首屏「创建节会房间」主入口，打开统一 `OnlineRoomWizard` 配置房型、玩法、人数和标题；旧页面长表单收进「备用创建表单」折叠区，并保留原创建 / 房型 / 玩法 / 人数 test id 供旧 QA 和 E2E 迁移。
- 在线 UI 通用空态与状态条第一批：新增 `OnlineEmptyState`、`OnlineStatusBanner`，在线村社、在线委托、在线邻里和在线庄园页已用统一空态收拢未加入、无公开村社、无申请、无委托、无接单、无凭证 / 补偿、无公开名片、无邻里成员 / 动态 / 订阅、庄园无快照、无留言 / 来访 / 导览 / 照料记录等说明，并用统一状态条承接模块错误、加载、重试和补偿失败提示；结构 QA 增加组件 test id、`aria-live`、村社 / 委托 / 邻里 / 庄园页引用断言。
- 在线玩家文案扫描预备：新增 `qa:online-player-copy`，按玩家可见模板文本扫描开发态词并输出文件、行号、替换建议；初始阶段保持 warn-only，用于驱动首批文案替换与技术详情 allowlist 收口。
- 同居共同仓库 11.4 固元山药丹 / 蒜辛驱寒丹 / 苦瓜清暑丹共同丹炉补齐：共同丹炉新增三条辅丹的成丹 / 偏丹 / 废丹 / 奇丹白名单，结果产物写回共同仓库 `deposit` ledger；`item_policy` 升到 v45，固元山药丹按稀有保护并要求 `storage.withdraw_rare`，蒜辛驱寒丹和苦瓜清暑丹进入普通目录但协作升品仍走高价值保护；前端共同工坊、结构 QA、契约 QA 和自动炼丹权重读回同步，个人料理 126/126 与当前炼丹差集清零。
- 同居共同仓库 11.4 赛舟辣南瓜干粮普通料理补齐：共同灶台新增 `shared_spicy_pumpkin_ration` 白名单，从共同仓库扣优质南瓜酱、优质泡椒和优质芝麻油并把 `food_spicy_pumpkin_ration` 回存共同仓库 `deposit` ledger。`item_policy` 升到 v44，将该成品纳入普通目录，前端共同工坊下拉、结构 QA 和契约 QA 同步读回配方；个人料理目录已对齐，后续新增料理和稀材丹方继续按批次扩展。
...
- 同居共同仓库 11.4 林间补药 / 灵草秘药稀有料理批次：共同灶台新增林间补药和灵草秘药白名单；服务端在消费人参前要求 `storage.withdraw_rare`，授权后从共同仓库扣人参、野蘑菇、草药和蜂蜜，并把料理产物回存共同仓库 `deposit` ledger。`item_policy` 升到 v42，两类料理成品按稀有物保护，前端共同工坊下拉与结构 QA 同步读回这些配方，传说盛宴 / 丝路饺子等后续稀材料理仍留后续批次。
- 同居共同仓库 11.4 雪莲 / 瀚海稀有料理批次：共同灶台新增雪梅羹、仙人掌汤和枣糕白名单；服务端在消费雪莲、瀚海仙人掌或瀚海椰枣前要求 `storage.withdraw_rare`，授权后从共同仓库扣雪莲、蜂蜜、瀚海仙人掌、西域香料、瀚海椰枣和稻米，并把料理产物回存共同仓库 `deposit` ledger。`item_policy` 升到 v41，三类料理成品按稀有物保护，前端共同工坊下拉与结构 QA 同步读回这些配方，更多稀材料理仍留后续批次。
- 同居共同仓库 11.4 矿石系高价值料理批次：水晶矿、金矿、暗影矿、虚空矿和水晶虾进入显式稀有目录，共同灶台新增水晶冻、金矿饺、虚空精华汤、虚空药剂、矿工荣光和深渊炖菜，共同酒坊新增暗影酿白名单；服务端在消费这些矿洞稀有材料、人参或水晶虾前要求 `storage.withdraw_rare`，授权后从共同仓库扣对应材料并把料理产物回存共同仓库 `deposit` ledger。`item_policy` 升到 v40，七类料理成品按稀有物保护，前端共同工坊下拉与结构 QA 同步读回这些配方，更多稀材料理仍留后续批次。
- 20.1 共同装修拆除来源资产状态同步：`shared_decoration_removal` 的 `delivered` 回执在写入 `shared_decoration_state` 与 `shared_decoration_ledger` 后，会把可追溯的 `origin_assets.decorations` 标记为 `removed`，保留原购买 / 基金 ledger，并新增拆除回执、拆除 ledger、草案和幂等键反链；在线与离线队列均读回来源资产关闭证据，仍不改个人小屋、个人背包、共同仓库或共同基金二次扣款。
- 20.1 共同基金稀有采购入仓保护：共同基金 `rare_item_purchase` 高风险大额草案在 `delivered` 回执后会把稀有物写入共同仓库 `deposit` ledger，反链原大额基金 ledger、写 `shared_fund_deliveries`、来源资产和 `fund_high_risk_receipt_recorded` 审计，并继续读回 `withdrawal_risk_level=rare` 与 `high_value_withdrawal_required=true`；在线 / 离线回执均不改个人背包或个人铜币，后续取出仍必须走高价值共同仓库确认。
- 同居共同仓库 11.4 鲟鱼 / 龙鱼高价值鱼获料理批次：鲟鱼和龙鱼进入显式稀有目录，共同灶台新增月下刺身、鲟鱼羹、龙鱼刺身和收藏家宴白名单；服务端在消费鲟鱼、龙鱼或人参前要求 `storage.withdraw_rare`，授权后从共同仓库扣鲟鱼、龙鱼、生姜、草药、木柴、人参、南瓜和稻米并把料理产物回存共同仓库 `deposit` ledger。`item_policy` 升到 v39，四类料理成品按稀有物保护，前端共同工坊下拉与结构 QA 同步读回这些配方，虚空矿和更多稀材料理仍留后续批次。
- 同居共同仓库 11.4 人参系稀有料理批次：人参进入显式稀有目录，共同灶台新增人参汤、百草丹、长生汤、文曲星粥和鹿茸汤白名单；服务端在消费人参前要求 `storage.withdraw_rare`，授权后从共同仓库扣人参、草药、木柴、蜂蜜、稻米、茶叶和鹿茸并把料理产物回存共同仓库 `deposit` ledger。`item_policy` 升到 v38，五类料理成品按稀有物保护，前端共同工坊下拉与结构 QA 同步读回这些配方，鲟鱼、龙鱼、虚空矿和更多稀材料理仍留后续批次。
- 同居共同仓库 11.4 花酒 / 桃酒普通料理补齐：共同酒坊新增菊花酒、共同灶台新增醉仙鸡白名单，服务端从共同仓库扣普通菊花、稻米、鸡蛋、桃花酒与生姜，并把料理产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v37，将菊花、桃花酒和两类料理成品纳入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，高价值红枣、瀚海仙人掌、雪莲、鲟鱼、龙鱼和虚空矿等材料仍留后续分级批次。
- 同居共同仓库 11.4 仓库流水分级证据恢复：共同仓库 ledger 归一化重新写回 `item_policy_version`、`item_policy_id`、`item_classification`、`withdrawal_risk_level`、`high_value_withdrawal_required` 和 `item_policy`，协作加工 / 料理升品产物会在 deposit 流水上直接读到 `high_quality` 与高价值取出要求，修复 20.2 异常检测收尾后共同工坊升品保护证据丢失的回归。
- 20.2 异常检测收尾：共同仓库、共同基金和庄园照料 / 偷菜治理接入 IP / 设备风险信号与跨账号短窗聚类，仓库出入、基金预算、庄园照料和偷菜会在写资产、发奖励或写记录前 429 限流并保留治理审计；活动房间重复结算 / 奖励重放、随机 NPC 生成异常和服务端存档字段越界均已补阻断或审计证据。新增 `qa:anomaly-detection-20-2` 固定仓库 / 基金 / 庄园网络聚类阻断，`qa:activity-room-visual-state`、`qa:save-corruption-guard`、`qa:random-npc-save-guard` 已覆盖其余路径。
- 作物用途标签真实替代：新增共用用途替代规划器，灶台会在原食材不足时按 `food` 标签、风味和药性自动选同类作物补位，丹炉会按 `alchemy / medicine` 标签、风味、药性与灵性补位丹材；可制作判断、最大制作数、品质预览、批量丹炉上限、扣料和日志均读同一计划。灶台 / 丹炉界面显示“用途替代”摘要，丹炉运行槽记录 `consumedInputs`，取消加工或拆机可退回真实消耗材料；`qa:crop-use-entry-guard` 已补运行态守护。
- 同居共同仓库 11.4 丝绸 / 香料普通料理补齐：共同灶台新增锦囊玉饺、孔雀宴和香料烤羊白名单，服务端从共同仓库扣普通蚕丝、孔雀羽、西域香料、羊奶、稻米、白菜与桂花，并把料理产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v36，将蚕丝、西域香料和三类料理成品纳入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，瀚海仙人掌、雪莲、鲟鱼、龙鱼和虚空矿等高价值材料仍留后续分级批次。
- 20.1 分居冷却时间收口：分居最终剧情 / 家庭回执现在会在执行 ledger 无剩余待办、农田 / 基金 / 仓库 / 装修 / 剧情 / 孩子安排均已收口后关闭契约，写入 `closed_at`、`closed_by`、`separation_state=closed`、`separation_contract_closed` 审计，并以关闭时间刷新恋爱 / 婚姻 72 小时关系切换冷却；幂等回放允许在已关闭契约上只读返回原回执。顺手补齐庄园退出交接记录的幂等读回字段，避免重复查询丢失 `manor_exit_handover_record`。
- 同居共同仓库 11.4 面粉 / 干菜加工料理链补齐：共同晒架新增干菜、共同石磨新增面粉，馒头和干菜米粉汤可从共同仓库扣优质加工材料与普通木柴并把料理产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v35，将面粉、干菜、馒头和干菜米粉汤纳入普通目录，契约 QA 覆盖加工产物来源流水追溯，后续仍保留人参、雪莲、鲟鱼、龙鱼和虚空矿等高价值材料分级批次。
- 雇工日结反馈增强：雇佣 NPC 的喂食、浇水、收获、除草除虫、工资不足离职等睡觉结算结果会进入“睡后摘要 / 记录中心”的“雇工帮忙”分区，普通日志仍只记录原有消息，不改变雇工效率、工资、执行时机或雨天 / 饲料 / 背包规则。
- 20.3 玩家申诉与恢复：共同庄园权限面板新增“恢复默认”入口，服务端提供 `/permissions/default-restore` 幂等恢复链，按当前契约类型或家族职位回滚成员权限误配；总览恢复面板新增玩家申诉提交、安全版本列表与固定确认回滚，服务端持久化 `recovery_appeals`、写客服定位器、仓库 ledger / 安全版本引用和 `contract_safe_version_rolled_back` 审计。分居预览新增 `separation_asset_dispute_source_rows`，统一列出仓库来源 / 未知来源、高价值取出冻结、基金未知贡献、装修拆除、装修来源、建筑来源和关系剧情回看；关系破裂剧情播放前必须提交固定确认文案，回执和审计保留确认匹配。本轮仍保持不自动改个人存档、不自动还仓、不越过人工补偿边界；`qa:cohabitation-recovery-20-3` 与 `qa:online-ui-structure` 覆盖后端写链、前端入口和来源清单读回。
- 同居共同仓库 11.4 节令 / 成就普通料理补齐：共同灶台 / 茶炉新增腊八粥、冬至饺、花生汤圆、百收粥、大厨特供和交际花茶白名单，输入从共同仓库扣普通稻米、花生、野果、冬小麦、冬白菜、蜂蜜、红枣、鸡蛋、芝麻和桂花，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v34，面粉、干菜、人参、雪莲、鲟鱼、龙鱼和虚空矿等未分级或高价值材料仍留给后续批次。
- 工具升级工作耗时与矿洞节奏：工具等级新增统一工作耗时倍率，铁制 / 精钢 / 铱金分别缩短 10% / 20% / 30%，并接入开垦、播种、浇水、收获、钓鱼、淘金、竹林采集和矿洞翻格等对应工具动作；工具升级页、角色工具一览和指南同步展示“工作耗时”。矿洞战斗耗时改为按实际碾压程度计算，攻击、防御、逃跑和 BOSS 最低基础耗时都会再叠加镐等级倍率，料理 / 丹药速度增益继续由 `advanceTime()` 最后生效。
- 同居共同仓库 11.4 山药 / 蒜香 / 苦瓜家常料理补齐：普通大蒜和苦瓜补入共同仓库普通目录，共同灶台新增山药团圆粥、蒜香萝卜和苦瓜清暑汤白名单，输入从共同仓库扣普通山药、大蒜、苦瓜、萝卜、稻米和木柴，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v33，人参、雪莲、鲟鱼、龙鱼和虚空矿等高价值材料仍留给后续分级批次。
- 同居共同仓库 11.4 野蘑菇 / 松果普通料理补齐：普通野蘑菇和松果补入共同仓库普通目录，共同灶台新增蘑菇炖和猎人烤白名单，输入从共同仓库扣普通野蘑菇、松果、草药和木柴，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v32，人参、暗影矿和虚空矿相关料理继续留给后续稀材分级批次。
- 18.1 存档版本收口：主存档升至 `SAVE_VERSION=6` 并保留随机 NPC 池，联机活动房间 store / `visual_state`、作物用途筛选、共同庄园 `shared_log` 均补版本号和缺字段默认；专项 QA 覆盖旧字段迁移、默认值补齐和不重复发奖励的幂等守卫。
- 19.3 随机 NPC 对话模板第一版收口：联机节会房间最近凭证和当前房间结算凭证会在本地随机 NPC 存档内触发 `online_festival_room:*` 对话记忆，短访 / 熟人 / 长住 NPC 均可承接节会场景；长住 NPC 同步节会型家族印象，`qa:random-npc-content-guard` 固定联机节会触发器和限长记忆入口。
- 19.3 随机 NPC 家人线中段差异奖励收口：核心家族深线第 2 段在家人线中会按父母 / 手足 / 远亲 / 师门 / 商队 / 旧债 / 家族产业 / 义亲 / 前缘切换加赠物品；关系模板与家族模板 TODO 第一版收口，`qa:random-npc-content-guard` 固定中段差异加赠、整组入包和有界深线历史入口。
- 18.2 共同庄园资产来源收口：共同装修现在会写入购买者、摆放者、可分割状态、基金来源与返还 / 补偿 / 纪念化策略；共同孩子、家庭心愿和家庭重大事件统一归属契约；分居预览 / 执行清单补齐装修、家庭剧情和不可分资产补偿规则，`qa:cohabitation-contract` 覆盖个人资产不误写边界。
- 同居共同仓库 11.4 钓鱼技能普通料理补齐：共同灶台新增红烧鲶鱼白名单，普通鲶鱼补入共同仓库普通目录，输入从共同仓库扣普通鲶鱼和辣椒，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v31，鲟鱼和龙鱼料理继续留给后续高价值鱼获分级批次。
- 20.1 共同资产跨账号 / 设备防搬空：共同仓库和共同基金 ledger 新增 IP、设备与 user-agent 风险字段，仓库短窗治理会按同 IP/设备多成员网络簇阻断普通入仓、普通取出 / 卖出和高价值取出草案创建；共同基金会按短窗预算支出次数阻断单成员高频小 / 中额支出和多成员同 IP/设备协同支出，阻断写入 `warehouse_cross_device_brush_blocked` / `fund_cross_device_brush_blocked` 审计并在快照读回 `suspicious_networks`，`qa:cohabitation-contract` 已覆盖单账号高频与跨成员网络簇边界。
- 20.1 共同仓库治理申诉入口：新增 `/warehouse/governance/appeals` record-only 写链，玩家被共同仓库入仓 / 出仓高频或同 IP/设备网络簇阻断时可提交申诉；申诉保留阻断原因、ledger、网络风险与审计证据，快照读回 pending / recovery_applied，管理恢复会反链申诉 ID。共同仓库高频治理面板新增申诉表单和 pending 读回，`qa:cohabitation-contract` 与 `qa:online-ui-structure` 覆盖幂等、非 owner 申诉和资产不变边界。
- 同居共同仓库 11.4 采集 / 战斗技能普通料理批次：共同灶台新增野菜沙拉、战士口粮、战斗炖菜和铁拳汤白名单，输入从共同仓库扣普通草药、野果、土豆、鸡蛋、辣椒、姜、铁矿石和木柴，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v30，将四类采集 / 战斗技能料理成品列入普通目录，野蘑菇、人参、暗影矿和虚空矿相关料理继续留给后续分级批次。
- 20.1 恋爱 / 婚姻关系频繁切换冷却：同居契约创建和接受现在会阻断成员同时发起或接受其他进行中的恋爱 / 婚姻契约；已关闭的恋爱 / 婚姻契约会按 72 小时关系切换冷却阻断新关系，分居个人剧情回执会写入 `relationship_switch_cooldown_until` 与审计证据；`qa:cohabitation-contract` 覆盖创建、接受、关闭冷却和分居回执路径。
- 19.3 随机 NPC 绑定偏好第一版收口：`RandomNpcTemplate.preferences` 新增 `bindings`，8 个随机 NPC 模板按作物 / 宠物 / 店铺 / 庄园四类写入结构化偏好；NPC 页短访、熟人册和长住卡片可回看“绑定偏好”，`qa:random-npc-content-guard` 固定类型、模板覆盖和页面入口。
- 19.3 随机 NPC 日常成长线收口：新增 `RANDOM_NPC_RELATIONSHIP_GROWTH_BEATS`，用好感、关系方向信号和见家人门槛把短访熟人化、熟人长住、短线暧昧、长住恋爱与家人线串成可预览进度；NPC 页短访 / 熟人 / 长住卡片展示“自然成长 / 日常长出关系线”，守护脚本固定内容入口与存档上限。
- 同居分居关系剧情演出资产化收口：分居剧情计划新增 `cinematic_asset_manifest`，按恋人、婚姻、知己、家族 / 合伙和季节合耕退出记录场景、角色剪影、道具和 motion asset；共同庄园分居面板新增轻量视觉舞台，能按 cue 播放搬离、账册、孩子安排卷轴、未来合作约定、家族交接册和落印动效，播放后仍只写共同契约回执，不改个人 NPC / 家庭 / 资产主状态。
- 同居分居建筑 / 家具 selector 迁移收口：真实拆除主状态候选快照新增 `ownedEntries / placedEntries` 数量明细；共同庄园前端“解析目标”不再写 `resolved_target_*` 占位，而是从快照生成宅院改造、农舍等级、山洞用途 / 解锁、酒窖格、温室解锁、已放置装饰和未放置装饰库存的真实窄 selector，并保留 hash、补偿 / 回滚确认、`ui-snapshot-selector-proof` 与个人 receipt 幂等边界。
- 同居分居长期离线确认前端收口：共同庄园分居面板新增长期离线确认保护读回，展示待确认成员最后活跃、离线时长、保护期和服务端策略；请求执行按钮现在同时接受双方确认或“已确认成员 + 冷静期结束 + 待确认成员超过离线确认保护期”的 override 门槛，提交后读回 `offline_timeout_override_applied / confirmation_basis=offline_timeout_override`，仍只进入待人工执行，不伪造确认、不改个人存档。
- 同居分居建筑后续串联计划：分居预览、执行 ledger 和 `split-decorations-buildings` receipt 新增 `family_building_followup_plan`，按建筑读回请求真实拆除复核、复核 / 执行、个人存档回执、精确 selector 收口、回滚或人工回执下一步；审计和共同庄园分居面板同步展示 pending / completed 计数、动作队列和 `safe_to_auto_execute=false` 边界，本轮仍不在分居拆分步骤直接删除个人 `home / decoration` 或改共同资产。
- 同居分居婚姻孩子抚养判定证据：`resolveSeparationChildArrangement` 现在会生成 `custody_decision_state`、`custody_rule`、共同照料 / 主照料成员、探视规则、教育重大选择规则和 `story_rule_not_resource_split` 家庭事件边界；`writeSeparationPersonalFamilyReceipts` 会把这些字段写入个人孩子 `separationArrangementSummary`、个人家庭 receipt 和迁移摘要，前端分居面板只读展示抚养判定、照料成员、探视 / 教育规则与 `no_resource_split`。本轮不公开孩子身份，不改铜币 / 背包 / 农田 / 小屋 / NPC 关系 / 共同资产，完整家庭主状态迁移与更复杂多阶段裁决仍留后续。
- 同居分居建筑主状态证据读回：分居建筑拆分清单和 `split-decorations-buildings` receipt 现在会读取家族建筑真实拆除 / 精确个人主状态 mutation 链路，暴露 `split_status`、真实拆除个人存档回执数、`main_state_exact_mutation_receipt_count`、每建筑状态行和审计计数；共同庄园分居面板新增“建筑 / 小屋主状态证据”只读区，能区分已读回既有主状态变更回执与仍待回滚 / 拆除 / 人工回执的建筑。本轮不由分居步骤重复删除个人 `home / decoration` 主状态，不改共同基金、共同仓库、个人铜币或背包。
- 随机 NPC 存档控制收尾：长住家庭线新增旧档接续、关系线开启、婚约 / 成婚、婚后日常和节会同行触发记录，继续复用 `familyLine.reviewHistory` 最近 4 条上限；深关系长住在 112 天低活跃且锁定名额可用时会锁定归档为旧日长住摘要，锁定位不足则继续留在长住名册。
- 同居共同仓库 11.4 采矿技能普通料理批次：共同灶台新增矿石汤和铁骨汤白名单，输入从共同仓库扣普通铜矿石、铁矿石、萝卜、草药和木柴，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v29，将两类采矿技能料理成品列入普通目录，水晶矿、金矿和虚空矿相关料理继续留给后续分级批次。
- 19.2 炼丹与料理内容表收尾：新增山药团圆粥、蒜香萝卜、苦瓜清暑汤三道低幅 buff + 剧情触发料理，并新增固元山药丹、蒜辛驱寒丹、苦瓜清暑丹三枚辅丹；山药 / 蒜 / 苦瓜在百科、搜索、灶台默认食谱、丹炉配方和背包用途建议中可读回料理价值 / 炼丹价值差异，新增 `qa-alchemy-cooking-content-table` 守护料理剧情消费入口、四类炼丹结果和双路径验收。
- 同居共同仓库 11.4 农耕技能普通料理批次：共同灶台新增南瓜饼、黄金炒饭和田园盛筵白名单，输入从共同仓库扣普通南瓜、稻米、鸡蛋、玉米和西瓜，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v28，将普通西瓜和三类农耕技能料理成品列入普通目录，金西瓜等高价值西瓜仍保留稀有 / 高价值保护路径。
- 17.1 高风险功能开关收尾：`onlineVisualFeatureFlags` 从 6 个可视化 / 活动开关扩展到 18 个负责范围，新增作物炼丹 / 料理 / 加工、宠物喂食、随机 NPC、恋爱、家族、孩子、双人同居、共同仓库、共同基金和分居演算开关；在线中心面板改为“高风险功能开关”，继续展示关闭提示、降级入口、安全收尾和缺失配置保守策略，QA 同步守护完整清单。本轮只补入口层定义与验收证据，不新增奖励、不改服务端结算。
- 同居分居婚姻孩子安排个人家庭摘要写入：`writeSeparationPersonalFamilyReceipts` 现在会在婚姻同居有孩子时通过 `separation_personal_family_main_state_v1` 把分居孩子安排写入各成员个人 `npc.children[].trainingState.familyEventHistory`、`familyEventStages`、`milestoneIds` 和 `separationArrangementSummary`，并读回 `personal_child_family_event_and_custody_summary_recorded`、`mutation_adapter`、`mutated_child_count`、`family_event_receipt_count`、`custody_arrangement_recorded_count` 和幂等证明；共同庄园分居面板、前端 API 类型、`qa:cohabitation-contract` 与 `qa:online-ui-structure` 同步守护该证据。本轮不公开孩子身份，不改铜币 / 背包 / 农田 / 小屋 / NPC 关系 / 共同资产，完整家庭主状态迁移和更复杂多阶段裁决仍留后续。
- 同居分居庄园退出交接记录前端读回：共同庄园分居面板新增“庄园退出交接”只读区，消费 `manor_exit_handover_record` 展示交接记录 ID、成员职位、家主 / 负责人、职位释放策略、资产域交接状态、记录人 / 时间和个人存档 / 契约职位不改写边界；前端 API 类型与 `qa:online-ui-structure` 同步守护 `family_role_handover_executed`、`member_roles` 和 `asset_domain_handover`，本轮只补 UI 读回，不执行职位重排或个人 / 家族主状态迁移。
- 同居分居共同基金消费差额争议明细读回：共同基金差额确认摘要新增 `consumption_delta_dispute_rows`，逐行保留原主、建议返还、拆分基数、资本 / 经营 / 未知经营贡献金额、来源 ledger、确认状态和不改个人铜币 / 共同基金 / 共同仓库边界；共同庄园分居面板、前端 API 类型、`qa:cohabitation-contract` 与 `qa:online-ui-structure` 同步守护确认前、单方确认和双方确认读回，本轮只补证据明细，不改变返还金额或写回行为。
- 同居共同仓库 11.4 结婚普通料理批次：共同灶台 / 茶炉新增凤凰糕、熔岩铁锅、茶宴八珍、铁匠炖、牧场鲜奶汤和月下茶泡饭白名单，输入从共同仓库扣稻米、桂花、红枣、蜂蜜、铁矿石、辣椒、土豆、茶叶、莲子、玉米、牛奶、红薯和春笋，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v27，将六类料理成品列入普通目录，高价值或未分级输入料理留后续批次。
- 同居共同仓库 11.4 挚友普通料理批次：共同灶台 / 酒坊新增农家盛宴、秋月宴、鸳鸯酥、锻造师套餐和灵果酒白名单，输入从共同仓库扣稻米、白菜、鸡蛋、木柴、桂花鱼、河蟹、桂花、桃子、铁矿石、土豆、野果和蜂蜜，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v26，将五类料理成品列入普通目录，长生汤等人参料理暂留后续高价值材料批次。
- 同居共同仓库 11.4 相知普通料理批次：共同灶台新增老萝卜炖肉、绣囊糕、深矿炖菜和野果酱白名单，输入从共同仓库扣萝卜、木柴、稻米、桂花、土豆、铜矿石、野果和蜂蜜，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v25，将野果和四类料理成品列入普通目录，人参类百草丹暂留后续高价值材料批次。
- 同居共同仓库 11.4 家常与相识料理批次：共同晒架 / 灶台 / 茶炉新增柿饼、鱼汤面、矿工铁锅饭、炒土豆丝、芝麻糊、韭菜炒蛋、花生糖和桂花甜茶白名单，输入从共同仓库扣柿子、鲫鱼、冬小麦、铜矿石、土豆、芝麻、韭菜、鸡蛋、花生、桂花和蜂蜜等普通材料，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v24，将柿子、韭菜、花生和八类料理成品列入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方。
- 同居共同仓库 11.4 物品分级策略 v23：雪莲、瀚海仙人掌和瀚海椰枣进入显式稀有目录，普通入仓会命中 `rare_item_policy` 并保留稀有高价值草案路径；基础山药补入普通目录，莲子 / 茶叶 / 莲藕继续按普通目录处理，避免把作物用途 `valuable` 标签直接等同于仓库稀有。
- 同居共同仓库 11.4 物品分级策略 v22：桃花茶、莲心茶、蜜桃瓜、晶山药、桂花茶、雪茶、双莲、山莲、桂莲、冰火椒、秋桂瓜、冬南瓜、雪菊、雪南瓜、风瓜、云豆、雨稻、霜薯、雷菜、虹果、晨茶、暮笋、翠玉茶、寒香菊和珠桃等二代 / 三代遗留高价值杂交作物进入显式稀有目录；普通入仓命中 `rare_item_policy`，仓库快照读回 v22 与稀有高价值草案许可，基础莲子 / 茶叶 / 莲藕等普通物仍不随本批次升档。
- 同居共同仓库 11.4 物品分级策略 v21：风华 / 玉系三代二十类高价值杂交作物进入显式稀有目录，普通放入 / 取出 / 卖出会命中 `rare_item_policy`，仓库快照读回 v21、目录包含与稀有高价值草案许可；基础普通作物仍留在普通或未分类保护路径，不被本批次误升为稀有。
- 同居 11.6 离线经营第一版收口：TODO 将“任一成员上线可经营”“所有操作写入流水”“冲突以服务端状态为准”按第一版关闭；当前已接入的农田、动物、宠物、工坊、装饰、高风险回执、共同基金购买、共同仓库卖出、共同日结与离线自动收益均走服务端状态、幂等键、审计 / 业务 ledger 和离线冲突分域证据包，后续新增经营动作继续按同一写链扩展。
- 同居共同仓库 11.4 高阶杂交稀材丹方批次：共同丹炉新增雪莲清心丹、露花凝神丹和星莲安神丹及各自偏丹 / 废丹 / 奇丹白名单，输入从共同仓库扣雪莲子 / 露花 / 星莲与优质草药膏、优质莲心粉、优质桂花蜜、优质绿茶等材料，结果产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v20，将三类高阶杂交稀材输入与三类新丹药输出纳入稀有目录，稀有输入继续要求 `storage.withdraw_rare`，前端共同工坊下拉与结构 QA 同步读回这些丹方，继续保留协作成功率、来源资产、高价值 / 稀有保护和个人 / 共同基金不变边界。
- 同居共同仓库 11.4 高阶动物产物料理批次：共同灶台 / 茶炉新增鸽蛋药草汤、牦牛奶麦粥、驴奶姜茶、鸵鸟蛋宴和鹿茸药草汤白名单，输入从共同仓库扣鸽蛋 / 牦牛奶 / 驴奶 / 鸵鸟蛋 / 鹿茸及草药、冬小麦、蜂蜜、茶叶、土豆、芝麻油等普通或精加工材料，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v19，将五类成品列入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，继续保留协作升品高价值取出保护和个人 / 共同基金不变边界。
- 同居共同仓库 11.4 节令点心料理批次：共同灶台 / 茶炉新增年糕、花糕、青团、月饼、粽子、巧果、斗茶饮和纸鸢糕白名单，输入从共同仓库扣稻米 / 蜂蜜 / 桃子 / 草药 / 莲子 / 芝麻油 / 春笋 / 冬小麦 / 茶叶等普通或精加工材料，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v18，将八类节令点心 / 饮品成品列入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，继续保留协作升品高价值取出保护和个人 / 共同基金不变边界。
- 同居分居个人关系主状态前端读回：共同庄园分居面板新增只读“个人关系主状态写回”证据区，读取 `personal_relationship_mutation_summary` 展示 receipt 数量、写回适配器、影响 NPC、清理动作和资产边界；前端 API 类型与 `qa:online-ui-structure` 同步守护该摘要，继续只展示证据，不新增个人存档写链。
- 同居共同仓库 11.4 日常农家料理批次：共同灶台新增麻辣火锅、冬笋炒肉、莲子羹、玉米烙和桂花藕粉白名单，输入从共同仓库扣辣椒 / 白菜 / 冬笋 / 莲子 / 玉米 / 芝麻油 / 桂花 / 莲藕等普通材料，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v17，将五类料理成品列入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，继续保留协作升品高价值取出保护和个人 / 共同基金不变边界。
- 同居共同仓库 11.4 普通点心料理批次：共同灶台新增桂花糕、红枣糕、桃花饼白名单，输入从共同仓库扣桂花 / 红枣 / 桃子与稻米，产物回存共同仓库 `deposit` ledger；`item_policy` 升到 v16，将红枣和三类点心成品列入普通目录，前端共同工坊下拉与结构 QA 同步读回这些配方，继续保留协作升品高价值取出保护和个人 / 共同基金不变边界。
- 同居共同仓库 11.4 自动产出分级证据：共同仓库 ledger 现在统一读回 `item_policy_version`、`item_policy_id`、`item_classification`、`withdrawal_risk_level`、`high_value_withdrawal_required` 和 `item_policy`；共同农田升品收获、协作加工 / 料理升品、共同炼丹自动结果与高品质卖出均补 QA，来源资产同步保留策略版本、分类和策略 ID。前端仓库最近流水新增分级 / 高价值取出提示，继续不改个人背包、个人铜币或共同基金。
- 同居分居个人关系主状态写回：`writeSeparationPersonalStoryReceipts` 现在会在恋人 / 婚姻 / 知己拆分后用 `separation_personal_relationship_main_state_v1` 清理个人存档里可识别的 `dating / married / zhiji`、婚龄 / 知己天数和待婚 / 孕育 / 待孩子轻量字段；回执、执行 ledger、预览执行请求和审计会读回 `personal_relationship_mutation_summary`，并固定孩子、铜币、背包、农田、房屋和家庭资产不被改写，结拜 / 合伙庄园仍保持契约记录-only。
- 同居分居家族庄园退出交接记录：`resolveSeparationFamilyStory` 现在会为结拜 / 合伙庄园生成 `manor_exit_handover_record`，写入成员职位交接、家主、共同田区 / 基金 / 仓库 / 装饰 / 家族建筑等资产域收口状态和 `family_role_handover_executed` 证据；该记录只落共同契约 ledger / 预览 / 审计，不改个人存档或静默重排契约职位。
- 同居共同仓库 11.4 稀莲 / 翠桃稀有丹方：共同丹炉新增稀莲护心丹、翠桃醒神丹及各自偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质草药膏 / 莲心粉 / 蜜桃脯 / 桂花蜜和稀有莲子 / 翠桃；`item_policy` 升到 v15 并将两类输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
- 同居共同仓库 11.4 玉兰稀有丹方：共同丹炉新增玉兰凝心丹及偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质桂花蜜、优质莲心粉和稀有玉兰；`item_policy` 升到 v14 并将玉兰凝心丹输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
- 同居共同仓库 11.4 月珠稀有丹方：共同丹炉新增月珠安神丹及偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质绿茶、优质莲心粉和稀有月珠；`item_policy` 升到 v13 并将月珠安神丹输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
- 同居共同仓库 11.4 泽地稀有丹方：共同丹炉新增泽光净息丹及偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质草药膏、稀有湿地孢样和夜光藻团；`item_policy` 升到 v12 并将泽光净息丹输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
- 同居共同仓库 11.4 风蚀稀有丹方：共同丹炉新增风蚀护脉丸及偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质腌姜、优质精制石英和稀有风蚀晶核；`item_policy` 升到 v11 并将风蚀护脉丸输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
- 同居共同仓库 11.4 灵脉稀有丹方：共同丹炉新增灵脉凝神丹及偏丹 / 废丹 / 奇丹分支，输入从共同仓库扣优质绿茶、优质精制石英和稀有灵脉碎晶；`item_policy` 升到 v10 并将灵脉凝神丹输出列入稀有保护目录，服务端在消费稀有输入前要求 `storage.withdraw_rare`，产物回存共同仓库并保留 consume / deposit 流水、自动结果权重、来源资产和个人存档边界证据。
...
- 同居分居关系剧情轻量演出时间线：共同庄园分居面板会从剧情拆分的 `dialogue_lines`、`animation_cues` 和场景调度生成前端播放步骤；“播放演出”会先逐段播放恋人告别、婚姻家庭、知己约定或家族交接 cue，完成后再调用 `/record-story-cinematic` 写共同契约回执，仍不改个人 NPC、家庭、孩子或资产主状态。
...
- 同居分居共同基金争议确认前端收口：分居面板的共同基金拆分读回现在会显示未知经营贡献总额、组数、稳定 hash 和来源流水；确认按钮从只处理消费差额扩展为同时覆盖消费差额 / 未知经营贡献争议，任一争议存在且后端处于确认阶段时即可提交确认，仍只写审计，不改个人铜币、共同基金或共同仓库。
- 同居共同仓库 11.4 节会暖席料理：共同工坊新增冬笋鸭蛋粥、水牛乳米糕、鹅蛋芝麻糕和鹌鹑药蛋羹白名单，`item_policy` 升到 v7 并把这些蛋奶 / 暖席料理成品列入普通目录；服务端继续从共同仓库扣普通与精加工材料，产物回写共同仓库 `deposit` ledger，协作升品来源资产仍走高品质取出保护。
- 同居共同仓库 11.4 高阶鱼宴料理：共同工坊新增莲藕鱼卷、芝麻鳗鱼饭、桂香蟹粥和节庆鱼宴白名单，`item_policy` 升到 v6 并把这些高阶鱼宴成品列入普通目录；服务端从共同仓库扣鱼获 / 精加工材料并把料理回写共同仓库 `deposit` ledger，协作升品继续保留来源资产保护。
- 同居共同仓库 11.4 鱼肉共同料理：共同工坊新增初钓鱼汤、红烧鲤鱼、清蒸鲈鱼、枫叶烤鱼、烤鳗鱼、蟹黄汤和渔夫拼盘白名单，`item_policy` 升到 v5 并把常规鱼获与这些鱼肉料理成品列入普通目录；服务端从共同仓库扣鱼获 / 普通材料并把料理回写共同仓库 `deposit` ledger，协作升品继续保留来源资产保护。
- 同居分居关系剧情专属演出内容：`resolveSeparationFamilyStory` 会为恋人、婚姻、知己、家族 / 合伙退出生成版本化 `dialogue_lines`、`animation_cues`、场景调度和播放策略；共同契约执行 ledger、演出播放回执、审计、个人剧情 receipt 和前端只读面板都会保留这些台词 / cue 证据，仍不改个人 NPC、家庭、孩子或资产主状态。
- 同居共同仓库 11.4 动物产物共同料理：共同工坊新增蛋炒饭、水煮蛋、乌鸡蛋羹、羊奶汤、松露炒饭和驼奶茶白名单，`item_policy` 升到 v4 并把这些料理成品列入普通目录；服务端从共同仓库动物产物与普通材料扣料，产物回写共同仓库 `deposit` ledger 并保留协作升品来源资产保护。
...
- 同居离线限定装饰 / 共同装修回执：离线队列新增 `record_limited_decoration_delivery_receipt`、`record_limited_decoration_refund_receipt`、`record_shared_decoration_removal_receipt` 和 `record_shared_decoration_removal_refund_receipt`，仅对已双方确认并执行扣款的限定装饰或共同装修拆除草案提交 delivered / refunded 回执；服务端复用高风险回执权限、幂等和审计，完成回执只写共同装饰状态，退款回执在补偿确认后写 `high_risk_fund_refund` 共同基金流水并退回共同基金，不改共同仓库、个人背包或个人小屋，限定装饰退款也不写共同装饰状态，前端本地离线缓存也可加入这些回执并批量预检 / 自动合并。
- 同居离线稀有物回执：离线队列新增 `record_rare_item_delivery_receipt` 与 `record_rare_item_refund_receipt`，可对已双方确认并执行扣款的 `rare_item_purchase` 草案离线提交交付或退款回执；交付只写契约 `shared_fund_deliveries` 与审计，退款需补偿确认并写 `high_risk_fund_refund` 共同基金流水，两条路径都不改个人背包、共同仓库或共同装饰状态，前端本地离线缓存可加入稀有物回执并批量预检 / 自动合并。
- 同居离线家庭重大事件回执：离线队列新增 `record_family_major_event_receipt` 与 `record_family_major_event_refund_receipt`，可对已双方确认并执行扣款的 `family_major_event` 草案离线提交家庭事件回执或退款；交付只写契约 `family_state.major_event_ledger` 与审计，退款需补偿确认并写 `high_risk_fund_refund` 共同基金流水，两条路径都不改个人家庭主状态、共同仓库或共同装饰状态，前端本地离线缓存可加入家庭事件回执并批量预检 / 自动合并。
...
- 同居共同仓库 11.4 丹炉专属概率权重：谷气、芝香、南瓜、辛火、桂露、茶心、石根和灵桃成功丹方的自动结果读取配方专属基础权重；服务端在输出 ledger、origin asset、审计和幂等回放中保留权重档位、基础权重、最终权重、roll 与 seed hash，前端共同工坊可预览基础权重并在回执中读回最终权重。
- 同居分居共同基金消费差额确认：分居个人田区写回后，若共同基金返还行带有消费差额确认要求，`refundSeparationSharedFund` 会先要求所有已接受成员通过 `confirm-shared-fund-delta` 记录确认；前端分居面板新增“确认差额”入口和共同日志摘要，确认只写执行 ledger / 审计，不改共同基金、个人铜币或个人存档。`qa:cohabitation-contract` 覆盖未确认退款阻断、错误 hash 拒绝、双方确认幂等和确认后再退款。
- 联机房间 12.1 权威操作收口：节会 / 远征活动房间和庄园协作护理房均确认由服务端 runtime 判定房主 / 成员权限、可用动作、状态流转、结算凭证、幂等与补偿；活动房间 QA 新增伪造进度 / 分数 / 视觉 revision / 结算凭证字段的回归，证明客户端只能提交动作意图。
- 同居共同基金 12.4 异常冻结恢复：共同基金新增手动复核解冻接口，复核通过后保留原冻结事件、写入 `fund_abnormal_unfrozen` 审计、清除冻结阻断并恢复后续基金写入；QA 覆盖冻结阻断、解冻幂等、余额 / 个人存档不变和解冻后小额支出恢复。
- 同居离线共同动物买卖队列：`mergeCohabitationOfflineQueue` 支持 `buy_shared_animal` / `sell_shared_animal`，复用服务端共同动物买卖写链，成功返回动物 ledger、基金 ledger、余额前后和个人存档不变证据；个人来源动物出售、权限或余额冲突会转为队列级 `rejected` 证据。前端离线队列面板可提交共同动物买入 / 出售并读回基金支出 / 入账摘要。
- 同居离线自动收益批处理：`offline-status` 的待领摘要暴露 `target_refs` 和 `batch_claim_supported`，`collect_offline_auto_income` / 离线队列可带 `target_refs`、`batch_limit` 只领取服务端当前仍待领的部分目标，并返回批处理模式、领取前 / 剩余数量、选中 / 跳过目标和个人存档不变证据；前端队列结果摘要显示按目标批处理与剩余待领数。
- 同居共同仓库 12.1 权威状态回填：共同庄园顶部提示不再标记高价值取出和自动入仓关闭，改为说明高价值取出走草案确认、共同产出入仓由服务端落账；TODO 同步标记共同仓库当前开放操作均由服务端判定。
- 同居家族庄园 11.10 写链收口：家族订单、家族声望、公开设置和节会席位从只读预备面板推进到服务端幂等写链，覆盖订单发布 / 接取 / 交付 / 结算、声望发放 / 奖励、公开设置写入 / 回滚、锁席 / 开房 / 供品消耗 / 奖励结算；前端共同庄园页提供最小写入口，契约 QA 固定共同基金 / 共同仓库 / 声望 ledger、审计、补偿与个人存档边界。
- 同居家族庄园 11.10 批次清单同步：TODO 后段“第七批：家族庄园多人化”同步改为完成口径，与 11.10 主清单保持一致，避免旧的只读 / 未完成描述继续误导后续排期。
- 同居共同仓库 11.4 物品策略快照：共同仓库快照新增 `item_policy`，读回策略版本、普通 / 稀有 / 任务保护目录、默认未分类拒绝策略、任务物禁止高价值草案声明和当前可见库存逐项分级；前端仓库策略卡展示策略版本与目录计数，契约 QA 固定普通、稀有、任务保护和未分类默认保护证据。
...
- 同居权限面板 11.8 收口：共同庄园权限页开放农田、动物、仓库、建设装修、资金和家庭完整业务权限矩阵；服务端补齐农田铲除 / 高级肥料独立权限消费，并让庄园扩建、限定装饰、共享装修拆除、家庭重大事件和稀有物购买的大额草案 / 执行 / 已交付回执记录 `required_permission_keys` 且按业务权限重检。
...
- 作物用途标签第三十八批三代杂交第三组：风华麦、云华芝、雨华椒、霜华参、雷华芽改为人工用途档案，三代杂交已推进到雷华芽；背包和百科可读回风华续行丹、云华润脉丹、雨华行气丹、霜参固元丹、雷芽行气丹、风华麦粉、云华芝油、腌雨华椒、霜华参粉、腌雷华芽等出口。
- 作物用途标签第三十七批三代杂交第二组：虹果、露花、晨茶、暮笋、星莲改为人工用途档案，三代杂交已推进到星莲；背包和百科可读回虹果润脉丹、露花凝神丹、晨茶凝神丹、暮笋护脉丹、星莲安神丹、虹果虹酿、露花香油、晨茶花酿、暮笋粉、星莲粉等出口。
- 作物用途标签第三十六批三代杂交首组：风瓜、云豆、雨稻、霜薯、雷菜改为人工用途档案，三代杂交已推进到雷菜；背包和百科可读回风瓜凝神丹、云豆行气丹、雨稻续行丹、霜薯固元丹、雷菜行气丹、风瓜果酿、云豆粉、雨稻米粉、霜薯粉、腌雷菜梗等出口。
...
- 公共订单接力故事凭证：多段订单交付、确认和补偿重放会把接力故事章节 ID、摘要、详情和结算摘要固化到服务端结算凭证；村社公共订单板最近公开凭证与在线委托页可读回同一故事字段，不改奖励、分账金额、幂等键或个人 / 共同基金落账路径。
...
- 作物用途标签第二十五批一代杂交后续：菊稻、花生玉米、霜甜瓜、双谷、莲白菜改为人工用途档案，背包和百科可读回菊稻凝神丹、坚谷固元丹、霜瓜清暑丹、双谷续行丹、莲白清心丹、菊稻米粉、花玉米油、双谷粉、腌莲白菜等出口。
- 作物用途标签第二十六批一代杂交后续：蒜芝、韭丝瓜、芥茄、冰火椒、冬玉米改为人工用途档案，背包和百科可读回蒜芝行气丹、韭瓜行气丹、芥茄护脉丹、冰火调息丹、冬玉米续行丹、蒜芝香油、腌韭丝瓜、腌冰火椒、冬玉米粉等出口。
- 作物用途标签第二十七批一代杂交后续：琥珀薯、双花、山花生、秋桂南瓜、姜山药改为人工用途档案，背包和百科可读回琥珀固元丹、双花清目丹、山仁固元丹、秋桂养胃丹、姜山固元丹、琥珀薯粉、双花花酿、山花生油、秋桂南瓜粉、腌姜山药等出口。
- 作物用途标签第二十八批一代杂交后续：金柿、菊枣、桂薯、冬南瓜、翡翠山药改为人工用途档案，背包和百科可读回金柿养胃丹、菊枣养气丹、桂薯固元丹、冬瓜养胃丹、翠山固元丹、金柿果酒、菊枣花酿、桂薯粉、冬南瓜粉、翡翠山药粉等出口。
- 作物用途标签第二十九批一代杂交后续：雪菊、桂蒜、麦山药、白花生、蒜枣改为人工用途档案，背包和百科可读回雪菊清心丹、桂蒜行气丹、麦山固元丹、白仁润脉丹、蒜枣养气丹、雪菊花酿、腌桂蒜瓣、麦山药粉、白花生油、蒜枣果酒等出口。
- 作物用途标签第三十批一代杂交后续：韭柿、芥姜、雪南瓜、碧白菜、蒜白菜改为人工用途档案，背包和百科可读回韭柿温中丹、芥姜驱寒丹、雪瓜养胃丹、碧白清络丹、蒜白行气丹、韭柿果酒、腌芥姜片、雪南瓜粉、腌碧白菜、腌蒜白菜等出口。
- 作物用途标签第三十一批一代杂交后续：长青菜、麦芥菜、百蒜王、翠麦、韭芥改为人工用途档案，背包和百科可读回长青清络丹、麦芥行气丹、百蒜行气丹、翠麦续行丹、韭芥行气丹、腌长青菜、麦芥粉、腌百蒜王、翠麦清酒、腌韭芥等出口。
- 作物用途标签第三十二批一代杂交后续：玉笋棒、姜翠菜、麻辣仁、蜜丝瓜、花薯改为人工用途档案，背包和百科可读回玉笋固元丹、姜翠驱寒丹、麻仁聚火丹、蜜瓜润脉丹、花薯固元丹、玉笋棒粉、腌姜翠菜、麻辣仁油、蜜仁香油、花薯粉等出口。
- 作物用途标签第三十三批一代杂交尾段：辛枣、豆茄、菊柿、紫玉薯、雪莲子改为人工用途档案，一代杂交段已补至二代杂交分隔线前；背包和百科可读回辛枣养气丹、豆茄护脉丹、菊柿清目丹、紫玉固元丹、雪莲清心丹、辛枣果酒、豆茄粉、菊柿果酒、紫玉薯粉、雪莲子酒等出口。
...
### 2026-06-02 联机共同庄园审计降噪

- Agent B：共同庄园的共享日志现在把玩家摘要和技术详情拆开，主路径不再直接露出 `hash / ledger / receipt / record-only / revision / idempotent` 等开发态词；原始动作、记录编号和完整字段保留在默认折叠详情里，方便 QA 追踪。
- Agent B：同步清理安全版本、分居证据、补偿审计和建筑明细里的玩家可见技术文案，并修复共同庄园模板里误显示的 `RotateCcw,` 文本。
- Agent B：跨域最小收口旧节会页的断线调试按钮，把「模拟断线」移入默认折叠技术详情并改为「网络异常测试」；玩家文案扫描当前已无禁用词命中。

### 2026-06-03 联机邻里名片降噪

- Agent B：在线邻里的公开名片首屏现在只展示摘要和编辑入口，名片编辑长表单改由通用弹窗承接；保存失败会留在弹窗并保留草稿，旧测试入口继续可定位。

### 2026-06-03 房间流程移动端 smoke 收束

- Agent A：`qa-mobile-ui-smoke` 新增共同庄园家族节会确认 390x844 / 360x780 场景，覆盖结算确认弹窗的影响对象、资产变化、恢复提示、确认文字门槛、禁用确认按钮和无横向溢出；`OUI-QA-004` 已回写为完成。
