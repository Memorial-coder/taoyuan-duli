...
- 同居分居婚姻孩子安排个人家庭事件写入：`writeSeparationPersonalFamilyReceipts` 现在会在婚姻同居有孩子时通过 `separation_personal_family_main_state_v1` 把分居孩子安排写入各成员个人 `npc.children[].trainingState.familyEventHistory`，并读回 `personal_child_family_event_recorded`、`mutation_adapter`、`mutated_child_count`、`family_event_receipt_count` 和幂等证明；共同庄园分居面板、前端 API 类型、`qa:cohabitation-contract` 与 `qa:online-ui-structure` 同步守护该证据。本轮不公开孩子身份，不改铜币 / 背包 / 农田 / 小屋 / NPC 关系 / 共同资产，完整抚养权和家庭主状态迁移仍留后续。
