...
- 同居分居个人家庭主状态迁移回执读回：`writeSeparationPersonalFamilyReceipts` 现在会把 `personal_family_main_state_migration_summary` 写入每位成员家庭回执、执行 ledger、预览执行请求和审计，固定 adapter、receipt 数、孩子数、后续迁移状态，以及孩子 / 家庭 / NPC / 铜币 / 背包 / 农田 / 小屋 / 共同资产不变边界；共同庄园分居面板新增“个人家庭主状态迁移证明”只读区，前端 API 类型、`qa:cohabitation-contract` 与 `qa:online-ui-structure` 同步守护读回。本轮仍不执行真实孩子 / 家庭主状态 mutation。
