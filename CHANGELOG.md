...
- `L50` 这一轮先把每周交换站做成了第一版可用入口：商店页现在能直接打开“每周交换站”，看到按现实周轮换的官方控价换物单，不再只停留在联机规划里的概念描述。
- 交换站当前走的是“慢交易 / 官方控价”路线，而不是玩家自由挂单：会固定保留两条基础换物，再按周补两条轮换单，并真实校验个人周限次、站点余量和存档内物资扣增。
- 后台这条链路也已经真正落地到服务端：每周站点配置、个人兑换记录和落账后的物资变化都会持久化保存，在线 smoke 也补进了站点读取、执行兑换和回读存档三段验证，避免“前端显示换到了，回档后没写进去”的半结算。
