...
- 同居离线限定装饰 / 共同装修回执：离线队列新增 `record_limited_decoration_delivery_receipt`、`record_shared_decoration_removal_receipt` 和 `record_shared_decoration_removal_refund_receipt`，仅对已双方确认并执行扣款的限定装饰或共同装修拆除草案提交 delivered / refunded 回执；服务端复用高风险回执权限、幂等和审计，完成回执只写共同装饰状态，退款回执在补偿确认后写 `high_risk_fund_refund` 共同基金流水并退回共同基金，不改共同仓库、个人背包或个人小屋，前端本地离线缓存也可加入这些回执并批量预检 / 自动合并。
