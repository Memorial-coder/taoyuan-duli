...
assert.equal(firstSteal.snapshot.visitor_activity_entries[0]?.visitor_daily_progress, '1/2', 'visitor activity audit should expose visitor daily progress')
assert.equal(firstSteal.snapshot.visitor_activity_entries[0]?.manor_daily_progress, '1/6', 'visitor activity audit should expose manor daily progress')
assert.equal(firstSteal.snapshot.visitor_activity_entries[0]?.object_daily_progress, '1/1', 'visitor activity audit should expose object daily progress')
assert.equal(firstSteal.snapshot.visitor_activity_entries[0]?.owner_reserved_percent, 100, 'visitor activity audit should expose owner reserved percent')
assert.ok(firstSteal.snapshot.visitor_activity_entries[0]?.risk_flags?.includes('object_daily_limit_reached'), 'visitor activity audit should expose risk flags')
