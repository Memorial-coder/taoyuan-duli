...
function assertTemplateSections(answer, label) {
  for (const section of ['结论：', '原因：', '步骤：', '注意事项：', '依据：']) {
...
assert.match(result.answer, /结构化公开资料回答：青菜/, 'resource source should keep structured lead');
...
assert.match(result.answer, /任务诊断/, 'task diagnosis should use task diagnosis wording');
assert.match(result.answer, /接取状态|库存|数量|前置|季节/, 'task diagnosis should include condition checks');
assert.match(result.answer, /任务阻塞|资源缺口/, 'task diagnosis should include blocker');
assert.match(result.answer, /下一步路线/, 'task diagnosis should include actionable route');

result = await ask('我今天该做什么', {
  routeName: 'farm',
  contextSnapshot: planningSnapshot,
});
assert.match(result.answer, /本地诊断/, 'today planning should use local diagnostics');
...
assert.match(result.answer, /结构化公开资料回答：鱼塘/, 'system explanation should use structured system entry');
